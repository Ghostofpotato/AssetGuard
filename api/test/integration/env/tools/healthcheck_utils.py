...
ASSETGUARD_LOG_PATH = '/var/assetguard-manager/logs/assetguard-manager.log'

# Variable used to compare default daemons_check.txt with an output with cluster disabled
CHECK_CLUSTERD_DAEMON = '1c1\n< assetguard-manager-clusterd not running...\n---\n> assetguard-manager-clusterd is running...\n'
...
def get_response(request_method, url, headers):
    """Make a AssetGuard API request and get its response.
...
def get_agent_health_base():
    ...
    shared_conf_restart = os.system(
        f"grep -q 'agentd: INFO: Agent is reloading due to shared configuration changes' {ASSETGUARD_LOG_PATH}")
    agent_connection = os.system(f"grep -q 'agentd: INFO: (4102): Connected to the server' {ASSETGUARD_LOG_PATH}")
    ...
    if shared_conf_restart == 0 and agent_connection == 0:
        ...
        output = os.popen(
            ...
            f"\|agentd: INFO: (4102): Connected to the server' {ASSETGUARD_LOG_PATH}").read().split("\n")[:-1]
...
def get_manager_health_base():
    return get_master_health() if socket.gethostname() == 'assetguard-master' else get_worker_health()
