...
framework = path.join(base, 'framework', 'assetguard')
...
def extract_module_from_path(file_path):
    ...
    assetguard_modules = parts[-1]
    return assetguard_modules
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        mapping_list = list()
        for module in assetguard_modules:
            chdir(module)
    ...
    # Calculate mappings for a given file
        ...
        if not path.exists(path.join(base, file_rel_path)):
            print('The relative path does not exist. Example: "framework/assetguard/agent.py"')
            exit(0)
