...
- Test the mapping
    Run the script with a relative path from the `assetguard` folder to check if a file is mapped.
    ```
    python3 _test_mapping.py framework/assetguard/agent.py
    ```
    ...
    > **NOTE:** Please use a relative path when trying the testing mode. For instance: `/home/assetguard/Desktop/git/assetguard/framework/assetguard/agent.py` -> `framework/assetguard/agent.py`
