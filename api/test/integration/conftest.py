...
def build_and_up(interval: int = 10, build: bool = True):
    ...
    if build:
        ...
        response = requests.get(f"https://github.com/assetguard/assetguard/tarball/{current_branch}")
        ...
        while retries < max_retries:
            if build:
                build_process = subprocess.Popen(["docker", "compose", "build",
                    "--build-arg", f"ASSETGUARD_BRANCH={current_branch}", "--no-cache"],
...
def check_health(node_type: str = 'manager', agents: list = None):
    """Check the AssetGuard nodes health.
    ...
    if node_type == 'manager':
        ...
        for node in nodes_to_check:
            health = subprocess.check_output(
                f"docker inspect env-assetguard-{node}-1 -f '{{{{json .State.Health.Status}}}}'",
                shell=True)
        ...
        for agent in agents:
            health = subprocess.check_output(
                f"docker inspect env-assetguard-agent{agent}-1 -f '{{{{json .State.Health.Status}}}}'",
                shell=True)
            if not health.startswith(b'"healthy"'):
                return False
...
def save_logs(test_name: str):
    """Save API, cluster and AssetGuard logs from every cluster node and AssetGuard logs from every agent if tests fail.
    Save haproxy-lb log.
        ...
        for log in logs:
            try:
                subprocess.check_output(
                    f"docker cp env-assetguard-{node}-1:{os.path.join(manager_logs_path, log)} "
...
def api_test(request: _pytest.fixtures.SubRequest):
    ...
    while retries < max_retries:
        ...
        try:
            error_message = subprocess.check_output(["docker", "exec", "-t", "env-assetguard-master-1", "sh", "-c",
