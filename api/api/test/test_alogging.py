...
REQUEST_HEADERS_TEST = {'authorization': 'Basic d2F6dWg6cGFzc3dvcmQxMjM='}  # assetguard:password123
...
def test_assetguardjsonformatter(message, dkt):
    """Check assetguard json formatter is working as expected.
    ...
    with patch('api.alogging.logging.LogRecord') as mock_record:
        ...
        wjf = alogging.AssetGuardJsonFormatter()
        log_record = {}
...
def test_custom_logging(path, hash_auth_context, body, loggerlevel):
    ...
    user, remote, method = ('assetguard', '1.1.1.1', 'POST')
