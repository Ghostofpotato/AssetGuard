...
brief: The 'assetguard-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'assetguard-agentd' daemon behaves when there are delays
       between connection attempts to the 'assetguard-manager-remoted' daemon using TCP protocol.
       The 'assetguard-manager-remoted' program is the server side daemon that communicates with the agents.
...
references:
    - https://documentation.assetguard.com/current/user-manual/registering/index.html
...
from assetguard_testing.modules.agentd.configuration import AGENTD_DEBUG, AGENTD_WINDOWS_DEBUG, AGENTD_TIMEOUT
from assetguard_testing.modules.agentd.patterns import AGENTD_TRYING_CONNECT, AGENTD_CONNECTED_TO_SERVER
...
def test_agentd_parametrized_reconnections(test_metadata, set_assetguard_configuration, configure_local_internal_options,
    ...
    assertions:
        - Verify that when the 'assetguard-agentd' daemon initializes, it connects to
          the 'assetguard-manager-remoted' daemon of the manager before reaching the maximum number of attempts.
    ...
    assetguard_log_monitor.start(accumulations = test_metadata['MAX_RETRIES'], callback=callbacks.generate_callback(AGENTD_TRYING_CONNECT,{'IP':'','PORT':''}))
    assert (assetguard_log_monitor.callback_result != None), f'Trying to connect to server message not found expected times'

    #Check number of connected message is the expected
    if test_metadata['ENROLL'] == 'yes':
        assetguard_log_monitor.start(callback=callbacks.generate_callback(AGENTD_CONNECTED_TO_SERVER))
