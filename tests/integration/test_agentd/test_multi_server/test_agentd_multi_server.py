...
brief: A AssetGuard cluster is a group of AssetGuard managers that work together to enhance the availability
...
from assetguard_testing.constants.ports import DEFAULT_SSL_REMOTE_CONNECTION_PORT
...
def test_agentd_multi_server(test_configuration, test_metadata, set_assetguard_configuration, configure_local_internal_options, truncate_monitored_files,
    ...
    assertions:
        - Agent without keys. Verify that all servers will refuse the connection to the 'assetguard-manager-remoted' daemon
        ...
        - Agent without keys. Verify that the first server only has enrollment available, and the third server
          only has the 'assetguard-manager-remoted' daemon available. The agent should enroll in the first server and
