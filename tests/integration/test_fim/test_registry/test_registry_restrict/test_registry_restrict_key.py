...
references:
    ...
    - https://documentation.assetguard.com/current/user-manual/reference/assetguard-conf/syscheck.html#windows-registry
...
from assetguard_testing.modules.fim.patterns import EVENT_TYPE_ADDED, EVENT_TYPE_DELETED, IGNORING_DUE_TO_RESTRICTION
...
from assetguard_testing.modules.fim.utils import get_fim_event_data, delete_registry
...
def test_restrict_key(test_configuration, test_metadata,configure_local_internal_options,
                            truncate_monitored_files, set_assetguard_configuration, daemons_handler, detect_end_scan, create_registry_key):
    '''
    description: Check if the 'assetguard-syscheckd' daemon detects or ignores events in monitored registry entries
    ...
    if test_metadata['triggers_event']:
        assetguard_log_monitor.start(callback=generate_callback(EVENT_TYPE_ADDED))
        ...
        assetguard_log_monitor.start(callback=generate_callback(EVENT_TYPE_DELETED))
        assert assetguard_log_monitor.callback_result
        event = get_fim_event_data(assetguard_log_monitor.callback_result)
        ...
        assetguard_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_RESTRICTION))
        assert assetguard_log_monitor.callback_result

        delete_registry(win32con.HKEY_LOCAL_MACHINE, test_metadata['sub_key'], KEY_WOW64_64KEY if test_metadata['arch'] == 'x64' else KEY_WOW64_32KEY)

        assetguard_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_RESTRICTION), only_new_events=True)
        assert not assetguard_log_monitor.callback_result
