...
from assetguard_testing.constants.platforms import MACOS, WINDOWS
from assetguard_testing.constants.paths.logs import ASSETGUARD_LOG_PATH
from assetguard_testing.modules.fim.configuration import SYSCHECK_DEBUG, RT_DELAY
from assetguard_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from assetguard_testing.modules.fim.patterns import EVENT_TYPE_MODIFIED, EVENT_TYPE_ADDED, ERROR_MSG_FIM_EVENT_NOT_DETECTED, \
                                               EVENT_TYPE_DELETED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
from assetguard_testing.modules.fim.utils import make_diff_file_path, get_fim_event_data
from assetguard_testing.tools.monitors.file_monitor import FileMonitor
from assetguard_testing.utils.file import write_file_write, delete_files_in_folder, truncate_file
from assetguard_testing.utils.string import generate_string
...
def test_reports_file_and_nodiff(test_configuration, test_metadata, configure_local_internal_options,
                        truncate_monitored_files, set_assetguard_configuration, create_paths_files, clean_fim_sync_db, daemons_handler, detect_end_scan):
    '''
    description: Check if the 'assetguard-syscheckd' daemon reports the file changes (or truncates if required)
    ...
    input_description: A test case is contained in external YAML files (configuration_report_changes_and_diff.yaml, cases_report_changes_and_diff.yaml)
                       which includes configuration settings for the 'assetguard-syscheckd' daemon and, these are
    ...
    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=30)
    assert assetguard_log_monitor.callback_result, ERROR_MSG_FIM_EVENT_NOT_DETECTED
    ...
    assetguard_log_monitor = FileMonitor(ASSETGUARD_LOG_PATH)
    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_REPORT_CHANGES), timeout=60)
    assert assetguard_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'No content changes were found for this file.' in str(assetguard_log_monitor.callback_result[0]), 'Wrong content_changes field'
    ...
    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED), timeout=20)
    assert assetguard_log_monitor.callback_result
    event = get_fim_event_data(assetguard_log_monitor.callback_result)
    ...
    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_DELETED))
    assert get_fim_event_data(assetguard_log_monitor.callback_result)['file']['mode'] == test_metadata.get('fim_mode')
