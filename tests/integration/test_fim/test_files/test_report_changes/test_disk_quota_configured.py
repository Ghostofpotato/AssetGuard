...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the 'queue/diff/local' folder, where AssetGuard stores the compressed files used to perform
...
from assetguard_testing.modules.fim.patterns import DIFF_DISK_QUOTA_LIMIT, ERROR_MSG_DISK_QUOTA_LIMIT
...
def test_disk_quota_default(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'assetguard-syscheckd' daemon limits the size of the folder where the data used to perform
    ...
    assetguard_log_monitor.start(generate_callback(DIFF_DISK_QUOTA_LIMIT), timeout=30)
    callback_result = assetguard_log_monitor.callback_result
    assert callback_result, ERROR_MSG_DISK_QUOTA_LIMIT
    assert str(assetguard_log_monitor.callback_result[0]) == str(DISK_QUOTA_DEFAULT_VALUE), 'Wrong value for disk_quota'
