...
from assetguard_testing.modules.fim.patterns import FILE_SIZE_LIMIT_REACHED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED, \
...
def test_file_size_default(test_configuration, test_metadata, configure_local_internal_options,
    ...
    assert 'More changes...' in str(assetguard_log_monitor.callback_result), 'Wrong content_changes field'
    ...
    assetguard_log_monitor.start(callback=generate_callback(DIFF_FOLDER_DELETED), timeout=60)
    ...
    assetguard_log_monitor.start(callback=generate_callback(FILE_SIZE_LIMIT_REACHED), timeout=30)
