...
references:
    ...
    - https://documentation.assetguard.com/current/user-manual/reference/assetguard-conf/syscheck.html#ignore
...
import pytest
from assetguard_testing import global_parameters
...
from assetguard_testing.modules.fim.patterns import EVENT_TYPE_ADDED, IGNORING_DUE_TO_SREGEX, IGNORING_DUE_TO_PATTERN
from assetguard_testing.modules.fim.utils import get_fim_event_data
...
# Skipped on Windows due Issue https://github.com/assetguard/assetguard/issues/9298"
...
def test_ignore_subdirectory(test_configuration, test_metadata, configure_local_internal_options,
                             ...
                             file_to_monitor):
    '''
    description: Check if the 'assetguard-syscheckd' daemon ignores the files that are in a monitored subdirectory
    ...
    input_description: Different test cases are contained in external YAML files
                       ...
                       for the 'assetguard-syscheckd' daemon and testing directories to monitor.
    ...
    if test_metadata.setdefault('xfail', False):
        pytest.xfail(reason="Expected fail - Issue https://github.com/assetguard/assetguard/issues/22186.")

    assetguard_log_monitor = FileMonitor(ASSETGUARD_LOG_PATH)

    if test_metadata.setdefault('triggers_event', False) == True:
        assetguard_log_monitor.start(timeout=60, callback=generate_callback(EVENT_TYPE_ADDED))
        ...
        if test_metadata['is_pattern'] == False:
            assetguard_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_SREGEX))
            assert assetguard_log_monitor.callback_result
        else:
            assetguard_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_PATTERN))
            assert assetguard_log_monitor.callback_result
