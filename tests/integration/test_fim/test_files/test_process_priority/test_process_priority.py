...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the 'assetguard-syscheckd' daemon set in the 'process_priority' tag is applied successfully.
       The FIM capability is managed by the 'assetguard-syscheckd' daemon, which checks
...
references:
    ...
    - https://documentation.assetguard.com/current/user-manual/reference/assetguard-conf/syscheck.html#process-priority
...
from assetguard_testing.modules.agentd.configuration import AGENTD_DEBUG
from assetguard_testing.constants.platforms import MACOS
from assetguard_testing.utils.services import search_process_by_command
from assetguard_testing.constants.daemons import SYSCHECK_DAEMON
...
def test_process_priority(test_configuration, test_metadata, configure_local_internal_options,
                             truncate_monitored_files, set_assetguard_configuration, folder_to_monitor, daemons_handler):
    '''
    description: Check if the process priority of the 'assetguard-syscheckd' daemon set in the 'process_priority' tag
                 ...
                 the 'assetguard-syscheckd' process. Finally, the test will compare the current process priority
    ...
    assertions:
        - Verify that the 'assetguard-syscheckd' daemon is running.
        - Verify that the process priority of the 'assetguard-syscheckd' daemon matches the 'process_priority' tag.

    input_description: A test case (assetguard_conf) is contained in external YAML file (cases_process_priority.yaml)
                       which includes configuration settings for the 'assetguard-syscheckd' daemon and,
