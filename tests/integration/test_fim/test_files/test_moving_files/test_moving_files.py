...
from assetguard_testing.modules.fim.patterns import EVENT_TYPE_DELETED, EVENT_TYPE_ADDED
...
def test_moving_file_to_whodata(test_configuration, test_metadata, set_assetguard_configuration, truncate_monitored_files,
                                configure_local_internal_options, create_paths_files, daemons_handler, start_monitoring):
    '''
    description: Check if the 'assetguard-syscheckd' daemon detects events when moving files from a directory
    ...
    # Check event 'delete'
    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_DELETED), timeout=60)
    ...
    # Check event 'add'
    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=60)
    callback_result = assetguard_log_monitor.callback_result
    assert callback_result
