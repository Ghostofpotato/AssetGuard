...
from assetguard_testing.modules.fim.patterns import FIM_EVENT_JSON
...
def test_fim_disabled(test_configuration, test_metadata, set_assetguard_configuration, truncate_monitored_files,
                      configure_local_internal_options, folder_to_monitor, daemons_handler):

    '''
    description: Check if the 'assetguard-syscheckd' daemon generates FIM events when it is disabled
    ...
    assertions:
        - Verify that when the 'assetguard-syscheckd' daemon is disabled, no FIM events are generated.
    ...
    assetguard_log_monitor.start(generate_callback(FIM_EVENT_JSON))
