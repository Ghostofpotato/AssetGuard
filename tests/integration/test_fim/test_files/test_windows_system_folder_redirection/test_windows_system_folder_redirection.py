...
from assetguard_testing.modules.fim.patterns import EVENT_TYPE_ADDED, WIN_CONVERT_FOLDER
...
def test_windows_system_monitoring(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'assetguard-syscheckd' monitors the windows system folders (System32 and SysWOW64) properly,
    and that monitoring for Sysnative folder is redirected to System32 and works properly.

    test_phases:
        - setup:
            - Set assetguard configuration and local_internal_options.
    ...
    if test_metadata['redirected']:
        assetguard_log_monitor.start(callback=generate_callback(WIN_CONVERT_FOLDER))
