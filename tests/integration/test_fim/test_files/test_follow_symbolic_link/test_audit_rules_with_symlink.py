...
from assetguard_testing.modules.fim.patterns import AUDIT_RULES_RELOADED, EVENT_TYPE_MODIFIED, LINKS_SCAN_FINALIZED
from assetguard_testing.modules.monitord.configuration import MONITORD_ROTATE_LOG
from assetguard_testing.modules.fim.configuration import SYMLINK_SCAN_INTERVAL, SYSCHECK_DEBUG
from assetguard_testing.tools.monitors.file_monitor import FileMonitor
from assetguard_testing.utils import file, commands
...
def test_audit_rules_with_symlink(test_configuration, test_metadata, set_assetguard_configuration, truncate_monitored_files,
                                  configure_local_internal_options, folder_to_monitor, symlink, symlink_new_target,
                                  daemons_handler, start_monitoring):
    '''
    description: Check if the 'assetguard-syscheckd' daemon removes the 'audit' rules when the target of
    ...
    assetguard_log_monitor.start(generate_callback(LINKS_SCAN_FINALIZED))
    assert assetguard_log_monitor.callback_result
    assetguard_log_monitor.start(generate_callback(AUDIT_RULES_RELOADED))

    assetguard_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED))
