...
brief: These tests will check if the 'assetguard-manager-authd' daemon correctly handles the enrollment requests,
       ...
       The 'assetguard-manager-authd' daemon can automatically add a AssetGuard agent to a AssetGuard manager and provide
...
def test_assetguard_auth_messages(test_configuration, test_metadata, set_assetguard_configuration,
    ...
    '''
    description:
        Checks if when the `assetguard-manager-authd` daemon receives different types of enrollment requests,
