...
brief: These tests will check if the 'remote enrollment' option of the 'assetguard-manager-authd' daemon
       settings is working properly. The 'assetguard-manager-authd' daemon can automatically add
...
from assetguard_testing.constants.ports import DEFAULT_SSL_CLUSTER_PORT
...
def test_remote_enrollment(test_configuration, test_metadata, set_assetguard_configuration,
                           truncate_monitored_files, daemons_handler):
    '''
    description:
        Checks if the 'assetguard-manager-authd' daemon remote enrollment is enabled/disabled according
    ...
    file_monitor.FileMonitor(ASSETGUARD_LOG_PATH).start(timeout=5,
