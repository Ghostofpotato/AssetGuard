...
brief: AssetGuard-db is the daemon in charge of the databases with all the AssetGuard persistent information, exposing a socket
       to receive requests and provide information. The AssetGuard core uses list-based databases to store information
       related to agent keys, and FIM/Rootcheck event data.
       AssetGuard-db confirms that is able to save, update and erase the necessary information into the corresponding
...
references:
    - https://documentation.assetguard.com/current/user-manual/reference/daemons/assetguard-manager-db.html
...
from assetguard_testing.constants.daemons import ASSETGUARD_DB_DAEMON
from assetguard_testing.tools.monitors import file_monitor
from assetguard_testing.utils.callbacks import make_callback
from assetguard_testing.modules.assetguard_db import ASSETGUARD_DB_PREFIX
from assetguard_testing.utils import configuration
from assetguard_testing.utils.database import query_wdb
from assetguard_testing.utils.db_queries.agent_db import agent_integrity_check
...
# Variables
receiver_sockets_params = [(ASSETGUARD_DB_SOCKET_PATH, 'AF_UNIX', 'TCP')]
ASSETGUARD_DB_CHECKSUM_CALCULUS_TIMEOUT = 20
...
# List items -> (assetguard_daemon: str,(
#                mitm: ManInTheMiddle
#                daemon_first: bool))
# Example1 -> ('assetguard-manager-clusterd', None)              Only start assetguard-manager-clusterd with no MITM
# Example2 -> ('assetguard-manager-clusterd', (my_mitm, True))   Start MITM and then assetguard-manager-clusterd
monitored_sockets_params = [(ASSETGUARD_DB_DAEMON, None, True)]
...
def validate_assetguard_db_response(expected_output, response):
    """ Method to validate the AssetGuard-DB response.
...
def test_assetguard_db_messages_agent(test_metadata, configure_sockets_environment_module, connect_to_sockets_module,
                                 clean_databases, clean_registered_agents, insert_agents_test):
    '''
    description: Check that every input agent message in assetguard-manager-db socket generates the proper output to assetguard-manager-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of assetguarddb.
        - insert_agents_test:
            type: fixture
    ...
    for index, stage in enumerate(test_metadata['test_case']):
        ...
        if 'use_regex' in stage and stage['use_regex'] == 'yes':
            ...
            match = validate_assetguard_db_response(expected_output, response)
...
def test_assetguard_db_messages_global(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in assetguard-manager-db socket generates the proper output to assetguard-manager-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_assetguard_db_range_checksum(configure_sockets_environment_module, connect_to_sockets_module,
    ...
    log_monitor.start(callback=make_callback('range checksum: Time: ', prefix=ASSETGUARD_DB_PREFIX,
                                             escape=True), timeout=ASSETGUARD_DB_CHECKSUM_CALCULUS_TIMEOUT)
    ...
    agent_integrity_check()
    log_monitor.start(callback=make_callback('range checksum avoided', prefix=ASSETGUARD_DB_PREFIX,
                                             escape=True), timeout=ASSETGUARD_DB_CHECKSUM_CALCULUS_TIMEOUT)
...
def test_assetguard_db_messages_tasks(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in assetguard-manager-db socket generates the proper output to assetguard-manager-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
