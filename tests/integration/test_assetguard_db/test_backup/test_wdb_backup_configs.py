...
brief: AssetGuard-db is the daemon in charge of the databases with all the AssetGuard persistent information, exposing a socket
       to receive requests and provide information. AssetGuard-db has the capability to do automatic database backups, based
...
from assetguard_testing.constants.paths.logs import ASSETGUARD_PATH, ASSETGUARD_LOG_PATH
from assetguard_testing.utils.time import validate_interval_format, time_to_seconds
from assetguard_testing.modules.assetguard_db import patterns
...
# Configuration
t_config_path = Path(CONFIGURATIONS_FOLDER_PATH, 'configuration_assetguard_db_backups_conf.yaml')
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_assetguard_db_backups_conf.yaml')
...
backups_path = Path(ASSETGUARD_PATH, 'backup', 'db')
...
def test_wdb_backup_configs(test_configuration, test_metadata, set_assetguard_configuration,
    ...
    input_description:
        - Test cases are defined in the parameters and metada variables, that will be applied to the the
          assetguard_db_backup_command.yaml file. The parameters tested are: "enabled", "interval" and "max_files".
    ...
    assetguard_log_monitor = file_monitor.FileMonitor(ASSETGUARD_LOG_PATH)
    try:
        control_service('restart')
    except (subprocess.CalledProcessError, ValueError) as err:
        if not validate_interval_format(test_interval):
            assetguard_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_INTERVAL_CALLBACK), timeout=timeout)
            assert assetguard_log_monitor.callback_result, 'Did not receive expected ' \
                                                    '"Invalid value element for interval..." event'

            return
        elif not isinstance(test_max_files, numbers.Number) or test_max_files==0:
            assetguard_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_MAX_FILES_CALLBACK), timeout=timeout)
            assert assetguard_log_monitor.callback_result, 'Did not receive expected ' \
    ...
    # Manage if backup generation is enabled - one or more backups expected
        ...
        assetguard_log_monitor.start(callback=callbacks.generate_callback(patterns.BACKUP_CREATION_CALLBACK),
                                        timeout=timeout, accumulations=int(test_max_files)+1)
        result = assetguard_log_monitor.callback_result
