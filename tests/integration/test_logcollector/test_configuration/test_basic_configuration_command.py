...
brief: The 'assetguard-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the AssetGuard API returns the same values for
...
references:
    ...
    - https://documentation.assetguard.com/current/user-manual/reference/assetguard-conf/localfile.html#command
...
config_path = Path(CONFIGURATIONS_PATH, 'assetguard_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'assetguard-logcollector' daemon can monitor commands that use multiple parameters.
    ...
    assetguard_log_monitor.start(callback=callback)
