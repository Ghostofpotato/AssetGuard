...
brief: The 'assetguard-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the logcollector detects invalid values for
       the 'log_format' tag and the AssetGuard API returns the same values for the configured
...
from assetguard_testing.constants.paths.logs import ASSETGUARD_LOG_PATH, MACOS_LOG_COMMAND_PATH
...
from assetguard_testing.modules.logcollector import configuration as logcollector_configuration
from assetguard_testing.modules.logcollector import patterns, PREFIX
...
default_config_path = Path(CONFIGURATIONS_PATH, 'assetguard_basic_configuration_log_format.yaml')
default_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format.yaml')

macos_duplicated_config_path = Path(CONFIGURATIONS_PATH, 'assetguard_duplicated_macos_configuration.yaml')
macos_duplicated_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_duplicated.yaml')

macos_no_defined_config_path = Path(CONFIGURATIONS_PATH, 'assetguard_no_defined_location_macos_configuration.yaml')
macos_no_defined_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_no_defined.yaml')

win_config_path = Path(CONFIGURATIONS_PATH, 'assetguard_basic_configuration_log_format_location.yaml')
...
def check_log_format_valid(test_configuration, test_metadata):
    """Check if AssetGuard run correctly with the specified log formats.
    ...
    if test_metadata['log_format'] not in log_format_not_print_analyzing_info:
        assetguard_log_monitor.start(timeout=15,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE,
                                                   {'file': test_metadata['location']}))
        assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_FILE
        ...
        if test_metadata['log_format'] == 'full_command':
            ...
            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MONITORING_COMMAND, {
                                'command': test_metadata['command']})
        assetguard_log_monitor.start(timeout=15, callback=callback)
        assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_COMMAND_MONITORING
        ...
        assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_DJB_MULTILOG_NOT_PRODUCED
        ...
        if 'location' in test_metadata and test_metadata['location'] != 'macos':
            ...
            assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_INVALID_MACOS_VALUE
        if 'location' not in test_metadata:
            assetguard_log_monitor.start(timeout=15,
                                    callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MISSING_LOCATION))
            assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_MISSING_LOCATION_VALUE

        assetguard_log_monitor.start(timeout=15,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                   {'command_path': MACOS_LOG_COMMAND_PATH}))
        assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_MACOS_LOG_NOT_PRODUCED


def check_log_format_invalid(test_metadata):
    """Check if AssetGuard fails because a invalid frequency configuration value.
...
def check_log_file_duplicated():
    """Check if AssetGuard shows a warning message when the configuration is duplicated."""
    assetguard_log_monitor = file_monitor.FileMonitor(ASSETGUARD_LOG_PATH)
    assetguard_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_LOG_FILE_DUPLICATED))
    assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_LOG_FILE_DUPLICATED
    assetguard_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                                 {'command_path': MACOS_LOG_COMMAND_PATH}))
    assert (assetguard_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_MACOS
...
def test_log_format(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                    set_assetguard_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'assetguard-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the AssetGuard API returns the same values for the 'localfile' section
    ...
    assertions:
        ...
        - Verify that the AssetGuard API returns the same values for the 'localfile' section as the configured one.


    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (assetguard_basic_configuration.yaml, assetguard_duplicated_macos_configuration.yaml, and
                       assetguard_no_defined_location_macos_configuration.yaml). Those templates are combined with
                       different test cases defined in the module. Those include configuration settings for
                       the 'assetguard-logcollector' daemon.
    ...
    if test_metadata['valid_value']:
        ...
        if sys.platform == WINDOWS:
            pytest.xfail("Windows agent allows invalid localfile configuration:\
                          https://github.com/assetguard/assetguard/issues/10890")
