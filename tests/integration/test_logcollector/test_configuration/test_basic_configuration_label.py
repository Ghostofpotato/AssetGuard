...
brief: The 'assetguard-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the AssetGuard API returns the same values for
...
references:
    ...
    - https://documentation.assetguard.com/current/user-manual/reference/assetguard-conf/localfile.html#label
...
config_path = Path(CONFIGURATIONS_PATH, 'assetguard_basic_configuration_label.yaml')
...
def test_configuration_label(test_configuration, test_metadata, set_assetguard_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'assetguard-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the AssetGuard API returns the same values for
    ...
    assetguard_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE, {
