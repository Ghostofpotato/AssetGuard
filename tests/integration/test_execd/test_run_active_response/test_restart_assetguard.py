...
brief: Active responses execute a script in response to the triggering of specific alerts based
       ...
       which are executed by the 'assetguard-execd' daemon via scripts, run correctly.
...
references:
    - https://documentation.assetguard.com/current/user-manual/capabilities/active-response/#active-response
'''
import sys
import pytest
...
from assetguard_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, ASSETGUARD_LOG_PATH
from assetguard_testing.constants.platforms import WINDOWS
from assetguard_testing.modules.execd.active_response.patterns import ACTIVE_RESPONSE_RESTART_ASSETGUARD
from assetguard_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from assetguard_testing.modules.execd.configuration import EXECD_DEBUG
from assetguard_testing.modules.execd.patterns import EXECD_SHUTDOWN_RECEIVED
...
ar_conf = 'restart-assetguard0 - restart-assetguard - 0\nrestart-assetguard0 - restart-assetguard.exe - 0'
...
def test_execd_restart(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                       set_assetguard_configuration, configure_ar_conf, remoted_simulator, authd_simulator,
                       daemons_handler, send_execd_message):
    '''
    description: Check if 'restart-assetguard' command of 'active response' is executed correctly.
                 For this purpose, a simulated server is used, from which the active response is sent.
                 This response includes the order to restart the AssetGuard agent, which must restart after
    ...
    assertions:
        ...
        - Check the restart-assetguard program is used.
    input_description:
    ...
    if error_message := test_metadata.get('expected_error'):
        ...
        assert ar_monitor.callback_result, 'AR `assetguard-restart` did not fail.'
        return

    assetguard_log_monitor.start(callback=generate_callback(EXECD_SHUTDOWN_RECEIVED))
    assert assetguard_log_monitor.callback_result, 'Execd `shutdown` log not raised.'

    ar_monitor.start(callback=generate_callback(ACTIVE_RESPONSE_RESTART_ASSETGUARD))
    assert ar_monitor.callback_result, 'AR `restart-assetguard` program not used.'
