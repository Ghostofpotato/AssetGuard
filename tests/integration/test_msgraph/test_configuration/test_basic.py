...
def test_enabled(test_configuration, test_metadata, set_assetguard_configuration, configure_local_internal_options,
    ...
    expected_output:
        - r'.*assetguard-(?:manager-)?modulesd:ms-graph.*INFO: Started module'
    '''

    assetguard_log_monitor = FileMonitor(ASSETGUARD_LOG_PATH)

    assetguard_log_monitor.start(callback=callbacks.generate_callback(r".*assetguard-(?:manager-)?modulesd:ms-graph.*INFO: Started module"))
    assert (assetguard_log_monitor.callback_result != None), f'Error module enabled event not detected'

    assetguard_log_monitor.start(callback=callbacks.generate_callback(r".*assetguard-(?:manager-)?modulesd:ms-graph.*{msg}", {
                              'msg': str(test_metadata['msg'])}))
    assert (assetguard_log_monitor.callback_result != None), f'Error module started or delayed event not detected'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t2_configurations, t2_configuration_metadata), ids=t2_case_ids)
def test_disabled(test_configuration, test_metadata, set_assetguard_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start):
    '''
    ...
    expected_output:
        - r'.*assetguard-(?:manager-)?modulesd:ms-graph.*INFO: (Module disabled). Exiting.'
    '''

    assetguard_log_monitor = FileMonitor(ASSETGUARD_LOG_PATH)
    assetguard_log_monitor.start(callback=callbacks.generate_callback(r".*assetguard-(?:manager-)?modulesd:ms-graph.*INFO: (Module disabled). Exiting."))

    assert (assetguard_log_monitor.callback_result != None), f'Error module disabled event not detected'
