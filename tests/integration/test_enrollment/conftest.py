...
from assetguard_testing.constants.paths.configurations import ASSETGUARD_CLIENT_KEYS_PATH, DEFAULT_AUTHD_PASS_PATH
from assetguard_testing.tools.simulators.authd_simulator import AuthdSimulator
from assetguard_testing.utils.file import write_file, remove_file
...
def shutdown_agentd(request):
    """Stop assetguard-agentd process."""
...
def set_keys(test_metadata):
    ...
    for key in test_metadata.get('pre_existent_keys', []):
        write_file(ASSETGUARD_CLIENT_KEYS_PATH, key)
