...
def test_limit_eps(test_configuration, test_metadata, set_assetguard_configuration, add_configuration,
    ...
    assetguard_config = " ".join(get_assetguard_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in assetguard_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in assetguard_config
