...
def test_accepted_values(test_configuration, test_metadata, load_assetguard_basic_configuration, set_assetguard_configuration,
    ...
    """
    # Start monitor
    monitor_enabled = file_monitor.FileMonitor(ASSETGUARD_LOG_PATH)
...
def test_invalid_values(test_configuration, test_metadata, load_assetguard_basic_configuration, set_assetguard_configuration,
                        truncate_monitored_files, daemons_handler):
    """
    description: Check for configuration error and assetguard-manager-analysisd if the EPS limiting configuration has unaccepted
    ...
    test_phases:
        ...
        - test:
            - Restart assetguard-manager service to apply configuration changes.
            - Check that a configuration error is raised when trying to start assetguard-manager.
            - Check that assetguard-manager-analysisd is not running (due to configuration error).
        - teardown:
            - Truncate assetguard logs.
            - Restore initial configuration.
    ...
    """
    # Start monitor
    monitor_error = file_monitor.FileMonitor(ASSETGUARD_LOG_PATH)
...
def test_missing_configuration(test_configuration, test_metadata, load_assetguard_basic_configuration, set_assetguard_configuration,
    ...
    test_phases:
        ...
        - test:
            ...
            - Check whether the EPS limitation is activated, deactivated or generates a configuration error due to a
              missing label.
            - Check if assetguard-manager-analysisd is running or not (according to the expected behavior).
        - teardown:
            - Truncate assetguard logs.
            - Restore initial configuration.
