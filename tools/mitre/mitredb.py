...
def main(database=None):
    ...
    :param database: Directory where mitre.db is. Default: /var/assetguard-manager/var/db/mitre.db
    :return:
    """
    if database is None:
        database = "/var/assetguard-manager/var/db/mitre.db"
    else:
    ...
    gid = grp.getgrnam("assetguard-manager").gr_gid
    os.chown(database, uid, gid)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script installs mitre.db in a directory.')
    parser.add_argument('--database', '-d', help='-d /your/directory/mitre.db (default: /var/assetguard-manager/var/db/mitre.db')
