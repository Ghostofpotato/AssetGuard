...
@patch('assetguard_integration.utils.find_assetguard_path', return_value=utils.TEST_ASSETGUARD_PATH)
@patch('assetguard_integration.utils.get_assetguard_version')
def test_assetguard_integration_initializes_properly(mock_version, mock_path, mock_client):
    """Test if the instances of AssetGuardIntegration are created properly."""

    args = utils.get_assetguard_integration_parameters()
    integration = assetguard_integration.AssetGuardIntegration(**args)
    mock_path.assert_called_once()
    mock_version.assert_called_once()
    assert integration.assetguard_path == utils.TEST_ASSETGUARD_PATH
    assert integration.assetguard_queue == os.path.join(integration.assetguard_path, utils.QUEUE_PATH)
    assert integration.assetguard_wodle == os.path.join(integration.assetguard_path, utils.WODLE_PATH)
...
@patch('assetguard_integration.config')
...
def test_default_config(mock_boto_config, file_exists, options, retry_attempts, retry_mode):
    """Test if `default_config` function returns the AssetGuard default Retry configuration if there is no user-defined
    configuration.
    ...
    with patch('assetguard_integration.path.exists', return_value=file_exists):
        if file_exists:
            with patch('aws_tools.get_aws_config_params') as mock_get_config, \
                ...
                config = assetguard_integration.AssetGuardIntegration.default_config(profile=utils.TEST_AWS_PROFILE)
            ...
            if aws_tools.RETRY_ATTEMPTS_KEY in profile_config or aws_tools.RETRY_MODE_CONFIG_KEY in profile_config:
                ...
                retries = assetguard_integration.ASSETGUARD_DEFAULT_RETRY_CONFIGURATION

            assert config['config'].retries == retries
        else:
            config = assetguard_integration.AssetGuardIntegration.default_config(profile=utils.TEST_AWS_PROFILE)

            mock_boto_config.Config.assert_called_with(retries=assetguard_integration.ASSETGUARD_DEFAULT_RETRY_CONFIGURATION)
...
@pytest.mark.parametrize('region', list(assetguard_integration.DEFAULT_GOV_REGIONS) + ['us-east-1', None])
@pytest.mark.parametrize('service_name', list(assetguard_integration.SERVICES_REQUIRING_REGION) + ['other'])
def test_assetguard_integration_get_client_authentication(access_key, secret_key, profile, region, service_name):
    ...
    service_name : str
        Name of the service.
    """
    kwargs = utils.get_assetguard_integration_parameters(access_key=access_key, secret_key=secret_key, profile=profile,
    ...
    if region and service_name in assetguard_integration.SERVICES_REQUIRING_REGION:
        expected_conn_args['region_name'] = region
    else:
        expected_conn_args['region_name'] = region if region in assetguard_integration.DEFAULT_GOV_REGIONS else None

    with patch('assetguard_integration.utils.find_assetguard_path', return_value=utils.TEST_ASSETGUARD_PATH), \
            patch('assetguard_integration.utils.get_assetguard_version', return_value=utils.ASSETGUARD_VERSION), \
            patch('assetguard_integration.boto3.Session') as mock_boto:
        assetguard_integration.AssetGuardIntegration(**kwargs)
...
def test_assetguard_integration_get_client(iam_role_arn, service_name, external_id):
    ...
    """
    kwargs = utils.get_assetguard_integration_parameters(access_key=None, secret_key=None, profile=None,
    ...
    sts_kwargs = {'aws_access_key_id': None, 'aws_secret_access_key': None, 'aws_session_token': utils.TEST_TOKEN,
                  'region_name': None}
    assume_role_kwargs = {'RoleArn': iam_role_arn, 'RoleSessionName': 'AssetGuardLogParsing',
    ...
    with patch('assetguard_integration.utils.find_assetguard_path', return_value=utils.TEST_ASSETGUARD_PATH), \
            patch('assetguard_integration.utils.get_assetguard_version', return_value=utils.ASSETGUARD_VERSION), \
            patch('assetguard_integration.boto3.Session', side_effect=[mock_boto_session, mock_sts_session]) as mock_session:
        instance = assetguard_integration.AssetGuardIntegration(**kwargs)
...
def test_assetguard_integration_get_client_handles_exceptions_on_botocore_error():
    """Test `get_client` function handles exceptions as expected."""
    mock_boto_session = MagicMock()
    mock_boto_session.client.side_effect = assetguard_integration.exceptions.ClientError({'Error': {'Code': 1}},
                                                                                             'operation')

    with patch('assetguard_integration.utils.find_assetguard_path', return_value=utils.TEST_ASSETGUARD_PATH), \
            patch('assetguard_integration.utils.get_assetguard_version', return_value=utils.ASSETGUARD_VERSION), \
            patch('assetguard_integration.boto3.Session', return_value=mock_boto_session):
        with pytest.raises(SystemExit) as e:
            assetguard_integration.AssetGuardIntegration(**utils.get_assetguard_integration_parameters())
...
def test_assetguard_integration_get_sts_client(access_key, secret_key, profile):
    ...
    profile : str
        AWS profile name.
    """
    instance = utils.get_mocked_assetguard_integration(access_key=access_key, secret_key=secret_key, profile=profile)
    expected_conn_args = {}
    if access_key and secret_key:
        expected_conn_args['aws_access_key_id'] = access_key
        expected_conn_args['aws_secret_access_key'] = secret_key
    ...
    with patch('assetguard_integration.boto3.Session', return_value=mock_session) as mock_boto:
...
def test_assetguard_integration_get_sts_client_handles_exceptions_when_invalid_creds_provided():
    """Test `get_sts_client` function handles invalid credentials exception as expected."""
    mock_boto_session = MagicMock()
    mock_boto_session.client.side_effect = assetguard_integration.exceptions.ClientError({'Error': {'Code': 1}},
                                                                                             'operation')

    instance = utils.get_mocked_assetguard_integration(access_key=None, secret_key=None, profile=None)

    with patch('assetguard_integration.boto3.Session', return_value=mock_boto_session):
        with pytest.raises(SystemExit) as e:
...
def test_assetguard_integration_send_msg(dump_json):
    ...
    """
    instance = utils.get_mocked_assetguard_integration()
    msg = dumps(utils.TEST_MESSAGE) if dump_json else utils.TEST_MESSAGE
    with patch('assetguard_integration.socket.socket') as mock_socket:
        m = MagicMock()
        mock_socket.return_value = m
        instance.send_msg(utils.TEST_MESSAGE, dump_json=dump_json)
        mock_socket.assert_called_once()
        m.send.assert_called_with(f"{assetguard_integration.MESSAGE_HEADER}{msg}".encode())
...
def test_assetguard_integration_send_msg_socket_error(error_code, expected_exit_code):
...
@patch('io.BytesIO')
def test_assetguard_integration_decompress_file(mock_io):
    """Test 'decompress_file' method calls the expected function for a determined file type."""
    integration = utils.get_mocked_assetguard_integration()
    integration.client = MagicMock()
    # Instance that inherits from AssetGuardIntegration sets the attribute bucket in its constructor
...
@patch('io.BytesIO')
def test_aws_assetguard_integration_decompress_file_handles_exceptions_when_decompress_fails(mock_io):
...
def test_assetguard_integration_send_msg_handles_exceptions():
...
@patch('assetguard_integration.utils.get_assetguard_version')
@patch('assetguard_integration.AssetGuardIntegration.get_client')
@patch('assetguard_integration.AssetGuardAWSDatabase.check_metadata_version')
@patch('assetguard_integration.sqlite3.connect')
def test_assetguard_aws_database_initializes_properly(mock_connect, mock_metadata, mock_client, mock_version, mock_path):
    """Test if the instances of AssetGuardAWSDatabase are created properly."""
    mock_connect.return_value = MagicMock()
    args = utils.get_assetguard_aws_database_parameters()
    assetguard_aws_db = assetguard_integration.AssetGuardAWSDatabase(**args)

    assert assetguard_aws_db.db_path == os.path.join(assetguard_aws_db.assetguard_wodle, f"{utils.TEST_DATABASE}.db")
    mock_connect.assert_called_once()
    assetguard_aws_db.db_connector.cursor.assert_called_once()
    mock_metadata.assert_called_once()


def test_assetguard_aws_database_create_table():
    """Test `create_table` function creates the table using the expected SQL."""
    instance = utils.get_mocked_assetguard_aws_database()
...
def test_assetguard_aws_database_create_table_handles_exceptions_when_table_not_created():
...
@patch('assetguard_integration.AssetGuardAWSDatabase.create_table')
def test_assetguard_aws_database_db_initialization(mock_create_table, table_list):
...
def test_assetguard_aws_database_db_initialization_handles_exceptions():
...
def test_assetguard_aws_database_close_db():
...
def test_assetguard_aws_database_check_metadata_version_existing_table(custom_database):
    ...
    instance = utils.get_mocked_assetguard_aws_database(db_name=utils.TEST_DATABASE)
    ...
    assert old_metadata_value != utils.ASSETGUARD_VERSION
    ...
    assert new_metadata_value == utils.ASSETGUARD_VERSION


def test_assetguard_aws_database_check_metadata_version_no_table(custom_database):
...
def test_assetguard_aws_database_check_metadata_version_handles_exceptions(custom_database, table_exists):
...
def test_assetguard_aws_database_delete_deprecated_tables(custom_database):
    ...
    for table in assetguard_integration.DEPRECATED_TABLES:
