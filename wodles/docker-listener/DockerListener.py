...
class DockerListener:
    ...
    def __init__(self):
        ...
        if sys.platform == "win32":
            ...
            sys.exit(1)
        # socket variables
        self.assetguard_path = os.path.abspath(os.path.join(__file__, "..", "..", ".."))
        self.assetguard_queue = os.path.join(self.assetguard_path, "queue", "sockets", "queue")
        self.msg_header = "1:AssetGuard-Docker:"
        # docker variables
        self.client = None
        self.thread1 = None
        self.thread2 = None
    ...
    def send_msg(self, msg):
        """
        Sends a Docker event to the AssetGuard Queue
        ...
        try:
            ...
            if e.errno == 111:
                sys.stderr.write('AssetGuard must be running.\n')
                sys.exit(11)
            else:
                sys.stderr.write("Error sending message to assetguard: {}\n".format(e))
                sys.exit(13)
        except Exception as e:
            sys.stderr.write("Error sending message to assetguard: {}\n".format(e))
            sys.exit(13)
