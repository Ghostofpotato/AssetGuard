...
# assetguard-manager-control        This shell script takes care of starting
...
ASSETGUARD_CONF="${ASSETGUARD_CONF:-assetguard-manager.conf}"
...
REVISION="alpha0"
TYPE="manager"
ASSETGUARD_ENGINE_GROUP="${ASSETGUARD_ENGINE_GROUP:-assetguard-manager}"
export ASSETGUARD_ENGINE_GROUP
...
USE_JSON=false
DAEMONS="assetguard-manager-clusterd assetguard-manager-modulesd assetguard-manager-monitord assetguard-manager-remoted assetguard-manager-analysisd assetguard-manager-db assetguard-manager-authd assetguard-manager-apid"
...
add_to_cgroup()
{
    CGROUP_PATH="/sys/fs/cgroup/systemd/system.slice/assetguard-manager.service/cgroup.procs"

    # Check if cgroup path exists
    if [ ! -f "$CGROUP_PATH" ]; then
        echo "Warning: cgroup path does not exist: $CGROUP_PATH" >&2
    else
        for pidfile in ${DIR}/var/run/assetguard-manager-*.pid; do
            [ -f "$pidfile" ] || continue
...
get_assetguard_engine_pid()
{
    local max_ticks=100
    local ticks=0
    local pidfile

    ${DIR}/bin/assetguard-manager-analysisd

    while [ $ticks -lt $max_ticks ]; do
        pidfile=$(ls ${DIR}/var/run/assetguard-manager-analysisd-*.pid 2>/dev/null | head -n1)
        if [ -n "$pidfile" ]; then
...
wait_for_assetguard_engine_ready()
{
    local attempts=0
    local max_attempts=240 # TODO Improve this value

    ENGINE_PID=$(get_assetguard_engine_pid)
    if [ $? -ne 0 ]; then
        echo "Failed to obtain PID for assetguard-manager-analysisd"
        return 1
    fi
    ...
    while [ $attempts -lt $max_attempts ]; do
        ...
        if ! kill -0 "$ENGINE_PID" 2>/dev/null; then
            echo "assetguard-manager-analysisd died during route check."
            return 1
        fi
    ...
    echo "assetguard-manager-analysisd did not respond correctly after $max_attempts attempts."
    kill $ENGINE_PID
    return 1
}
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting AssetGuard $VERSION..."
    fi
    ...
    node_type=$(grep '<node_type>' ${DIR}/etc/${ASSETGUARD_CONF} | sed 's/<node_type>\(.*\)<\/node_type>/\1/' | tr -d ' ');
    if [ -z $node_type ]; then
        echo "Invalid cluster configuration, check the $DIR/etc/${ASSETGUARD_CONF} file."
        unlock;
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ...
        if [ X"$i" = "Xassetguard-manager-apid" ] && [ "$node_type" != "master" ]; then
            continue
        fi

        ## If assetguard-manager-authd is disabled, don't try to start it.
        if [ X"$i" = "Xassetguard-manager-authd" ]; then
             start_config="$(grep -n "<auth>" ${DIR}/etc/${ASSETGUARD_CONF} | cut -d':' -f 1)"
             end_config="$(grep -n "</auth>" ${DIR}/etc/${ASSETGUARD_CONF} | cut -d':' -f 1)"
             if [ -n "${start_config}" ] && [ -n "${end_config}" ]; then
                sed -n "${start_config},${end_config}p" ${DIR}/etc/${ASSETGUARD_CONF} | grep "<disabled>yes" >/dev/null 2>&1
                if [ $? = 0 ]; then
                    continue
                fi
             else
        ...
        if [ $? = 0 ]; then
            ...
            if [ ! -z "$LEGACY_SYSTEMD_VERSION" ]; then
                ...
                if [ "$i" = "assetguard-manager-analysisd" ]; then
                    wait_for_assetguard_engine_ready
        ...
        for pid in `cat ${DIR}/var/run/${daemon_name}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ] && [ "X${_pstatus_quiet}" = "X" ]; then
                    echo "${pfile}: Process ${pid} not used by AssetGuard, removing..."
                fi
...
stop_service()
{
    ...
    if [ $USE_JSON = true ]; then
        ...
        echo "AssetGuard $VERSION Stopped"
    fi
}
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"ASSETGUARD_VERSION":"'${VERSION}'"},'
            echo -n '{"ASSETGUARD_REVISION":"'${REVISION}'"},'
            echo -n '{"ASSETGUARD_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "ASSETGUARD_VERSION=\"${VERSION}\""
            echo "ASSETGUARD_REVISION=\"${REVISION}\""
            echo "ASSETGUARD_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
