...
# Install functions for AssetGuard
# AssetGuard.com (https://github.com/assetguard)

InstallSELinuxPolicyPackage(){

    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/assetguard.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/assetguard.pp /tmp && semodule -i /tmp/assetguard.pp
                rm -f /tmp/assetguard.pp
...
CheckModuleIsEnabled(){
    ...
    local conf_path="$PREINSTALLEDDIR/etc/${ASSETGUARD_CONF:-ossec.conf}"
...
AssetGuardUpgrade()
{
    ...
    # Remove existing SQLite databases for AssetGuard DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/assetguard
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated AssetGuard tools
    ...
    rm -f $PREINSTALLEDDIR/bin/assetguard-logtest
    rm -f $PREINSTALLEDDIR/bin/assetguard-logtest-legacy
    rm -f $PREINSTALLEDDIR/bin/assetguard-regex

    # Remove old AssetGuard daemons
    ...
    rm -f $PREINSTALLEDDIR/bin/assetguard-agentlessd
    rm -f $PREINSTALLEDDIR/bin/assetguard-csyslogd
    rm -f $PREINSTALLEDDIR/bin/assetguard-dbd
    rm -f $PREINSTALLEDDIR/bin/assetguard-integratord
    rm -f $PREINSTALLEDDIR/bin/assetguard-maild
    rm -f $PREINSTALLEDDIR/bin/assetguard-reportd
