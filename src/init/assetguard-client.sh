...
# assetguard-control        This shell script takes care of starting
...
DAEMONS="assetguard-modulesd assetguard-logcollector assetguard-syscheckd assetguard-agentd assetguard-execd"
...
check_folders()
{
    ...
    if [ ! -d $ALERTS_FOLDER ]
    then
        if rm -rf $ALERTS_FOLDER && mkdir -p $ALERTS_FOLDER && chown assetguard:assetguard $ALERTS_FOLDER && chmod 770 $ALERTS_FOLDER
        then
...
add_to_cgroup()
{
    AGENTD_PID=$(head -n 1 ${DIR}/var/run/assetguard-agentd-*.pid 2>/dev/null)
    CGROUP_PATH="/sys/fs/cgroup/systemd/system.slice/assetguard-agent.service/cgroup.procs"

    # Check if cgroup path exists
    if [ ! -f "$CGROUP_PATH" ]; then
        echo "Warning: cgroup path does not exist: $CGROUP_PATH" >&2
    else
        for pidfile in ${DIR}/var/run/assetguard-*.pid; do
            [ -f "$pidfile" ] || continue
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/assetguard-agentd//')
    if is_systemd; then
    ...
    pid=`cat ${DIR}/var/run/assetguard-agentd-*.pid`
    kill -USR1 $pid
    ;;
status)
    lock
