...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ...
        StageT(R"([{"assetguard-indexer":{"index":"assetguard-events-v5-${assetguard.integration.category}"}}])",
               outputsBuilder,
               SUCCESS(
                   [](const auto& mocks)
                   {
                       const auto& innerRegistry = mocks.registry->template getRegistry<StageBuilder>();
                       EXPECT_CALL(*mocks.ctx, registry()).WillOnce(testing::ReturnRef(*mocks.registry));
                       EXPECT_CALL(innerRegistry, get("assetguard-indexer")).WillOnce(testing::Return(dummyStageBuilder()));
                       return base::Broadcast::create("outputs",
