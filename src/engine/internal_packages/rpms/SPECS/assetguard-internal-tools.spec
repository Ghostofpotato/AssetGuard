Name:           assetguard-internal-tools
Version:        0.1.0
Release:        1
Summary:        AssetGuard Internal Engine Tools (runtime venv + offline wheels)
...
# Installation paths
# ------------------------------
%global prefix  /opt/assetguard-internal-tools
%global venv    %{prefix}/venv
%global wheelsd %{prefix}/wheels

%description
AssetGuard internal CLI utilities.  
...
#!/bin/bash
exec /opt/assetguard-internal-tools/venv/bin/python -m ${mod} "\$@"
EOF
...
if [ "${1:-0}" -eq 0 ]; then
  ...
  # Safety guard
  case "$PREFIX" in
    /opt/assetguard-internal-tools|/opt/assetguard-internal-tools/*) ;;
...
%changelog
* Thu Oct 16 2025 AssetGuard Team <dev@assetguard.com> - 0.1.0-1
