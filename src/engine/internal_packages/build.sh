...
SRC_ROOT="${SRC_ROOT:-/assetguard-local-src}"      # mounted repo path
OUTDIR="${OUTDIR:-/var/local/assetguard}"          # artifacts path (bind-mounted)
...
# Working directories
WORK="/tmp/pkgbuild/assetguard-internal-tools"
rm -rf "${WORK}"
mkdir -p "${WORK}"
mkdir -p "${OUTDIR}"
