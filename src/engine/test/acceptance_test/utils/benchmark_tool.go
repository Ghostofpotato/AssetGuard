...
func main() {
	...
	flag.StringVar(&watchedFile, "o", "/var/assetguard-manager/logs/alerts/alerts.json", "Watched file. The Output file")
	flag.BoolVar(&truncateWatched, "T", false, "Truncate the output file")
	// Protocol
	flag.StringVar(&sockPath, "s", "/var/assetguard-manager/queue/sockets/queue", "Path/Adress to the sockets")
