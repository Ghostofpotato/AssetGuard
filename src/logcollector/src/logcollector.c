...
#if defined(Darwin) || (defined(__linux__) && defined(ASSETGUARD_UNIT_TESTING))
            ...
            for (i = 0, j = -1;; i++) {
                ...
                if (current->fp) {
...
#endif
                    {
                        ...
                        snprintf(msg_alert, 512, "assetguard: File rotated (inode "
                                 "changed): '%s'.",
                                 current->file);
...
#endif
                    {
                        ...
                        snprintf(msg_alert, 512, "assetguard: File size reduced "
                                 "(inode remained): '%s'.",
                                 current->file);
...
#ifndef WIN32
    if (strcmp("assetguardalert", current->logformat) == 0) {
        current->read = read_assetguardalert;
    }
#endif
...
void * w_output_thread(void * args){
    ...
    while(1)
    {
        ...
        if (strcmp(message->log_target->log_socket->name, "agent") == 0) {
            ...
            if (result != 0) {
                if (result != 1) {
                    mdebug1("Unable to send message to '%s' (assetguard-agentd might be down). Attempting to reconnect.", DEFAULTQUEUE);
                }
