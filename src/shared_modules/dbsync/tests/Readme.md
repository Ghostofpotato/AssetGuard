# Unit Tests
## Index
1. [Compile AssetGuard](#compile-assetguard)
...
Navigate into `assetguard/src/dbsync/` and run the following commands:
```
mkdir build
cd build
cmake -DEXTERNAL_LIB=~/path/to/assetguard/src/external/ -DCMAKE_BUILD_TYPE=Debug -DUNIT_TEST=ON ..
cmake --build .
```
