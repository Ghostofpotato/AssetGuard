#!/usr/bin/env python
 ...
 ###
 # Integration of AssetGuard agent with Kaspersky endpoint security for Linux
...
# Sets the file paths
##################################################################################################################
assetguard_path = os.path.abspath(os.path.join(__file__, "../../.."))
assetguard_queue = '{0}/queue/sockets/queue'.format(assetguard_path)
now = time.strftime("%a %b %d %H:%M:%S %Z %Y")
ar_log_file = '{0}/logs/active-responses.log'.format(assetguard_path)
...
def send_msg(assetguard_queue, msg):
	...
	try:
		s.connect(assetguard_queue)
	except:
		print('Error: AssetGuard must be running.')
		sys.exit(1)
...
def main():
    ar_log()
    set_logger('assetguard-kaspersky', foreground=args.verbose)
    run_kaspersky()
