...
int config_dir(char *name, char *dir, char *vfile)
{
    ...
    if (dogrep(ASSETGUARDCONF, vfile)) {
    ...
    printf("%s: You may have it configured in a format different\n"
           ...
           "               log available.\n", name);
    printf("%s: https://documentation.assetguard.com\n\n", name);

    /* Add IIS config */
    fp = wfopen(ASSETGUARDCONF, "a");
    if (!fp) {
...
int main(int argc, char **argv)
{
    ...
    if (!fileexist(ASSETGUARDCONF)) {
        printf("%s: Unable to find assetguard config: '%s'", argv[0], ASSETGUARDCONF);
        exit(0);
    }
    ...
    printf("%s: Looking for IIS log files to monitor.\r\n",
           argv[0]);
    printf("%s: For more information: https://documentation.assetguard.com\r\n",
           argv[0]);
    printf("\r\n");
