...
' Copyright (C) 2015, AssetGuard Inc. <support@assetguard.com>
'
...
public function config()
    ...
    ASSETGUARD_MANAGER = Replace(args(2), Chr(34), "")
    ASSETGUARD_MANAGER_PORT = Replace(args(3), Chr(34), "")
    NOTIFY_TIME = Replace(args(4), Chr(34), "")
    ASSETGUARD_REGISTRATION_SERVER = Replace(args(5), Chr(34), "")
    ASSETGUARD_REGISTRATION_PORT = Replace(args(6), Chr(34), "")
    ASSETGUARD_REGISTRATION_PASSWORD = Replace(args(7), Chr(34), "")
    ASSETGUARD_KEEP_ALIVE_INTERVAL = Replace(args(8), Chr(34), "")
    ASSETGUARD_TIME_RECONNECT = Replace(args(9), Chr(34), "")
    ASSETGUARD_REGISTRATION_CA = Replace(args(10), Chr(34), "")
    ASSETGUARD_REGISTRATION_CERTIFICATE = Replace(args(11), Chr(34), "")
    ASSETGUARD_REGISTRATION_KEY = Replace(args(12), Chr(34), "")
    ASSETGUARD_AGENT_NAME = Replace(args(13), Chr(34), "")
    ASSETGUARD_AGENT_GROUP = Replace(args(14), Chr(34), "")
    ...
    If objFSO.fileExists(home_dir & "ossec.conf") Then
        ...
        If ASSETGUARD_MANAGER <> "" or ASSETGUARD_MANAGER_PORT <> "" or ASSETGUARD_KEEP_ALIVE_INTERVAL <> "" or ASSETGUARD_TIME_RECONNECT <> "" Then
            If ASSETGUARD_MANAGER <> "" Then
                Set re = new regexp
                re.Pattern = "\s+<(server|manager)>(.|\n)+?</\1>"
                If InStr(ASSETGUARD_MANAGER,",") Then
                    ip_list=Split(ASSETGUARD_MANAGER,",")
                Else
                    ip_list=Array(ASSETGUARD_MANAGER)
                End If
            ...
            If ASSETGUARD_MANAGER_PORT <> "" Then ' manager server_port
                If InStr(strText, "<port>") > 0 Then
                    strText = Replace(strText, "<port>1514</port>", "<port>" & ASSETGUARD_MANAGER_PORT & "</port>")
                End If

            End If

            If ASSETGUARD_KEEP_ALIVE_INTERVAL <> "" Then
                If InStr(strText, "<notify_time>") > 0 Then
                    Set re = new regexp
                    re.Pattern = "<notify_time>.*</notify_time>"
                    re.Global = True
                    strText = re.Replace(strText, "<notify_time>" & ASSETGUARD_KEEP_ALIVE_INTERVAL & "</notify_time>")
                End If
            End If

            If ASSETGUARD_TIME_RECONNECT <> "" Then 'TODO fix the - and use _
                If InStr(strText, "<time-reconnect>") > 0 Then
                    Set re = new regexp
                    re.Pattern = "<time-reconnect>.*</time-reconnect>"
                    re.Global = True
                    strText = re.Replace(strText, "<time-reconnect>" & ASSETGUARD_TIME_RECONNECT & "</time-reconnect>")
                End If
            End If
        End If

        If ASSETGUARD_REGISTRATION_SERVER <> "" or ASSETGUARD_REGISTRATION_PORT <> "" or ASSETGUARD_REGISTRATION_PASSWORD <> "" or ASSETGUARD_REGISTRATION_CA <> "" or ASSETGUARD_REGISTRATION_CERTIFICATE <> "" or ASSETGUARD_REGISTRATION_KEY <> "" or ASSETGUARD_AGENT_NAME <> "" or ASSETGUARD_AGENT_GROUP <> "" or ENROLLMENT_DELAY <> "" or ASSETGUARD_MANAGER <> "" Then
            ...
            If ASSETGUARD_REGISTRATION_SERVER <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <manager_address>" & ASSETGUARD_REGISTRATION_SERVER & "</manager_address>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_REGISTRATION_PORT <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <port>" & ASSETGUARD_REGISTRATION_PORT & "</port>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_REGISTRATION_PASSWORD <> "" Then
                Set objFile = objFSO.CreateTextFile(home_dir & "authd.pass", ForWriting)
                objFile.WriteLine ASSETGUARD_REGISTRATION_PASSWORD
                objFile.Close
                strText = Replace(strText, "    </enrollment>", "      <authorization_pass_path>authd.pass</authorization_pass_path>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_REGISTRATION_CA <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <server_ca_path>" & ASSETGUARD_REGISTRATION_CA & "</server_ca_path>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_REGISTRATION_CERTIFICATE <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_certificate_path>" & ASSETGUARD_REGISTRATION_CERTIFICATE & "</agent_certificate_path>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_REGISTRATION_KEY <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_key_path>" & ASSETGUARD_REGISTRATION_KEY & "</agent_key_path>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_AGENT_NAME <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <agent_name>" & ASSETGUARD_AGENT_NAME & "</agent_name>"& vbCrLf &"    </enrollment>")
            End If

            If ASSETGUARD_AGENT_GROUP <> "" Then
                strText = Replace(strText, "    </enrollment>", "      <groups>" & ASSETGUARD_AGENT_GROUP & "</groups>"& vbCrLf &"    </enrollment>")
            End If
...
Public Function CheckSvcRunning()
    ...
    Set objExec = WshShell.Exec(scPath & " query AssetGuardSvc")
    If IsStateRunning(objExec.StdOut.ReadAll()) Then
        Session.Property("ASSETGUARDRUNNING") = "Running"
    End If
...
Public Function StartAssetGuardSvc()
	Set WshShell = CreateObject("WScript.Shell")
    StartSvc = "NET START AssetGuardSvc"
    WshShell.run StartSvc, 0, True
End Function

Public Function SetAssetGuardPermissions()
    strArgs = Session.Property("CustomActionData")
    args = Split(strArgs, "/+/")
...
Public Function CreateDumpRegistryKey()
    ...
    strKeyPath = "SOFTWARE\Microsoft\Windows\Windows Error Reporting\LocalDumps\assetguard-agent.exe"

    oReg.CreateKey HKEY_LOCAL_MACHINE, strKeyPath
    oReg.SetExpandedStringValue HKEY_LOCAL_MACHINE, strKeyPath, "DumpFolder",  "%LOCALAPPDATA%\AssetGuardCrashDumps"
