...
void w_rotate_log(int compress, int keep_log_days, int new_day, int rotate_json, int daily_rotations) {
    ...
    os_snprintf(new_path, PATH_MAX, "%s/assetguard-%02d.log", month_dir, tm.tm_mday);
    os_snprintf(new_path_json, PATH_MAX, "%s/assetguard-%02d.json", month_dir, tm.tm_mday);
    ...
    if (new_day || (!new_day && !rotate_json)) {
        ...
        while(!IsFile(compressed_path)){
            counter++;
            os_snprintf(new_path, PATH_MAX, "%s/assetguard-%02d-%03d.log", month_dir, tm.tm_mday, counter);
            os_snprintf(compressed_path, PATH_MAX, "%s.gz", new_path);
        }
        ...
        if (counter == daily_rotations) {
            if (daily_rotations == 1 && counter == 1) {
                ...
                os_snprintf(rename_path, PATH_MAX, "%s/assetguard-%02d.log.gz", month_dir, tm.tm_mday);
                os_snprintf(old_rename_path, PATH_MAX, "%s/assetguard-%02d-001.log.gz", month_dir, tm.tm_mday);
                counter = 1;
                while (counter < daily_rotations) {
                    ...
                    os_snprintf(old_rename_path, PATH_MAX, "%s/assetguard-%02d-%03d.log.gz", month_dir, tm.tm_mday, counter);
                }
                os_snprintf(new_path, PATH_MAX, "%s/assetguard-%02d-%03d.log", month_dir, tm.tm_mday, counter - 1);
            }
        }
    ...
    if (new_day || (!new_day && rotate_json)) {
        ...
        while(!IsFile(compressed_path)) {
            counter++;
            os_snprintf(new_path_json, PATH_MAX, "%s/assetguard-%02d-%03d.json", month_dir, tm.tm_mday, counter);
            os_snprintf(compressed_path, PATH_MAX, "%s.gz", new_path_json);
        }
        ...
        if (counter == daily_rotations) {
            if (daily_rotations == 1 && counter == 1) {
                ...
                os_snprintf(rename_path, PATH_MAX, "%s/assetguard-%02d.json.gz", month_dir, tm.tm_mday);
                os_snprintf(old_rename_path, PATH_MAX, "%s/assetguard-%02d-001.json.gz", month_dir, tm.tm_mday);
                counter = 1;
                while (counter < daily_rotations) {
                    if (rename_ex(old_rename_path, rename_path) != 0) {
                        merror("Couldn't rename compressed log '%s' to '%s': '%s'", old_rename_path, rename_path, strerror(errno));
                    ...
                    counter++;
                    snprintf(rename_path, PATH_MAX, "%s", old_rename_path);
                    os_snprintf(old_rename_path, PATH_MAX, "%s/assetguard-%02d-%03d.json.gz", month_dir, tm.tm_mday, counter);
                }
                os_snprintf(new_path_json, PATH_MAX, "%s/assetguard-%02d-%03d.json", month_dir, tm.tm_mday, counter - 1);
            }
        }
...
void remove_old_logs_m(const char * base_dir, int year, int month, time_t threshold) {
    ...
    while (dirent = readdir(dir), dirent) {
        ...
        if (sscanf(dirent->d_name, "assetguard-%02d.log", &day) > 0) {
            tm.tm_mday = day;
        ...
        if (sscanf(dirent->d_name, "assetguard-%02d-%03d.log", &day, &counter) > 0) {
            tm.tm_mday = day;
        ...
        if (sscanf(dirent->d_name, "assetguard-%02d.json", &day) > 0) {
            tm.tm_mday = day;
        ...
        if (sscanf(dirent->d_name, "assetguard-%02d-%03d.json", &day, &counter) > 0) {
            tm.tm_mday = day;
