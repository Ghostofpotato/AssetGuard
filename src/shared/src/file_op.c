    ...
    if (!muname[0]){
        if (read_version = get_unix_version(), read_version){
            snprintf(muname, 512, "%s |%s |%s |%s |%s [%s|%s: %s] - %s %s",
                    ...
                    __assetguard_name, __assetguard_version);
...
const char *getuname()
{
    ...
    snprintf(ret, OS_SIZE_1024, "Microsoft Windows - %s %s", __assetguard_name, __assetguard_version);
    return ret;
}
...
char *w_homedir(char *arg) {
    ...
    if (realpath("/proc/self/exe", buff) != NULL) {
        ...
        // The path was not found so read ASSETGUARD_HOME env var
        char * home_env = NULL;
        if (home_env = getenv(ASSETGUARD_HOME_ENV), home_env) {
