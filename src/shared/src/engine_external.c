...
static const char* const XML_TAG_ASSETGUARD_CONFIG = ASSETGUARDCONFIG;
...
char* get_indexer_cnf(const char* cnf_file, char* err_buf, size_t err_buf_size)
{
    ...
    snprintf(err_buf, err_buf_size, "Could not find <%s> element in configuration file %s", ASSETGUARDCONFIG, cnf_file);
    XML_NODE nodes_to_clear[] = {root_nodes};
    cleanup_xml_resources(&xml, nodes_to_clear, 1);
    return NULL;
}
