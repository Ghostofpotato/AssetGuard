/*
 * AssetGuard Module for Agent Upgrading
...
int wm_agent_upgrade_validate_version(const char *assetguard_version, const char *platform, wm_upgrade_command command, void *task) {
    char *tmp_agent_version = NULL;
    char *manager_version = NULL;
    int return_code = WM_UPGRADE_GLOBAL_DB_FAILURE;

    if (assetguard_version) {
        if (tmp_agent_version = strchr(assetguard_version, 'v'), tmp_agent_version) {

            if (compare_assetguard_versions(tmp_agent_version, WM_UPGRADE_MINIMAL_VERSION_SUPPORT, true) < 0) {
                ...
                if (manager_version = strchr(__ossec_version, 'v'), manager_version) {
                    ...
                    if (compare_assetguard_versions(upgrade_task->wpk_version, WM_UPGRADE_5X_MINIMUM_VERSION, true) >= 0 &&
                        compare_assetguard_versions(tmp_agent_version, WM_UPGRADE_REQUIRED_INTERMEDIATE_VERSION, true) < 0) {
                        return_code = WM_UPGRADE_INTERMEDIATE_VERSION_REQUIRED;
                    } else if (!upgrade_task->force_upgrade) {
                        if (compare_assetguard_versions(tmp_agent_version, upgrade_task->wpk_version, true) >= 0) {
                            return_code = WM_UPGRADE_NEW_VERSION_LESS_OR_EQUAL_THAN_CURRENT;
                        } else if (compare_assetguard_versions(upgrade_task->wpk_version, manager_version, true) > 0) {
...
int wm_agent_upgrade_validate_wpk_version(wm_agent_info *agent_info, wm_upgrade_task *task, const char *wpk_repository_config) {
    ...
    if (!task->wpk_repository) {
        ...
        } else if (compare_assetguard_versions(task->wpk_version, "v4.0.0", true) < 0) {
    ...
    if (!strcmp(agent_info->platform, "windows")) {
        snprintf(path_url, OS_SIZE_2048, "%swindows/",
                 repository_url);
        snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_windows.wpk",
                 task->wpk_version);
    } else if (!strcmp(agent_info->platform, "darwin")) {
        if (compare_assetguard_versions(task->wpk_version, WM_UPGRADE_NEW_VERSION_STRUCTURE_REPOSITORY, true) >= 0) {
            ...
            snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_macos_%s.%s.wpk",
            ...
            snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_macos_%s.wpk",
                    task->wpk_version, agent_info->architecture);
        }
    } else {
        if (compare_assetguard_versions(task->wpk_version, WM_UPGRADE_NEW_LINUX_VERSION_REPOSITORY, true) >= 0) {
            if (compare_assetguard_versions(task->wpk_version, WM_UPGRADE_NEW_VERSION_STRUCTURE_REPOSITORY, true) >= 0) {
                ...
                snprintf(path_url, OS_SIZE_2048, "%slinux/%s/%s/",
                         repository_url, agent_info->package_type, package_architecture);
                snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_linux_%s.%s.wpk",
                         task->wpk_version, package_architecture, agent_info->package_type);
            } else {
                snprintf(path_url, OS_SIZE_2048, "%slinux/%s/",
                         repository_url, agent_info->architecture);
                snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_linux_%s.wpk",
                         task->wpk_version, agent_info->architecture);
            }
            ...
            snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_%s_%s.%s_%s.wpk",
            ...
            snprintf(file_url, OS_SIZE_2048, "assetguard_agent_%s_%s_%s_%s.wpk",
            ...
            if (next_line = strchr(version, '\n'), next_line) {
                *next_line = '\0';
                if (sha1 = strchr(version, ' '), sha1) {
                    *sha1 = '\0';
                    if (compare_assetguard_versions(task->wpk_version, version, true) == 0) {
