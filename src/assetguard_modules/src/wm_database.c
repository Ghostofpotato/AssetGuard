/*
 * AssetGuard Module for SQLite database syncing
 ...
 *        In addition it will remove from global.db in assetguard-db all the agents that
...
wmodule* wm_database_read() {
    ...
    data.sync_agents = getDefine_Int("assetguard_database", "sync_agents", 0, 1);
    data.real_time = getDefine_Int("assetguard_database", "real_time", 0, 1);
    data.interval = getDefine_Int("assetguard_database", "interval", 0, 86400);
    data.max_queued_events = getDefine_Int("assetguard_database", "max_queued_events", 0, INT_MAX);
