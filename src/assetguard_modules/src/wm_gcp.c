/*
 * AssetGuard Module for Security Configuration Assessment
...
void wm_gcp_pubsub_run(const wm_gcp_pubsub *data) {
    ...
    if (data->max_messages) {
        ...
        snprintf(int_to_string, OS_SIZE_1024, "%d", data->max_messages);
    ...
    if (data->num_threads) {
        char *int_to_string;
        os_malloc(OS_SIZE_1024, int_to_string);
        snprintf(int_to_string, OS_SIZE_1024, "%d", data->num_threads);
