/*
 * AssetGuard Module for custom command execution
...
void * wm_command_main(wm_command_t * command) {
    ...
    if (command->agent_cfg && !getDefine_Int("assetguard_command", "remote_commands", 0, 1)) {
    ...
    do {
        ...
        case WM_ERROR_TIMEOUT:
            mterror(WM_COMMAND_LOGTAG, "%s: Timeout overtaken. You can modify your command timeout at '%s'. Exiting...", command->tag, ASSETGUARDCONF);
            break;
