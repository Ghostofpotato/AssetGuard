/*
 * AssetGuard Module for Agent control
...
cJSON *wm_control_dump() {
    ...
    cJSON_AddItemToObject(root,"assetguard_control",wm_wd);
    return root;
}
...
/**
 * @brief Wait for assetguard-manager service to be in active state
 *
...
STATIC bool wm_control_wait_for_service_active() {
    ...
    while (elapsed < timeout) {
        FILE *fp = popen("systemctl is-active assetguard-manager 2>/dev/null", "r");
        if (fp) {
            char state[256];
            if (fgets(state, sizeof(state), fp)) {
                ...
                if (strcmp(state, "inactive") == 0 || strcmp(state, "failed") == 0) {
                    pclose(fp);
                    mterror(WM_CONTROL_LOGTAG, "Service assetguard-manager is in state '%s', cannot reload", state);
                    return false;
                }
    ...
    mterror(WM_CONTROL_LOGTAG, "Service assetguard-manager is not active after waiting %d seconds", timeout);
    return false;
}
...
size_t wm_control_execute_action(const char *action, char **output) {
    ...
    if (use_systemd) {
        ...
        exec_cmd[2] = "assetguard-manager";
        mtinfo(WM_CONTROL_LOGTAG, "Executing '%s' on manager using systemctl", action);
    } else {
        exec_cmd[0] = "bin/assetguard-control";
        exec_cmd[1] = (char *)action;
        mtinfo(WM_CONTROL_LOGTAG, "Executing '%s' on manager using assetguard-control", action);
    }
