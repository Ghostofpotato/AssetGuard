...
The **InventorySync VD Test Tool** (`inventory_sync_testtool`) is a comprehensive testing utility designed to validate the complete end-to-end integration between the **InventorySync** module and the **Vulnerability Detection (VD)** scanner in AssetGuard.
...
It simulates agent inventory data ingestion with real **FlatBuffer** messages and triggers vulnerability scanning workflows without requiring a full AssetGuard deployment.
...
The test tool **requires** a running AssetGuard Indexer instance to validate end-to-end functionality.
  ...
  "indexer": {
    ...
    "ssl": {
      "certificate_authorities": [
        "/var/assetguard-manager/etc/certs/root-ca.pem"
      ],
      "certificate": "/var/assetguard-manager/etc/certs/manager.pem",
      "key": "/var/assetguard-manager/etc/certs/manager-key.pem"
    }
  }
}
```
...
"data_values": [
  {
    ...
    "payload": {
      ...
      "_id": "assetguard_001_f033cfe690b80a478fb4c832934f0fd55927c349",
      "_score": 2,
      "_source": {
        "assetguard": {
          "agent": {
  ...
  },
  {
    ...
    "payload": {
      "_index": "assetguard-states-inventory-packages",
      "_id": "assetguard_001_f033cfe690b80a478fb4c832934f0fd55927c350",
      "_score": 2,
      "_source": {
        "assetguard": {
          "agent": {
...
"data_context": [
  {
    "payload": {
      "_index": "assetguard-states-inventory-system",
      "_id": "assetguard_001_f033cfe690b80a478fb4c832934f0fd55927c351",
      "_score": 2,
      "_source": {
        "assetguard": {
          "agent": {
...
Start {
  ...
  index: ["assetguard-states-inventory-system", "assetguard-states-inventory-packages"],
  agentname: "ubuntu22",
  agentversion: "v5.0.0",
  architecture: "aarch64",
  hostname: "ubuntu22",
...
DataValue {
  ...
  index: "assetguard-states-inventory-packages" / "assetguard-states-inventory-hotfixes",
...
DataContext {
  session: <StartAck.session>,
  seq: <...>,
  index: "assetguard-states-inventory-system" / ...,
  data: <JSON payload stored as raw bytes>
}
```
  ...
  "configurations": [
    {
      ...
      "args": [
        "--input",
        "${workspaceFolder}/src/assetguard_modules/inventory_sync/testtool/test_data/INPUT_000.json",
        "--config",
        "${workspaceFolder}/src/assetguard_modules/inventory_sync/testtool/test_data/config.json",
        "--wait",
        "15",
        "--verbose"
      ],
...
./inventory_sync_testtool \
  --input src/assetguard_modules/inventory_sync/testtool/test_data/INPUT_000.json \
  --config src/assetguard_modules/inventory_sync/testtool/test_data/config.json \
  --wait 15 \
  --verbose
```
...
1. **Stateful Testing**
   ...
   ```bash
   rm -rf /tmp/assetguard_inventorysync_test.db
   ```
