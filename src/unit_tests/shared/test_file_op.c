...
#include "../wrappers/assetguard/shared/string_op_wrappers.h"
...
{
    char *argv0 = "/usr/share/assetguard/bin/test";
    ...
    expect_string(__wrap_stat, __file, "/usr/share/assetguard");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/usr/share/assetguard");
    free(val);
}
...
void test_w_homedir_env_var(void **state)
{
    ...
    expect_string(__wrap_getenv, name, ASSETGUARD_HOME_ENV);
    will_return(__wrap_getenv, "/home/assetguard");

    expect_string(__wrap_stat, __file, "/home/assetguard");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/home/assetguard");
    free(val);
}
...
void test_w_homedir_stat_fail(void **state)
{
    ...
    expect_string(__wrap__merror_exit, formatted_msg, "(1108): Unable to find AssetGuard install directory. Export it to ASSETGUARD_HOME environment variable.");
