...
int __wrap_wm_agent_upgrade_validate_version(const char *assetguard_version, const char *platform, wm_upgrade_command command, void *task) {
    check_expected(assetguard_version);
    check_expected(platform);
    check_expected(command);
