...
void test_success_valid_configuration_host_IP(void **state) {
    ...
    expect_string(__wrap_Read_Indexer, config_file, ASSETGUARDCONF);
    expect_string(__wrap_get_indexer_cnf, cnf_file, test_path);

    assert_int_equal(Read_Indexer(ASSETGUARDCONF), OS_SUCCESS);
...
void test_fail_invalid_enabled_setting(void **state) {
    ...
    assert_int_equal(Read_Indexer(ASSETGUARDCONF), OS_INVALID);
