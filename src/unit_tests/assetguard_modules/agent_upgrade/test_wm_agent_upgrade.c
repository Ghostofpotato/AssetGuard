...
#include "../../wrappers/assetguard/assetguard_modules/wm_agent_upgrade_agent_wrappers.h"
...
void test_wm_agent_upgrade_dump_enabled(void **state)
{
    ...
    #ifdef TEST_SERVER
    os_strdup("assetguard.com/packages", config->manager_config.wpk_repository);
    ...
    assert_string_equal(cJSON_GetObjectItem(conf, "wpk_repository")->valuestring, "assetguard.com/packages");
    #else
...
void test_wm_agent_upgrade_destroy(void **state)
{
    ...
    #else
    expect_string(__wrap__mtinfo, tag, "assetguard-modulesd:agent-upgrade");
    #endif
