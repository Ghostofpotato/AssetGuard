    ...
    case FIM_AUDIT_KEY:
        if ((pconfig = strstr(buffer, "type=CONFIG_CHANGE"), pconfig) &&
            ...
            if (p_dir && *p_dir != '\0') {
                ...
                snprintf(msg_alert, 512, "assetguard: Audit: Monitored directory was removed: Audit rule removed");
                ...
                // Send alert
                char msg_alert[512 + 1];
                snprintf(msg_alert, 512, "assetguard: Audit: Detected rules manipulation: Audit rules removed");
                SendMSG(syscheck.queue, msg_alert, "syscheck", LOCALFILE_MQ);
                ...
                if (count_reload_retries < AUDIT_LOAD_RETRIES) {
                    ...
                    // Send alert
                    char msg_alert[512 + 1];
                    snprintf(msg_alert, 512, "assetguard: Audit: Detected rules manipulation: Max rules reload retries");
                    SendMSG(syscheck.queue, msg_alert, "syscheck", LOCALFILE_MQ);
                    // Stop thread
