...
title: 'Release [ASSETGUARD VERSION] - Release Candidate [RC VERSION] - WPK upgrade tests'
labels: 'WPK, core'
assignees: ''
...
AssetGuard versions to test (Upgrade to the current agent version):
### Linux
...
- [ ] The assetguard home backup is restored correctly (no traces of the installation, but only the `.tar.gz` backup and the logs).
...
- [ ] AssetGuard service runs assetguard-control (`systemctl cat assetguard-agent.service`)
- [ ] AssetGuard service runs ossec-control (`systemctl cat assetguard-agent.service`)
- [ ] The service was enabled (`systemctl is-enabled assetguard-agent.service`)
- [ ] Init file runs assetguard-control (`cat /etc/rc.d/init.d/assetguard-agent`)
- [ ] Init file runs ossec-control (`cat /etc/rc.d/init.d/assetguard-agent`)
- [ ] AssetGuard as service is enabled `chkconfig --list` 
- [ ] AssetGuard starts and connects when the backup is restored (`cat /var/ossec/var/run/ossec-agentd.state`)
- [ ] AssetGuard starts and connects automatically when the system is rebooted.
- [ ] Restore SELinux policies (`semodule -l | grep -i assetguard`) (DISABLED)
...
## Windows:
### UPGRADE FAIL
- [ ] AssetGuard-Agent folder tree:  No files are lost after the rollback. The logs of the failed upgrade (`ossec.log`) are kept.
- [ ] After the rollback the agent connects to the manager
- [ ] After reboot, the AssetGuard-Agent starts and connects to the manager.
- [ ] The correct AssetGuard-Agent version is shown in the list of Windows' `programs and features`.
- [ ] A new version of AssetGuard-Agent can be manually installed via MSI after the rollback process.

### UPGRADE OK

- [ ] Message `Upgrade finished successfully.` is shown in `upgrade.log` file.
- [ ] AssetGuard service is started and the agent is connected to the manager.
