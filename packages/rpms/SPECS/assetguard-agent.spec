...
Conflicts:   ossec-hids ossec-hids-agent assetguard-local
AutoReqProv: no
...
%package -n assetguard-agent-debuginfo
Requires: assetguard-agent = %{_version}-%{_release}
Summary: Debug information for package %{name}.
%description -n assetguard-agent-debuginfo
...
install -m 0755 src/init/templates/ossec-hids-rh.init ${RPM_BUILD_ROOT}%{_initrddir}/assetguard-agent
sed -i "s:ASSETGUARD_HOME_TMP:%{_localstatedir}:g" src/init/templates/assetguard-agent.service
install -m 0644 src/init/templates/assetguard-agent.service ${RPM_BUILD_ROOT}/usr/lib/systemd/system/
...
%pre
# Create the assetguard group if it doesn't exists
if command -v getent > /dev/null 2>&1 && ! getent group assetguard > /dev/null 2>&1; then
  groupadd -r assetguard
elif ! getent group assetguard > /dev/null 2>&1; then
  groupadd -r assetguard
fi
# Create the assetguard user if it doesn't exists
if ! getent passwd assetguard > /dev/null 2>&1; then
  useradd -g assetguard -G assetguard -d %{_localstatedir} -r -s /sbin/nologin assetguard
fi

# Get version information (only for old installations)
if [ -f "%{_localstatedir}/bin/assetguard-control" ]; then
  OLD_VERSION=`%{_localstatedir}/bin/assetguard-control info -v 2>/dev/null`
...
if [ $1 = 2 ]; then
  ...
  # Check for SysV
    ...
    touch %{_localstatedir}/tmp/assetguard.restart
  elif %{_localstatedir}/bin/assetguard-control status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    touch %{_localstatedir}/tmp/assetguard.restart
  elif %{_localstatedir}/bin/ossec-control status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    touch %{_localstatedir}/tmp/assetguard.restart
  fi
  %{_localstatedir}/bin/ossec-control stop > /dev/null 2>&1 || %{_localstatedir}/bin/assetguard-control stop > /dev/null 2>&1
fi
...
if [ $1 = 1 ]; then
  ...
  chown assetguard:assetguard %{_localstatedir}/logs/active-responses.log
  ...
  chown root:assetguard %{_localstatedir}/etc/ossec.conf
...
if [ -r /etc/SuSE-release ] && grep -q "VERSION = 11" /etc/SuSE-release 2>/dev/null; then
    cp -p %{_localstatedir}/packages_files/agent_installation_scripts/src/init/ossec-hids-suse.init /etc/init.d/assetguard-agent
    chmod 755 /etc/init.d/assetguard-agent
fi

if [[ -d /run/systemd/system ]]; then
  rm -f %{_initrddir}/assetguard-agent
fi
...
if ([ "X${DIST_NAME}" = "Xrhel" ] || [ "X${DIST_NAME}" = "Xcentos" ] || [ "X${DIST_NAME}" = "XCentOS" ]) && [ "${DIST_VER}" == "5" ]; then
  if command -v getenforce > /dev/null 2>&1; then
    if [ $(getenforce) !=  "Disabled" ]; then
      chcon -t textrel_shlib_t  %{_localstatedir}/lib/libassetguardext.so
    fi
  fi
else
...
if getent group ossec > /dev/null 2>&1; then
  find %{_localstatedir}/ -group ossec -user root -exec chown root:assetguard {} \; > /dev/null 2>&1 || true
  if getent passwd ossec > /dev/null 2>&1; then
    find %{_localstatedir}/ -group ossec -user ossec -exec chown assetguard:assetguard {} \; > /dev/null 2>&1 || true
    userdel ossec > /dev/null 2>&1
  fi
  if getent passwd ossecm > /dev/null 2>&1; then
    find %{_localstatedir}/ -group ossec -user ossecm -exec chown assetguard:assetguard {} \; > /dev/null 2>&1 || true
    userdel ossecm > /dev/null 2>&1
  fi
  if getent passwd ossecr > /dev/null 2>&1; then
    find %{_localstatedir}/ -group ossec -user ossecr -exec chown assetguard:assetguard {} \; > /dev/null 2>&1 || true
...
if [ $1 = 0 ]; then
  ...
  validate_uninstall() {
    ...
    if [ -n "$VALIDATION_TOKEN" ] && [ -n "$VALIDATION_LOGIN" ]; then
      validation_command="%{_localstatedir}/bin/assetguard-agentd --uninstall-auth-token=${VALIDATION_TOKEN} --uninstall-auth-login=${VALIDATION_LOGIN} --uninstall-auth-host=${VALIDATION_HOST} --uninstall-ssl-verify=${VALIDATION_SSL_VERIFY}"
    elif [ -n "$VALIDATION_TOKEN" ]; then
      validation_command="%{_localstatedir}/bin/assetguard-agentd --uninstall-auth-token=${VALIDATION_TOKEN} --uninstall-auth-host=${VALIDATION_HOST} --uninstall-ssl-verify=${VALIDATION_SSL_VERIFY}"
    elif [ -n "$VALIDATION_LOGIN" ]; then
      validation_command="%{_localstatedir}/bin/assetguard-agentd --uninstall-auth-login=${VALIDATION_LOGIN} --uninstall-auth-host=${VALIDATION_HOST} --uninstall-ssl-verify=${VALIDATION_SSL_VERIFY}"
    else
...
[ -r %{_sysconfdir}/localtime ] && cp -fpL %{_sysconfdir}/localtime %{_localstatedir}/etc
 chown root:assetguard %{_localstatedir}/etc/localtime
 chmod 0640 %{_localstatedir}/etc/localtime

%postun

DELETE_ASSETGUARD_USER_AND_GROUP=0

# If the upgrade downgrades to earlier versions, it will create the ossec
# group and user, we need to delete assetguard ones
if [ $1 = 1 ]; then
  if command -v %{_localstatedir}/bin/ossec-control > /dev/null 2>&1; then
    find %{_localstatedir} -group assetguard -exec chgrp ossec {} +
    find %{_localstatedir} -user assetguard -exec chown ossec {} +
    DELETE_ASSETGUARD_USER_AND_GROUP=1
  fi
...
# If the package is been uninstalled or we want to delete assetguard user and group
if [ $1 = 0 ] || [ $DELETE_ASSETGUARD_USER_AND_GROUP = 1 ]; then
  # Remove the assetguard user if it exists
  if getent passwd assetguard > /dev/null 2>&1; then
    userdel assetguard >/dev/null 2>&1
  fi
  # Remove the assetguard group if it exists
  if command -v getent > /dev/null 2>&1 && getent group assetguard > /dev/null 2>&1; then
    groupdel assetguard >/dev/null 2>&1
  elif getent group assetguard > /dev/null 2>&1; then
    groupdel assetguard >/dev/null 2>&1
  fi
...
%posttrans
if [ -f %{_sysconfdir}/systemd/system/assetguard-agent.service ]; then
  rm -rf %{_sysconfdir}/systemd/system/assetguard-agent.service
...
if [ -f %{_localstatedir}/tmp/assetguard.restart ]; then
  ...
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 ; then
    systemctl daemon-reload > /dev/null 2>&1
    systemctl restart assetguard-agent.service > /dev/null 2>&1
  elif command -v service > /dev/null 2>&1; then
    service assetguard-agent restart > /dev/null 2>&1
  else
    %{_localstatedir}/bin/assetguard-control restart > /dev/null 2>&1
  fi
fi
...
%files
%defattr(-,root,root)
%config(missingok) %{_initrddir}/assetguard-agent
/usr/lib/systemd/system/assetguard-agent.service
%dir %attr(750, root, assetguard) %{_localstatedir}
%attr(440, assetguard, assetguard) %{_localstatedir}/VERSION.json
%dir %attr(750, root, assetguard) %{_localstatedir}/active-response
%dir %attr(750, root, assetguard) %{_localstatedir}/active-response/bin
%attr(750, root, assetguard) %{_localstatedir}/active-response/bin/*
%dir %attr(750, root, root) %{_localstatedir}/bin
%attr(750, root, root) %{_localstatedir}/bin/*
%dir %attr(750, root, assetguard) %{_localstatedir}/backup
%dir %attr(770, assetguard, assetguard) %{_localstatedir}/etc
%attr(640, root, assetguard) %config(noreplace) %{_localstatedir}/etc/client.keys
%attr(640, root, assetguard) %{_localstatedir}/etc/internal_options*
%attr(640, root, assetguard) %{_localstatedir}/etc/localtime
%attr(640, root, assetguard) %config(noreplace) %{_localstatedir}/etc/local_internal_options.conf
%attr(640, root, assetguard) %ghost %{_localstatedir}/etc/ossec.conf
%attr(640, root, assetguard) %{_localstatedir}/etc/wpk_root.pem
%dir %attr(770, root, assetguard) %{_localstatedir}/etc/shared
%dir %attr(750, root, assetguard) %{_localstatedir}/lib
%attr(750, root, assetguard) %{_localstatedir}/lib/*
%dir %attr(770, assetguard, assetguard) %{_localstatedir}/logs
%attr(660, assetguard, assetguard) %ghost %{_localstatedir}/logs/active-responses.log
%attr(660, root, assetguard) %ghost %{_localstatedir}/logs/ossec.log
%attr(660, root, assetguard) %ghost %{_localstatedir}/logs/ossec.json
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/logs/assetguard
...
%dir %attr(750, root, assetguard) %{_localstatedir}/queue
%dir %attr(770, assetguard, assetguard) %{_localstatedir}/queue/sockets
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/diff
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/fim
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/fim/db
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/syscollector
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/syscollector/db
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/sca
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/sca/db
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/agent_info
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/agent_info/db
%attr(640, root, assetguard) %{_localstatedir}/queue/syscollector/norm_config.json
%dir %attr(770, assetguard, assetguard) %{_localstatedir}/queue/alerts
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/rids
%dir %attr(750, assetguard, assetguard) %{_localstatedir}/queue/logcollector
%dir %attr(750, root, assetguard) %{_localstatedir}/ruleset/
%dir %attr(750, root, assetguard) %{_localstatedir}/ruleset/sca
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/generic
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/generic/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/1
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/1/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2023
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amzn/2023/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/sca.files
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/6
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/6/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/7
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/7/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/8
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/8/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/9
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/9/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/10
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/centos/10/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ol/9
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ol/9/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ol/10
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/ol/10/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/sca.files
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/6
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/6/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/7
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/7/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/8
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/8/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/9
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/9/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/10
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rhel/10/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/sca.files
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/12
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/12/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/15
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/15/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/16
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/sles/16/*
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/sca.files
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/12
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/suse/12/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amazon
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/amazon/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/fedora
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/fedora/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/almalinux
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/almalinux/*
%dir %attr(750, assetguard, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rocky
%attr(640, root, assetguard) %config(missingok) %{_localstatedir}/tmp/sca-%{version}-%{release}-tmp/rocky/*
%dir %attr(1770, root, assetguard) %{_localstatedir}/tmp
%dir %attr(750, root, assetguard) %{_localstatedir}/var
%dir %attr(770, root, assetguard) %{_localstatedir}/var/incoming
%dir %attr(770, root, assetguard) %{_localstatedir}/var/run
%dir %attr(770, root, assetguard) %{_localstatedir}/var/selinux
%attr(640, root, assetguard) %{_localstatedir}/var/selinux/*
%dir %attr(770, root, assetguard) %{_localstatedir}/var/upgrade
%dir %attr(770, root, assetguard) %{_localstatedir}/var/wodles
%dir %attr(750, root, assetguard) %{_localstatedir}/wodles
%attr(750, root, assetguard) %{_localstatedir}/wodles/*

%files -n assetguard-agent-debuginfo -f debugfiles.list
...
- Fixed daemon list for service reloading at assetguard-control.
