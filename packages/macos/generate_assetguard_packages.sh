#!/bin/bash
set -e
# Program to build and package OSX assetguard-agent
...
ARCH="intel64"
ASSETGUARD_SOURCE_REPOSITORY="https://github.com/assetguard/assetguard"
SERVICE_PATH="/Library/LaunchDaemons/com.assetguard.agent.plist"
STARTUP_PATH="/Library/StartupItems/ASSETGUARD/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/ASSETGUARD/AssetGuard-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/ASSETGUARD/ASSETGUARD"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $ASSETGUARD_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in AssetGuard's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${ASSETGUARD_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${ASSETGUARD_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${ASSETGUARD_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/assetguard-agent/payload

    if [ -d "$CURRENT_PATH/assetguard-agent" ]; then

        echo "\nThe assetguard agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/assetguard-agent
    fi

    munkipkg --create --json $CURRENT_PATH/assetguard-agent

    cp -f $build_info_file $CURRENT_PATH/assetguard-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/assetguard-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/assetguard-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/assetguard-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/assetguard-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/assetguard-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        ASSETGUARD_PATH="${SOURCES_DIRECTORY}/assetguard"
        git clone --depth=1 -b ${BRANCH_TAG} ${ASSETGUARD_SOURCE_REPOSITORY} "${ASSETGUARD_PATH}"
    else
        ASSETGUARD_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${ASSETGUARD_PATH}" && git rev-parse --short=7 HEAD)"

    export CONFIG="${ASSETGUARD_PATH}/etc/preloaded-vars.conf"
    ASSETGUARD_PACKAGES_PATH="${ASSETGUARD_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${ASSETGUARD_PACKAGES_PATH}/entitlements.plist"

    VERSION="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $ASSETGUARD_PATH/VERSION.json)"

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="assetguard-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="assetguard-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe assetguard agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${ASSETGUARD_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${ASSETGUARD_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/assetguard-agent ; then
        echo "The assetguard agent package for macOS has been successfully built."
        mv $CURRENT_PATH/assetguard-agent/build/* $DESTINATION/
        symbols_pkg_name="assetguard-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
        cp -R "${ASSETGUARD_PATH}/src/symbols"  "${DESTINATION}"
