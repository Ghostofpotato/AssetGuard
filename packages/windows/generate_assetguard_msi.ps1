...
param (
    [string]$MSI_NAME = "assetguard-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows AssetGuard agent msi package.

    PARAMETERS TO BUILD ASSETGUARD-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * ASSETGUARD:
          $ ./generate_assetguard_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_assetguard_msi.ps1 -MSI_NAME assetguard-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_assetguard_msi.ps1 -MSI_NAME assetguard-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildAssetGuardMsi(){
    ...
    & $CANDLE_EXE -nologo .\assetguard-installer.wxs -out "assetguard-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\assetguard-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
  ...
  #compress every pdb file in current folder
    ...
    $ZIP_NAME = $MSI_NAME -replace 'assetguard-agent', 'assetguard-agent-debug-symbols' -replace '\.msi$', '.zip'
