...
from assetguard.core.utils import clean_pid_files
from assetguard.core.wlogging import AssetGuardLogger

#
# Aux functions
#

def set_logging(foreground_mode=False, debug_mode=0) -> AssetGuardLogger:
    """Set cluster logger.
...
def print_version():
    """Print AssetGuard metadata."""
    from assetguard.core.cluster import __author__, __licence__, __version__, __assetguard_name__
    print(f"\n{__assetguard_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('assetguard-manager-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('assetguard-manager-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: AssetGuardLogger):
    ...
    logger : AssetGuardLogger
        Cluster logger.
    """
    from assetguard.core.cluster import local_server, master
    from assetguard.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: AssetGuardLogger):
    ...
    from assetguard.core.cluster import local_server, worker
...
def main():
    """Main function of the assetguard-manager-clusterd script in charge of starting the cluster process."""
    import assetguard.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.ASSETGUARD_PATH}/logs/cluster.log'):
        os.chown(f'{common.ASSETGUARD_PATH}/logs/cluster.log', common.assetguard_uid(), common.assetguard_gid())
        os.chmod(f'{common.ASSETGUARD_PATH}/logs/cluster.log', 0o660)  # nosec B103
    ...
    try:
        assetguard.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    assetguard.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('assetguard-manager-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to assetguard
    if not args.root:
        os.setgid(common.assetguard_gid())
        os.setuid(common.assetguard_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('assetguard-manager-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('assetguard-manager-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('assetguard-manager-clusterd', pid)


if __name__ == '__main__':
    import assetguard.core.cluster.utils as cluster_utils
    from assetguard.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('assetguard_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0
