...
# Set framework path
path.append(dirname(argv[0]) + '/../framework')  # It is necessary to import AssetGuard package

# Import framework
try:
    import assetguard.agent
    from assetguard.agent import upgrade_agents, get_upgrade_result, get_agents
...
def get_script_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--version", type=str, help="Version to upgrade. [Default: latest AssetGuard version]")
...
def list_outdated():
    ...
    agents = assetguard.agent.get_outdated_agents()
...
async def main():
    ...
    except AssetGuardError as assetguard_err:
        print(f"Error {assetguard_err.code}: {assetguard_err.message}")
        if args.debug:
            raise
