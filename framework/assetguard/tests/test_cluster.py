...
@patch('assetguard.cluster.read_config', side_effect=AssetGuardError(1001))
def test_read_config_wrapper_exception(mock_read_config):
    """Verify the exceptions raised in read_config_wrapper."""
    result = cluster.read_config_wrapper()
    assert list(result.failed_items.keys())[0] == AssetGuardError(1001)


@patch('assetguard.cluster.get_node', return_value={'cluster': default_config["name"],
...
@patch('assetguard.cluster.get_node', side_effect=AssetGuardError(1001))
...
def test_get_status_json():
    ...
    expected = AssetGuardResult({'data': {"running": "no"}})
    assert result == expected


@pytest.mark.asyncio
@patch('assetguard.core.cluster.local_client.LocalClient.start', side_effect=None)
async def test_get_health_nodes(mock_unix_connection):
    ...
    with patch('assetguard.cluster.get_health', side_effect=async_mock):
...
async def test_get_nodes_info():
    ...
    with patch('assetguard.cluster.get_nodes', side_effect=valid_node):
    ...
    assert result.failed_items[AssetGuardResourceNotFound(1730)] == {'noexists'}
