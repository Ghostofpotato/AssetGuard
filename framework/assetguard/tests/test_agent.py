...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        ...
        from assetguard.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
        ...
        from assetguard.core.agent import Agent
        ...
        from assetguard.core.tests.test_agent import InitAgent
        from api.util import remove_nones_to_dict
        from assetguard.core.exception import AssetGuardResourceNotFound
        from assetguard.core.wdb_http import AgentsSummary
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsAssetGuardResult), 'The returned object is not an "AffectedItemsAssetGuardResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsAssetGuardResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, AssetGuardResult), 'The returned object is not an "AssetGuardResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsAssetGuardResult), 'The returned object is not an "AssetGuardResult" instance.'
...
@patch('assetguard.core.agent.Agent.reconnect')
@patch('assetguard.agent.get_agents_info', return_value=short_agent_list)
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsAssetGuardResult), 'The returned object is not an "AffectedItemsAssetGuardResult".'
...
@patch('assetguard.agent.send_restart_command')
@patch('assetguard.agent.get_agents_info', return_value=set(short_agent_list))
@patch('assetguard.core.wdb_http.AssetGuardDBHTTPClient._post', side_effect=send_msg_to_wdb_http_post_restartinfo)
@patch('socket.socket.connect')
...
@patch('assetguard.agent.get_agents')
@patch('assetguard.agent.get_groups')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~assetguard,status~active)', 'group=default;((name~assetguard,status~active))'),
    ('default', 'name~assetguard,status~active', 'group=default;(name~assetguard,status~active)')
])
@patch('assetguard.agent.get_agents')
@patch('assetguard.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list, {'status': 'all', 'older_than': '1s', 'version': 'AssetGuard v4.2.0'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, AssetGuardError(1726), None),
])
@patch('assetguard.agent.Agent.remove')
@patch('assetguard.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, AssetGuardException):
...
@patch('assetguard.core.agent.get_manager_status', return_value={'assetguard-manager-authd': 'running'})
...
@patch('assetguard.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('assetguard.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('assetguard.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('assetguard.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, AssetGuardResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, AssetGuardError(1006)),
    (test_shared_path, ['default'], True, AssetGuardError(1405), AssetGuardError(1405)),
    (test_shared_path, ['default'], True, AssetGuardException(1400), AssetGuardInternalError(1727))
])
@patch('assetguard.agent.process_array')
@patch('assetguard.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('assetguard.core.common.SHARED_PATH', new=shared_path):
        ...
        except (AssetGuardError, AssetGuardInternalError) as e:
...
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
@patch('assetguard.core.common.assetguard_gid', return_value=getgrnam('root'))
@patch('assetguard.core.common.assetguard_uid', return_value=getpwnam('root'))
@patch('assetguard.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, AssetGuardResult), 'The returned object is not an "AssetGuardResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', AssetGuardError, 1711),
    ('group-1', AssetGuardError, 1711),
    ('invalid!', AssetGuardError, 1722),
    ('delete-me', AssetGuardInternalError, 1005),
    ('ar.conf', AssetGuardError, 1713),
    ('agent-template.conf', AssetGuardError, 1713)
])
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the AssetGuard Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('assetguard.agent.get_groups')
@patch('assetguard.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [AssetGuardResourceNotFound(1710)]),
    (['default'], [AssetGuardError(1712)]),
    (['none-1', 'none-2'], [AssetGuardResourceNotFound(1710)]),
    (['default', 'none-1'], [AssetGuardError(1712), AssetGuardResourceNotFound(1710)]),
])
@patch('assetguard.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of AssetGuardError
        List of expected AssetGuardError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('assetguard.agent.Agent.add_group_to_agent')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], AssetGuardResourceNotFound(1710), True),
    (['group-1'], ['100'], AssetGuardResourceNotFound(1701), False),
])
@patch('assetguard.agent.Agent.group_exists')
@patch('assetguard.agent.Agent.add_group_to_agent')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
async def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : AssetGuardError
    ...
    catch_exception : bool
        ...
        `AffectedItemsAssetGuardResult` containing the exceptions in its 'failed_items'.
    """
    ...
    except AssetGuardException as ex:
...
@patch('assetguard.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('assetguard.core.agent.Agent.unset_single_group_agent')
@patch('assetguard.agent.get_groups')
@patch('assetguard.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', AssetGuardResourceNotFound(1701)),
    ('group-1', '005', AssetGuardResourceNotFound(1710)),
])
@patch('assetguard.agent.get_agents_info', return_value=short_agent_list)
@patch('assetguard.agent.get_groups', side_effect={'default'})
async def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : AssetGuardError
        The AssetGuardError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (AssetGuardError, AssetGuardResourceNotFound) as error:
...
@patch('assetguard.core.agent.Agent.unset_single_group_agent')
@patch('assetguard.agent.get_agents_info', return_value=short_agent_list)
@patch('assetguard.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], AssetGuardResourceNotFound(1701), True),
    (['any-group'], ['005'], AssetGuardResourceNotFound(1710), False),
])
@patch('assetguard.core.agent.Agent.unset_single_group_agent')
@patch('assetguard.agent.get_agents_info', return_value=short_agent_list)
@patch('assetguard.agent.get_groups', return_value={'group-1'})
async def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            'An "AssetGuardError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsAssetGuardResult), 'The returned object is not an "AffectedItemsAssetGuardResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            'No exception should be raised at this point. An AffectedItemsAssetGuardResult object with at least one ' \
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating AssetGuardInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('assetguard.agent.get_agents_info', return_value=set(full_agent_list))
@patch('assetguard.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('assetguard.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a AssetGuard Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(AssetGuardError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(AssetGuardInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('assetguard.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a AssetGuard Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(AssetGuardInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, assetguard_socket_mock, agent_list, component, configuration):
    ...
    """
    assetguard_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    try:
        ...
        assert error == AssetGuardError(1740)
...
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
@patch('assetguard.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('assetguard.agent.get_agents_info', return_value=full_agent_list)
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('agent_list, expected_error', [
    (['100'], AssetGuardResourceNotFound(1701))
])
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : AssetGuardError
        Expected AssetGuardError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsAssetGuardResult), 'The returned object is not an "AffectedItemsAssetGuardResult".'
    # Check failed items
...
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
@patch('assetguard.core.configuration.upload_group_configuration')
...
@patch('assetguard.core.common.SHARED_PATH', new=test_shared_path)
@patch('assetguard.agent.get_distinct_agents')
@patch('assetguard.agent.get_agent_groups')
@patch('assetguard.agent.get_agents_summary_status')
@patch('assetguard.agent.get_agents')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('assetguard.agent.get_agents_info', return_value=['test', 'test2'])
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('assetguard.agent.get_agents_info', return_value=set(agent_ids_format(range(1, 100000)))):
...
@patch('assetguard.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('assetguard.core.agent.Agent.group_exists', return_value=False)
...
async def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected AssetGuardException code error.
    """
    get_groups_mock.return_value = ['default']
    with pytest.raises(AssetGuardException, match=f".* {expected_exc} .*"):
...
def test_check_uninstall_permission():
    ...
    expected = AssetGuardResult({'message': 'User has permission to uninstall agents'})
