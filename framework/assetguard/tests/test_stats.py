...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        ...
        import assetguard.stats as stats
...
SOCKET_PATH_DAEMONS_MAPPING = {'/var/assetguard-manager/queue/sockets/remote': 'assetguard-manager-remoted',
                               '/var/assetguard-manager/queue/sockets/analysis': 'assetguard-manager-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'assetguard-manager-remoted': '/var/assetguard-manager/queue/sockets/remote',
                               'assetguard-manager-analysisd': '/var/assetguard-manager/queue/sockets/analysis'}
...
def test_totals():
    ...
    with patch('assetguard.stats.totals_', return_value=({})):
        response = stats.totals(date(2019, 8, 13))
        assert response.total_affected_items == len(response.affected_items)
        assert isinstance(response, AffectedItemsAssetGuardResult), 'The result is not AssetGuardResult type'
...
@patch('assetguard.core.common.REMOTED_SOCKET', '/var/assetguard-manager/queue/sockets/remote')
@patch('assetguard.core.common.ANALYSISD_SOCKET', '/var/assetguard-manager/queue/sockets/analysis')
@patch('assetguard.core.common.WDB_SOCKET', '/var/assetguard-manager/queue/db/wdb')
@patch('assetguard.stats.get_daemons_stats_socket')
def test_get_daemons_stats(mock_get_daemons_stats_socket):
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['assetguard-manager-remoted', 'assetguard-manager-analysisd', 'assetguard-manager-db'])

    calls = [call('/var/assetguard-manager/queue/sockets/remote'), call('/var/assetguard-manager/queue/sockets/analysis'),
             call('/var/assetguard-manager/queue/db/wdb')]
    mock_get_daemons_stats_socket.assert_has_calls(calls)
    assert isinstance(response, AffectedItemsAssetGuardResult), 'The result is not AffectedItemsAssetGuardResult type'
    assert response.total_affected_items == len(response.affected_items)


@patch('assetguard.core.common.REMOTED_SOCKET', '/var/assetguard-manager/queue/sockets/wrong_socket_name')
def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['assetguard-manager-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['assetguard-manager-remoted', 'assetguard-manager-analysisd']),
    (['assetguard-manager-remoted'], ['assetguard-manager-remoted']),
    (['assetguard-manager-remoted', 'assetguard-manager-analysisd'], ['assetguard-manager-remoted', 'assetguard-manager-analysisd'])
])
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('assetguard.stats.get_agents_info', return_value={'001', '002', '003', '004', '005'})
@patch('assetguard.core.common.REMOTED_SOCKET', '/var/assetguard-manager/queue/sockets/remote')
@patch('assetguard.core.common.ANALYSISD_SOCKET', '/var/assetguard-manager/queue/sockets/analysis')
@patch('assetguard.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats)
def test_get_daemons_stats_agents(mock_get_daemons_stats_socket, mock_get_agents_info, mock_socket_connect,
    ...
    assert isinstance(result, AffectedItemsAssetGuardResult), 'The result is not an AffectedItemsAssetGuardResult object'
...
@patch('assetguard.core.common.REMOTED_SOCKET', '/var/assetguard-manager/queue/sockets/remote')
@patch('assetguard.core.common.ANALYSISD_SOCKET', '/var/assetguard-manager/queue/sockets/analysis')
@patch('assetguard.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats_all)
...
@patch('assetguard.stats.get_daemons_stats_', return_value=[{"events_decoded": 1.0}])
...
@patch('assetguard.core.agent.Agent.get_stats')
@patch('assetguard.stats.get_agents_info', return_value=['001', '002'])
