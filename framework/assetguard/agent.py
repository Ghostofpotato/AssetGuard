...
from assetguard.core.agent import AssetGuardDBQueryAgents, AssetGuardDBQueryGroupByAgents, Agent, \
    AssetGuardDBQueryGroup, create_upgrade_tasks, get_agents_info, get_groups, get_rbac_filters, send_restart_command, \
...
from assetguard.core.exception import AssetGuardError, AssetGuardInternalError, AssetGuardException, AssetGuardResourceNotFound
from assetguard.core.results import AssetGuardResult, AffectedItemsAssetGuardResult
from assetguard.core.utils import AssetGuardVersion, chmod_r, chown_r, get_hash, mkdir_with_mode, md5, process_array, clear_temporary_caches, \
    full_copy
...
# The rest of codes generated by the socket will be raised as AssetGuard internal errors (500)
...
def get_distinct_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, fields: list = None, q: str = None) -> AffectedItemsAssetGuardResult:
    ...
    result = AffectedItemsAssetGuardResult(all_msg='All selected agents information was returned',
    ...
    if agent_list:
        ...
        with AssetGuardDBQueryGroupByAgents(filter_fields=fields, select=fields, offset=offset, limit=limit, sort=sort,
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
async def get_agents_summary(agent_list: list[str] = None) -> AssetGuardResult:
    ...
    return AssetGuardResult({'data': agents_summary.to_dict()})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_status(agent_list: list[str] = None) -> AssetGuardResult:
    ...
    if agent_list:
        ...
        with AssetGuardDBQueryGroupByAgents(
    ...
    return AssetGuardResult({'data': {'connection': connection, 'configuration': sync_configuration}})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_os(agent_list: list[str] = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Affected items.
    """
    result = AffectedItemsAssetGuardResult(none_msg='Could not get the operative system of the agents',
...
def reconnect_agents(agent_list: Union[list, str] = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Affected items.
    """
    result = AffectedItemsAssetGuardResult(all_msg='Force reconnect command was sent to all agents',
...
async def restart_agents(agent_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Affected items.
    """
    result = AffectedItemsAssetGuardResult(all_msg='Restart command was sent to all agents',
        ...
        with AssetGuardQueue(common.AR_SOCKET) as wq:
            for agent_id in agent_list:
                ...
                if agent_id not in active_agents:
                    result.add_failed_item(id_=agent_id, error=AssetGuardError(1707))
                    continue
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'],
                  ...
                  post_proc_func=async_list_handler)
async def restart_agents_by_node(agent_list: list = None) -> AffectedItemsAssetGuardResult:
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707], 'force': True},
                  post_proc_func=async_list_handler)
async def restart_agents_by_group(agent_list: list = None) -> AffectedItemsAssetGuardResult:
...
def get_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
    ...
    if agent_list:
        ...
        with AssetGuardDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
                                query=q, **rbac_filters, distinct=distinct) as db_query:
            data = db_query.run()

        if sort and 'version' in sort['fields']:
            data["items"].sort(key=lambda item: AssetGuardVersion(item.get("version"), treat_as_empty=[None, 'N/A']),
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701]})
def get_agents_keys(agent_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Affected items.
    """
    result = AffectedItemsAssetGuardResult(all_msg='Obtained keys for all selected agents',
...
def delete_agents(agent_list: list = None, purge: bool = False, filters: dict = None,
                  q: str = None) -> AffectedItemsAssetGuardResult:
    ...
    """
    result = AffectedItemsAssetGuardResult(all_msg='All selected agents were deleted',
    ...
    if agent_list:
        ...
        with AssetGuardDBQueryAgents(limit=None, select=["id"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        list(map(lambda ag: result.add_failed_item(id_=ag, error=AssetGuardResourceNotFound(1701)), not_found_agents))

        # Add non eligible agents to failed_items
        non_eligible_agents = agent_list - not_found_agents - can_purge_agents
        list(map(lambda ag: result.add_failed_item(id_=ag, error=AssetGuardError(
            1731,
...
def add_agent(name: str = None, agent_id: str = None, key: str = None, ip: str = 'any',
              force: dict = None) -> AssetGuardResult:
    ...
    if len(name) > common.AGENT_NAME_LEN_LIMIT:
        raise AssetGuardError(1738)

    new_agent = Agent(name=name, ip=ip, id=agent_id, key=key, force=force)

    return AssetGuardResult({'data': {'id': new_agent.id, 'key': new_agent.key}})
...
def get_agent_groups(group_list: list = None, offset: int = 0, limit: int = None, sort_by: list = None,
    ...
    result = AffectedItemsAssetGuardResult(all_msg='All selected groups information was returned',
    ...
    if group_list:
        ...
        for invalid_group in set(group_list) - system_groups:
            result.add_failed_item(id_=invalid_group, error=AssetGuardResourceNotFound(1710))

        rbac_filters = get_rbac_filters(system_resources=system_groups, permitted_resources=group_list)

        with AssetGuardDBQueryGroup(**rbac_filters, limit=None) as group_query:
...
def get_group_files(group_list: list = None, offset: int = 0, limit: int = None, search_text: str = None,
                    ...
                    select: str = None, distinct: bool = False) -> AssetGuardResult:
    ...
    -------
    AssetGuardResult
        AssetGuardResult object with the groups files.
    """
    ...
    result = AffectedItemsAssetGuardResult(all_msg='All selected groups files were returned',
                                      some_msg='Some groups files were not returned',
                                      none_msg='No groups files were returned'
                                      )
    if group_id:
        if not Agent.group_exists(group_id):
            result.add_failed_item(id_=group_id, error=AssetGuardResourceNotFound(1710))
            return result
        group_path = path.join(common.SHARED_PATH, group_id)

    if not path.exists(group_path):
        result.add_failed_item(id_=group_path, error=AssetGuardError(1006))
        return result
    ...
    try:
        ...
        raise AssetGuardInternalError(1727, extra_message=str(e))

    return result


@expose_resources(actions=["group:create"], resources=["*:*:*"], post_proc_func=None)
def create_group(group_id: str) -> AssetGuardResult:
    """Creates a group.
    ...
    AssetGuardError(1711)
        If the group already exists.
    AssetGuardError(1713)
    ...
    -------
    AssetGuardResult
        AssetGuardResult object with a operation message.
    """
    ...
    if group_id.lower() == "default" or path.exists(group_path):
        if not path.isfile(group_path):
            raise AssetGuardError(1711, extra_message=group_id)
        else:
            raise AssetGuardError(1713, extra_message=group_id)
    ...
    try:
        ...
        chown_r(group_path, common.assetguard_uid(), common.assetguard_gid())
        chmod_r(group_path, 0o660)
        chmod(group_path, 0o700)
        msg = f"Group '{group_id}' created."
    except Exception as e:
        raise AssetGuardInternalError(1005, extra_message=str(e))

    return AssetGuardResult({'message': msg})


@expose_resources(actions=["group:delete"], resources=["group:id:{group_list}"],
                  post_proc_kwargs={'exclude_codes': [1710, 1712]})
def delete_groups(group_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Affected items.
    """
    result = AffectedItemsAssetGuardResult(all_msg='All selected groups were deleted',
...
async def assign_agents_to_group(group_list: list = None, agent_list: list = None, replace: bool = False,
                           replace_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsAssetGuardResult(all_msg=f'All selected agents were assigned to {group_id}'
...
@expose_resources(actions=["agent:modify_group"], resources=['agent:id:{agent_list}'],
                  post_proc_func=async_list_handler)
async def remove_agent_from_group(group_list: list = None, agent_list: list = None) -> AssetGuardResult:
    ...
    return AssetGuardResult({'message': message})
...
async def remove_agent_from_groups(agent_list: list = None, group_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    """
    agent_id = agent_list[0]
    result = AffectedItemsAssetGuardResult(all_msg='Specified agent was removed from returned groups',
...
async def remove_agents_from_group(agent_list: list = None, group_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsAssetGuardResult(all_msg=f'All selected agents were removed from group {group_id}',
...
def get_outdated_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, select: str = None, q: str = None) -> AffectedItemsAssetGuardResult:
    ...
    if agent_list:
        # Get manager version
        server_info = AssetGuard().to_dict()
...
def upgrade_agents(agent_list: list = None, wpk_repo: str = None, version: str = None, force: bool = False,
                   ...
                   filters: dict = None, q: str = None) -> AffectedItemsAssetGuardResult:
    ...
    """
    result = AffectedItemsAssetGuardResult(all_msg='All upgrade tasks were created',
    ...
    if agent_list:
        ...
        with AssetGuardDBQueryAgents(limit=None, select=["id", "status"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        [result.add_failed_item(id_=agent, error=AssetGuardError(1707)) for agent in non_active_agents]
        non_active_agents = set(non_active_agents)

        # Add non eligible agents to failed_items
        non_eligible_agents = agent_list - not_found_agents - non_active_agents - filtered_agents
        [result.add_failed_item(id_=ag, error=AssetGuardError(
            1731,
            extra_message="some of the requirements are not met -> {}".format(
                ', '.join(f"{key}: {value}" for key, value in filters.items() if key != 'rbac_ids') +
                (f', q: {q}' if q else '')
        ...
        for agent_result_chunk in tasks_results:
            for agent_result in agent_result_chunk['data']:
                ...
                # Upgrade error for specific agents
                    ...
                    error = AssetGuardError(error_code, cmd_error=True, extra_message=agent_result['message'])
                ...
                # Upgrade error for all agents, bad request
                    ...
                    raise AssetGuardError(error_code, cmd_error=True, extra_message=agent_result['message'])

                # Upgrade error for all agents, internal server error
                else:
                    raise AssetGuardInternalError(error_code, cmd_error=True, extra_message=agent_result['message'])
...
def get_upgrade_result(agent_list: list = None, filters: dict = None, q: str = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Upgrade results.
    """
    result = AffectedItemsAssetGuardResult(all_msg='All upgrade tasks were returned',
        ...
        for task_result_chunk in task_results:
            for task_result in task_result_chunk['data']:
                ...
                # Upgrade error for specific agents (no task in DB)
                    ...
                    error = AssetGuardError(error_code, cmd_error=True, extra_message=task_result['message'])
                    result.add_failed_item(id_=str(task_result['agent']).zfill(3), error=error)

                # Upgrade error for all agents, internal server error
                else:
                    raise AssetGuardInternalError(error_code, cmd_error=True, extra_message=task_result['message'])
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agent_config(agent_list: list = None, component: str = None, config: str = None) -> AssetGuardResult:
...
def get_agents_sync_group(agent_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    """
    result = AffectedItemsAssetGuardResult(all_msg='Sync info was returned for all selected agents',
...
def get_file_conf(group_list: list = None, type_conf: str = None, raw: bool = False,
                  filename: str = None) -> AssetGuardResult:
    ...
    -------
    AssetGuardResult
        AssetGuardResult object with the configuration.
    """
    ...
    return AssetGuardResult({'data': configuration.get_file_conf(filename, group_id=group_id, type_conf=type_conf,
                                                            raw=raw)})


@expose_resources(actions=["group:read"], resources=["group:id:{group_list}"], post_proc_func=None)
def get_agent_conf(group_list: list = None, filename: str = 'agent.conf', offset: int = 0,
                   limit: int = common.DATABASE_LIMIT) -> AssetGuardResult:
...
def upload_group_file(group_list: list = None, file_data: str = None, file_name: str = 'agent.conf') -> AssetGuardResult:
    """Update a group file.
    ...
    -------
    AssetGuardResult
        AssetGuardResult object with the confirmation message.
    """
    # We access unique group_id from list, this may change if and when we decide to add option to update files for
    # a list of groups
    group_id = group_list[0]

    return AssetGuardResult({'message': configuration.upload_group_file(group_id, file_data, file_name=file_name)})


def get_full_overview() -> AssetGuardResult:
    """Get information about agents.

    Returns
    -------
    AssetGuardResult
        AssetGuardResult object with information about agents.
    """
    q = ''
    ...
    return AssetGuardResult({'data': result})


@expose_resources(actions=["agent:uninstall"], resources=["*:*:*"], post_proc_func=None)
def check_uninstall_permission() -> AssetGuardResult:
    ...
    -------
    AssetGuardResult
        AssetGuardResult object containing a message indicating if the
        user has the permission to uninstall agents.
    """
    return AssetGuardResult({'message': 'User has permission to uninstall agents'})
