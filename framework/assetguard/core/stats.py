...
from assetguard.core import common, utils
from assetguard.core import assetguard_socket
from assetguard.core.exception import AssetGuardError, AssetGuardInternalError, AssetGuardException
...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise AssetGuardError(1308, extra_message=stat_filename)
        ...
        if len(data) == 4:
            ...
            if len(data) != 5:
                if len(data) in (0, 1):
                    ...
                    raise AssetGuardInternalError(1309)
...
def get_daemons_stats_socket(socket: str, agents_list: Union[list[int], str] = None, last_id: int = None) -> dict:
    """Send message to AssetGuard socket to get statistical information.
    ...
    """
    # Create message
    full_message = assetguard_socket.create_assetguard_socket_message(
    ...
    # Connect to socket
    try:
        s = assetguard_socket.AssetGuardSocketJSON(socket)
    except Exception:
        raise AssetGuardInternalError(1121, extra_message=socket)
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise AssetGuardInternalError(1104, extra_message=str(e))
    except IOError:
        raise AssetGuardError(1308, extra_message=filename)
...
def send_command_to_socket(dest_socket: str, command: str) -> dict:
    ...
    try:
        ...
        raise AssetGuardInternalError(1121)
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise AssetGuardError(1117, extra_message=rec_msg)
