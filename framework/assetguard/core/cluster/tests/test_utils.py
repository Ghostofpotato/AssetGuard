...
with patch('assetguard.core.common.getgrnam'):
    with patch('assetguard.core.common.getpwnam'):
        with patch('assetguard.core.common.assetguard_uid'):
            with patch('assetguard.core.common.assetguard_gid'):
                ...
                from assetguard.core.cluster import utils
                from assetguard.core.exception import AssetGuardError, AssetGuardException, AssetGuardInternalError, AssetGuardPermissionError, \
                    AssetGuardResourceNotFound, AssetGuardHAPHelperError
...
def test_read_cluster_config():
    ...
    with patch('assetguard.core.configuration.load_assetguard_xml', return_value=SystemExit):
    ...
    with patch('assetguard.core.cluster.utils.get_ossec_conf', return_value={'cluster': default_cluster_config}):
        ...
        with pytest.raises(AssetGuardError, match='.* 3004 .*'):
            utils.read_cluster_config()
...
def test_parse_haproxy_helper_config_ko(config: dict, exception_type: AssetGuardException, expected_error_code: str):
...
def test_get_manager_status():
    ...
    with patch('assetguard.core.cluster.utils.glob', return_value=['ossec-0.pid']):
        ...
        # with patch('assetguard.core.cluster.utils.join', return_value='failed') as join_mock:
        with patch('assetguard.core.cluster.utils.os.path.exists', side_effect=exist_mock):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'
...
def test_get_manager_status_ko(mock_stat, exc):
    ...
    with pytest.raises(AssetGuardInternalError, match='.* 1913 .*'):
...
def test_get_cluster_status():
    ...
    AssetGuardInternalError is raised."""
    status = utils.get_cluster_status()
    assert {'running': 'no'} == status

    with patch('assetguard.core.cluster.utils.get_manager_status', side_effect=AssetGuardInternalError(1913)):
        status = utils.get_cluster_status()
        assert {'running': 'no'} == status
...
def test_manager_restart():
    ...
    with patch('assetguard.core.cluster.utils.open', side_effect=None):
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        with patch('socket.socket.recv', return_value=b'ok\n'):
                            with patch('socket.socket.close', side_effect=None):
                                ...
                                assert AssetGuardResult({'message': 'Restart request sent'}) == status
...
def test_get_cluster_items():
    ...
    with patch("os.path.abspath", side_effect=FileNotFoundError):
        with pytest.raises(AssetGuardException, match=".* 3005 .*"):
    ...
    expected = {
        "files": {
            ...
            "excluded_files": ["ar.conf", "assetguard-manager.conf"],
...
def test_ClusterLogger():
    """Verify that ClusterLogger defines the logger used by assetguard-manager-clusterd."""
...
@patch('assetguard.core.cluster.utils.pyDaemonModule.create_pid')
def test_process_spawn_sleep(pyDaemon_create_pid_mock, get_pid_mock):
    ...
    pyDaemon_create_pid_mock.assert_called_once_with(f'assetguard-manager-clusterd_child_{child}', get_pid_mock.return_value)


@pytest.mark.asyncio
@patch('concurrent.futures.ThreadPoolExecutor')
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI')
...
@patch('assetguard.core.cluster.utils.read_cluster_config')
...
@pytest.mark.parametrize('result', [
    AssetGuardError(6001),
    AssetGuardInternalError(1000),
    AssetGuardPermissionError(4000),
...
def test_safe_join(base_dir, input_paths, should_raise):
    if should_raise:
        with pytest.raises(AssetGuardInternalError):
