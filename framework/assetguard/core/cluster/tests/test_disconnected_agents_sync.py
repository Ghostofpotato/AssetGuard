...
with patch("assetguard.core.common.assetguard_uid"), patch("assetguard.core.common.assetguard_gid"):
    ...
    from assetguard.core.cluster.master import DisconnectedAgentSyncTasks
...
@patch("assetguard.core.indexer.disconnected_agents.core_utils.get_utc_now")
...
@pytest.mark.asyncio
@patch(
    "assetguard.core.indexer.disconnected_agents.AssetGuardDBQueryAgents",
...
async def test_sync_agent_batch_success(task):
    ...
    task.disconnected_agent_group_sync = AsyncMock(
        return_value=AffectedItemsAssetGuardResult()
    )
...
async def test_disconnected_agent_group_sync_missing_params(task):
    """
    Verify that missing required parameters raise a AssetGuardError.
    """
    with pytest.raises(AssetGuardError):
...
@pytest.mark.asyncio
@patch("assetguard.core.indexer.disconnected_agents.get_agents_info", return_value={})
...
@pytest.mark.asyncio
@patch(
    "assetguard.core.indexer.disconnected_agents.get_agents_info", return_value={"001": {}}
)
...
@pytest.mark.asyncio
@patch(
    "assetguard.core.indexer.disconnected_agents.asyncio.sleep",
...
async def test_run_cluster_name_sync_full_flow(
    ...
    mock_conf.return_value = {"cluster": {"name": "assetguard-new"}}
