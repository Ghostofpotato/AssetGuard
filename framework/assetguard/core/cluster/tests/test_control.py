...
with patch('assetguard.common.getgrnam'):
    with patch('assetguard.common.getpwnam'):
        with patch('assetguard.common.assetguard_uid'):
            with patch('assetguard.common.assetguard_gid'):
                ...
                from assetguard.core.cluster import control
...
async def test_get_nodes():
    ...
    with patch('assetguard.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
    ...
    with patch('assetguard.core.cluster.local_client.LocalClient.execute', side_effect=[AssetGuardClusterError(3020), 'error']):
        with pytest.raises(AssetGuardClusterError):
            await control.get_nodes(lc=local_client)
...
async def test_get_system_nodes():
    ...
    with patch('assetguard.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
        ...
        for expected in expected_result:
            with patch('assetguard.core.cluster.control.get_nodes', return_value=expected):
