...
import assetguard.core.cluster.cluster
import assetguard.core.manager
import assetguard.core.results as wresults
from assetguard import agent
from assetguard.cluster import get_node_wrapper, get_nodes_info
from assetguard.core import common, exception
from assetguard.core.cluster import local_client, common as c_common
from assetguard.core.exception import AssetGuardException, AssetGuardClusterError, AssetGuardError
from assetguard.core.pyDaemonModule import spawn_process_pool_worker, API_AUTHENTICATION_PROCESS
from assetguard.core.assetguard_socket import assetguard_sendsync


authentication_funcs = {'check_token', 'check_user_master', 'get_permissions', 'get_security_conf'}
events_funcs = {'send_event_to_analysisd'}

node_info = assetguard.core.cluster.cluster.get_node()
pools = common.mp_pools.get()
if node_info['type'] == 'master':
    # Suppress exception when the user running AssetGuard cannot access /dev/shm.
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = assetguard.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        ...
        if not basic_services:
            self.basic_services = ('assetguard-manager-modulesd', 'assetguard-manager-analysisd',
                                   'assetguard-manager-db', 'assetguard-manager-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'assetguard-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.AssetGuardException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or AssetGuardException
            Dictionary with API response or AssetGuardException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_assetguard_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractAssetGuardResult, exception.AssetGuardException)) \
                else wresults.AssetGuardResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.AssetGuardInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.AssetGuardInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.AssetGuardClusterError))
            return e
        except exception.AssetGuardError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.AssetGuardInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_assetguard_status(self):
        """
        There are some services that are required for assetguard to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * AssetGuard must be started before using the API is the services are stopped.

        The basic services assetguard needs to be running are: assetguard-manager-modulesd, assetguard-manager-remoted,
        assetguard-manager-analysisd
        and assetguard-manager-db
        """
        if self.f == assetguard.core.manager.status:
            return

        status = assetguard.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.AssetGuardInternalError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.AssetGuardInternalError(3021)
                except OperationalError as exc:
                    raise exception.AssetGuardInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.AssetGuardInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.AssetGuardInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.AssetGuardInternalError(900)
            ...
            # AssetGuardClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.AssetGuardClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.AssetGuardJSONEncoder)
        except (exception.AssetGuardError, exception.AssetGuardResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.AssetGuardJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.AssetGuardInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.AssetGuardJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.AssetGuardException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.AssetGuardException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of AssetGuardInternalError
        if isinstance(e, exception.AssetGuardInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('ASSETGUARD_HOME', os.path.relpath(h.baseFilename, start=common.ASSETGUARD_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        # a temporary file in /var/assetguard-manager/tmp which needs to be sent to the master before forwarding the request
        client = self.get_client()
        res = json.loads(await client.send_file(os.path.join(common.ASSETGUARD_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_assetguard_object)
        os.remove(os.path.join(common.ASSETGUARD_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.AssetGuardJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_assetguard_object)

    async def forward_request(self) -> [wresults.AbstractAssetGuardResult, exception.AssetGuardException]:
        ...
        Returns
        -------
        wresults.AbstractAssetGuardResult or exception.AssetGuardException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractAssetGuardResult, exception.AssetGuardException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a AssetGuardError exception
                if isinstance(result, AssetGuardClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = AssetGuardError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractAssetGuardResult, exception.AssetGuardException)) \
                else wresults.AssetGuardResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsAssetGuardResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractAssetGuardResult):
        ...
        # It might be a AssetGuardError after reducing
        if isinstance(response, wresults.AffectedItemsAssetGuardResult):
...
class AssetGuardRequestQueue:
    """Represents a queue of AssetGuard requests"""
...
class APIRequestQueue(AssetGuardRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('assetguard').getChild('dapi')
        self.logger.addFilter(assetguard.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_assetguard_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.AssetGuardJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(AssetGuardRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('assetguard').getChild('sendsync')
        self.logger.addFilter(assetguard.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await assetguard_sendsync(**request)
