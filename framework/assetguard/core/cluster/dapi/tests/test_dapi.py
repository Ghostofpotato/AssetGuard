...
with patch('assetguard.common.assetguard_uid'):
    with patch('assetguard.common.assetguard_gid'):
        ...
        from assetguard.core.cluster.dapi.dapi import DistributedAPI, APIRequestQueue, SendSyncRequestQueue
        from assetguard.core.manager import get_manager_status
        from assetguard.core.results import AssetGuardResult, AffectedItemsAssetGuardResult
        from assetguard import agent, cluster, manager, AssetGuardError, AssetGuardInternalError
        from assetguard.core.exception import AssetGuardClusterError, AssetGuardException
...
@pytest.mark.parametrize('kwargs', [
    {'f_kwargs': {'select': ['id']}, 'rbac_permissions': {'mode': 'black'}, 'nodes': ['worker1'],
     'basic_services': ('assetguard-manager-modulesd', 'assetguard-manager-db'), 'request_type': 'local_master'},
...
def test_DistributedAPI_debug_log():
    ...
    logger_ = TestingLogger(logger_name="assetguard-api")
    ...
    logger_ = TestingLogger(logger_name="assetguard")
...
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=AssetGuardResult({'result': 'local'})))
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.forward_request',
       new=AsyncMock(return_value=AssetGuardResult({'result': 'forward'})))
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.execute_remote_request',
       new=AsyncMock(return_value=AssetGuardResult({'result': 'remote'})))
...
def test_DistributedAPI_distribute_function(api_request, request_type, node, expected, f_kwargs):
    ...
    """
    # Mock get_node
    with patch('assetguard.core.cluster.cluster.get_node', return_value={'type': node}):
...
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=AssetGuardResult({'result': 'local'})))
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=AssetGuardResult({'unknown': ['001', '002']})))
...
def test_DistributedAPI_distribute_function_mock_solver(api_request, request_type, node, expected):
    ...
    expected : str
        Expected result
    """
    with patch('assetguard.core.cluster.cluster.get_node', return_value={'type': node, 'node': 'master'}):
...
def test_DistributedAPI_distribute_function_exception():
    ...
    logger_ = logging.getLogger("assetguard")
    with patch("assetguard.core.cluster.dapi.dapi.get_node_wrapper", side_effect=AssetGuardError(4000)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        get_error_result = dapi.get_error_info(Exception("testing"))
        assert 'unknown-node' in get_error_result
        assert get_error_result['unknown-node']['error'] == 'AssetGuard Internal Error. See log for more detail'

    with patch("assetguard.core.cluster.dapi.dapi.get_node_wrapper", side_effect=AssetGuardError(4001)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        with pytest.raises(AssetGuardError, match='.* 4001 .*'):
            dapi.get_error_info(Exception("testing"))

    with patch("assetguard.core.cluster.dapi.dapi.get_node_wrapper", return_value=NodeWrapper()):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
...
def test_DistributedAPI_local_request_errors():
    ...
    with patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
               new=AsyncMock(side_effect=AssetGuardInternalError(1001))):
        dapi_kwargs = {'f': agent.get_agents_summary_status, 'logger': logger}
        ...
        try:
            raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
        except AssetGuardInternalError as e:
            assert e.code == 1001
    ...
    with patch('assetguard.cluster.get_nodes_info', side_effect=json.decoder.JSONDecodeError('test', 'test', 1)):
        with patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.check_assetguard_status'):
...
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.check_assetguard_status', side_effect=None)
...
def test_DistributedAPI_local_request(mock_local_request):
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=AssetGuardError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=AssetGuardInternalError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    exc_code = 3000
    cluster_exc = AssetGuardClusterError(exc_code)
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=cluster_exc)):
        with patch.object(TestingLogger, "error") as logger_error_mock:
            # Test AssetGuardClusterError caught in execute_local_request and ProblemException raised
            dapi_kwargs = {'f': manager.status, 'logger': testing_logger}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=exc_code)

            # Test AssetGuardClusterError is raised when using debug in execute_local_request and distribute_function
            dapi = DistributedAPI(f=manager.status, logger=testing_logger, debug=True)
            try:
                raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
            except AssetGuardClusterError as e:
...
@patch('assetguard.core.cluster.dapi.dapi.node_info', {'type': 'worker'})
@patch('assetguard.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('assetguard.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
...
@patch('assetguard.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'master-node'})
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.get_solver_node', return_value={'worker1': ['001', '002']})
@patch('assetguard.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('assetguard.core.cluster.local_client.LocalClient.send_file', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('assetguard.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('assetguard.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=AssetGuardResult({'testing': ['001', '002']})))
def test_DistributedAPI_tmp_file():
    ...
    with patch('assetguard.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('assetguard.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsAssetGuardResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                           'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs)

    open('/tmp/dapi_file.txt', 'a').close()
    with patch('assetguard.core.cluster.cluster.get_node', return_value={'type': 'unk', 'node': 'master'}):
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs)
...
def test_DistributedAPI_tmp_file_cluster_error():
    ...
    with patch('assetguard.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('assetguard.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsAssetGuardResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            with patch('assetguard.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=AssetGuardClusterError(3022))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            ...
            open('/tmp/dapi_file.txt', 'a').close()
            with patch('assetguard.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=AssetGuardClusterError(1000))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
                raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1000)


@patch('assetguard.core.cluster.local_client.LocalClient.execute',
       new=AsyncMock(return_value='{"items": [{"name": "master"}], "totalItems": 1}'))
@patch('assetguard.agent.Agent.get_agents_overview', return_value={'items': [{'id': '001', 'node_name': 'master'},
...
def test_DistributedAPI_get_solver_node(mock_agents_overview):
    ...
    nodes_info_result = AffectedItemsAssetGuardResult()
    nodes_info_result.affected_items.append({'name': 'master'})
    common.cluster_nodes.set(['master'])

    with patch('assetguard.core.cluster.dapi.dapi.get_nodes_info', new=AsyncMock(return_value=nodes_info_result)):
        with patch('assetguard.core.cluster.dapi.dapi.node_info', {'type': 'master', 'node': 'unknown'}):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
            ...
            expected = AffectedItemsAssetGuardResult()
            expected.affected_items = [{'id': '001', 'node_name': 'master'}]
            with patch('assetguard.agent.get_agents_in_group', return_value=expected):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
...
@pytest.mark.parametrize('api_request', [
    ...
    assetguard.core.manager.status
])
@patch('assetguard.core.manager.get_manager_status', return_value={process: 'running' for process in get_manager_status()})
def test_DistributedAPI_check_assetguard_status(status_mock, api_request):
    """Test `check_assetguard_status` method from class DistributedAPI."""
    dapi = DistributedAPI(f=api_request, logger=logger)
    data = dapi.check_assetguard_status()
    assert data is None
...
@patch('assetguard.core.cluster.dapi.dapi.node_info', {'node': 'random_node'})
def test_DistributedAPI_check_assetguard_status_exception(status_value):
    """Test exceptions from `check_assetguard_status` method from class DistributedAPI."""
    statuses = {process: status_value for process in sorted(get_manager_status())}
    with patch('assetguard.core.manager.get_manager_status',
               return_value=statuses):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger)
        try:
            dapi.check_assetguard_status()
        except AssetGuardInternalError as e:
            assert e.code == 1017
            assert statuses
...
class TestDistributedAPIRunLocal:
    ...
    @patch("assetguard.core.common.reset_context_cache")
    @patch("assetguard.core.common.cluster_nodes")
    @patch("assetguard.core.common.origin_module")
    @patch("assetguard.core.common.current_user")
    @patch("assetguard.core.common.broadcast")
    @patch("assetguard.core.common.rbac")
    ...
    @patch("assetguard.core.common.broadcast")
    @patch("assetguard.core.common.rbac")
    def test_run_local_with_assetguard_exception(
        self,
        mock_rbac_ctx,
        mock_broadcast_ctx,
        mock_current_user_ctx,
        ...
        """Test that AssetGuardException is re-raised properly."""
        mock_broadcast_token = "test_token"
        mock_broadcast_ctx.set.return_value = mock_broadcast_token

        assetguard_exception = AssetGuardException("Test error")
        mock_function.side_effect = assetguard_exception

        with pytest.raises(AssetGuardException) as exc_info:
            DistributedAPI.run_local(
                f=mock_function, f_kwargs=sample_kwargs, **sample_context_vars
            )

        # Verify it's the same exception instance
        assert exc_info.value is assetguard_exception
    ...
    def test_run_local_with_other_exception(
        ...
        """Test that non-AssetGuardException exceptions are not caught."""
        mock_broadcast_token = "test_token"
        mock_broadcast_ctx.set.return_value = mock_broadcast_token
...
@patch("assetguard.core.cluster.common.import_module", return_value="os.path")
@patch("asyncio.get_running_loop")
async def test_APIRequestQueue_run(loop_mock, import_module_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return 'assetguard*request_queue*test ' \
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        logger_mock.assert_called_once_with("Error in DAPI request. The destination node is "
                                            "not connected or does not exist: 'assetguard'.")

        node = NodeMock()
        with patch.object(node, "send_request", side_effect=AssetGuardClusterError(3020, extra_message="test")):
            with patch.object(node, "send_string", return_value=b"noerror"):
                with patch("assetguard.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    server.clients = {"assetguard": node}
                    with pytest.raises(Exception):
                        await apirequest.run()

            with patch.object(node, "send_string", Exception("break while true")):
                with patch("assetguard.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    with patch("assetguard.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception()):
                        apirequest.logger = logging.getLogger("apirequest")
                        with pytest.raises(Exception):
                            await apirequest.run()


@patch("assetguard.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception())
@patch("asyncio.get_running_loop")
async def test_SendSyncRequestQueue_run(loop_mock, contexlib_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return "assetguard*request_queue*test {\"daemon_name\": \"test\"}"
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        with patch.object(node, "send_request", Exception("break while true")):
            with patch("assetguard.core.cluster.dapi.dapi.assetguard_sendsync", side_effect=Exception("break while true")):
                server.clients = {"assetguard": node}
                sendsync.logger = logging.getLogger("sendsync")
                with pytest.raises(Exception):
                    await sendsync.run()

            with patch("assetguard.core.cluster.dapi.dapi.assetguard_sendsync", side_effect="noerror"):
                with pytest.raises(Exception):
                    await sendsync.run()
