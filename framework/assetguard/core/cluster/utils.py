...
from assetguard.core.assetguard_socket import create_assetguard_socket_message
...
api_restart_lockfile = os.path.join(common.ASSETGUARD_PATH, "var", "run", ".api_restart_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'assetguard_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise AssetGuardError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise AssetGuardError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise AssetGuardHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        config_cluster = get_ossec_conf(section='cluster', conf_file=config_file, from_import=from_import)['cluster']
            ...
            raise AssetGuardError(3006, extra_message=e.message)
    except Exception as e:
        raise AssetGuardError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise AssetGuardError(3004, extra_message="Cluster port must be an integer.")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise AssetGuardInternalError(1913, extra_message=str(e))

    processes = ['assetguard-manager-analysisd', 'assetguard-manager-authd', 'assetguard-manager-monitord',
                 'assetguard-manager-remoted', 'assetguard-manager-clusterd',
                 'assetguard-manager-modulesd', 'assetguard-manager-db', 'assetguard-manager-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.ASSETGUARD_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    """
    try:
        cluster_status = {"running": "yes" if get_manager_status()['assetguard-manager-clusterd'] == 'running' else "no"}
...
def manager_restart() -> AssetGuardResult:
    """Restart AssetGuard manager.

    Send 'restart' command to common.CONTROL_SOCKET socket.

    Raises
    ------
    AssetGuardInternalError(1901)
        If the socket path doesn't exist.
    AssetGuardInternalError(1902)
    ...
    try:
        ...
        # command for restarting AssetGuard manager
        msg = 'restart'
        # initialize socket
        if os.path.exists(socket_path):
            try:
                ...
                raise AssetGuardInternalError(1902)
        else:
            raise AssetGuardInternalError(1901)
        ...
        try:
            ...
            if not response.startswith('ok'):
                raise AssetGuardInternalError(1014, extra_message=response)
        except socket.error as e:
            raise AssetGuardInternalError(1014, extra_message=str(e))
    finally:
        fcntl.lockf(lock_file, fcntl.LOCK_UN)
        lock_file.close()
        read_config.cache_clear()

    return AssetGuardResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.ASSETGUARD_PATH, here, 'cluster.json')) as f:
        ...
        raise AssetGuardError(3005, str(e))
...
def safe_join(base_path: str, *paths: str) -> str:
    ...
    if os.path.commonpath([base, final_path]) != base:
        raise AssetGuardInternalError(3003, extra_message=f"unsafe path '{final_path}'")
...
class ClusterLogger(AssetGuardLogger):
    """
    Define the logger used by assetguard-manager-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from assetguard-manager-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'assetguard-manager-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `AssetGuardResult` instance in case the execution did not fail. Return an exception otherwise.
    """
