...
import pytest
from assetguard.agent import get_agents, reconnect_agents
from assetguard.cluster import get_nodes_info
from assetguard.core.results import AffectedItemsAssetGuardResult

from framework.assetguard.core.cluster.hap_helper.assetguard import AssetGuardAgent, AssetGuardDAPI


class TestAssetGuardAgent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert AssetGuardAgent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert AssetGuardAgent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.assetguard.core.cluster.hap_helper.assetguard.DistributedAPI', autospec=True)
class TestAssetGuardDAPI:
    assetguard_dapi = AssetGuardDAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsAssetGuardResult()
    ...
    @mock.patch('framework.assetguard.core.cluster.hap_helper.assetguard.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsAssetGuardResult,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""

        self.assetguard_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.assetguard_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.assetguard_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.assetguard_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.assetguard_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'limit': self.assetguard_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.assetguard_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsAssetGuardResult, limit: Optional[int]
    ):
        ...
        ret_val = await self.assetguard_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'limit': limit or self.assetguard_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.assetguard_dapi.logger,
            request_type='local_master',
        )
