...
ASSETGUARD_UID_KEY = 'assetguard-uid'
ASSETGUARD_TAG_KEY = 'assetguard-tag'
...
def get_ossec_log_fields(log: str, log_format: LoggingFormat = LoggingFormat.plain) -> Union[tuple, None]:
    ...
    if "rootcheck" in tag:  # Unify rootcheck category
        tag = "assetguard-rootcheck"

    return get_utc_strptime(date, '%Y/%m/%d %H:%M:%S'), tag, level.lower(), description


def get_assetguard_active_logging_format() -> LoggingFormat:
    """Obtain the AssetGuard active logging format.
...
def get_ossec_logs(limit: int = 2000) -> list:
    ...
    log_format = get_assetguard_active_logging_format()
    if log_format == LoggingFormat.plain and exists(common.ASSETGUARD_LOG):
        assetguard_log_content = tail(common.ASSETGUARD_LOG, limit)
    elif log_format == LoggingFormat.json and exists(common.ASSETGUARD_LOG_JSON):
        assetguard_log_content = tail(common.ASSETGUARD_LOG_JSON, limit)
    else:
    ...
    for line in assetguard_log_content:
        ...
        if log_fields:
            ...
            # We transform local time (assetguard log file) to UTC with ISO8601 maintaining time integrity
...
def validate_ossec_conf() -> dict:
    ...
    try:
        ...
        # Re-raise AssetGuardError (includes validation errors)
        raise
    except Exception as e:
        # Wrap other exceptions as validation errors
        raise AssetGuardError(1908, extra_message=str(e))
...
def get_update_information_template(
        uuid: str,
        update_check: bool,
        current_version: str = f"v{assetguard.__version__}",
    ...
    current_version : str, optional
        Indicates the current version of AssetGuard, by default assetguard.__version__.
...
async def query_update_check_service(installation_uid: str) -> dict:
    ...
    installation_uid : str
        AssetGuard UID to include in the query.
    ...
    update_information : dict
        Updates information.
    """
    current_version = f'v{assetguard.__version__}'
    headers = {
        ASSETGUARD_UID_KEY: installation_uid,
        ASSETGUARD_TAG_KEY: current_version,
        USER_AGENT_KEY: f'AssetGuard UpdateCheckService/{current_version}'
    }
