...
from assetguard.core.utils import AssetGuardDBQuery, AssetGuardDBBackend, get_date_from_timestamp
from assetguard.core.assetguard_socket import AssetGuardSocket
...
class AssetGuardDBQueryTask(AssetGuardDBQuery):
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, query: str = '', count: bool = True,
                 ...
                 filters: dict = None):
        """Create an instance of AssetGuardDBQueryTasks query.
        ...
        AssetGuardDBQuery.__init__(self, offset=offset, limit=limit, table=table, sort=sort, search=search, select=select,
                              ...
                              min_select_fields=min_select_fields, backend=AssetGuardDBBackend(query_format='task'))
...
def send_to_tasks_socket(command: dict) -> str:
    ...
    """
    try:
        s = AssetGuardSocket(common.TASKS_SOCKET)
    except Exception:
