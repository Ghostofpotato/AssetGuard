...
from assetguard.core.cluster.utils import read_cluster_config
from assetguard.core.exception import AssetGuardError
from assetguard.core.utils import AssetGuardVersion
...
class ARMessageBuilder:
    ...
    def choose_builder(cls, agent_version: str):
        ...
        raise AssetGuardError(1000, "No suitable message builder found for agent version: {}".format(agent_version))
...
class ARStrMessage(ARMessageBuilder):
    ...
    def can_handle(agent_version: str) -> bool:
        ...
        """
        return AssetGuardVersion(agent_version) < AssetGuardVersion(common.AR_LEGACY_VERSION)
...
class ARJsonMessage(ARMessageBuilder):
    ...
    def create_message(self, command: str = '', arguments: list = None, alert: dict = None) -> str:
        ...
        msg_queue = json.dumps(
            create_assetguard_socket_message(origin={'name': node_name, 'module': common.origin_module.get()},
                                        command=command,
...
def send_ar_message(agent_id: str = '', agent_version = '', wq: AssetGuardQueue = None, command: str = '',
    ...
    wq.send_msg_to_agent(msg=msg_queue, agent_id=agent_id, msg_type=AssetGuardQueue.AR_TYPE)
