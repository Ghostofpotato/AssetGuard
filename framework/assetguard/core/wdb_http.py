...
class AssetGuardDBHTTPClient:
    ...
    def __init__(self, retries: int = 5, timeout: float = 10):
        ...
        try:
            ...
            raise AssetGuardInternalError(2011, e)

    async def close(self) -> None:
        """Close the assetguard-manager-db HTTP client."""
    ...
    async def _get(self, endpoint: str) -> Any:
        ...
        try:
            ...
            if response.is_error:
                raise AssetGuardError(2012, extra_message=response.text)

        except RequestError as exc:
            raise AssetGuardError(2013, extra_message=str(exc))
...
@asynccontextmanager
async def get_wdb_http_client() -> AsyncIterator[AssetGuardDBHTTPClient]:
    """Create and return the engine client.

    Returns
    -------
    AsyncIterator[AssetGuardDBHTTP]
        AssetGuard DB HTTP client iterator.
    """
    ...
    try:
        ...
        raise AssetGuardInternalError(2014)
    except UnsupportedProtocol:
        raise AssetGuardInternalError(2015)
    except ConnectError:
        raise AssetGuardInternalError(2016)
    finally:
        await client.close()
