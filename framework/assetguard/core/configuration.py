...
from assetguard.core import common, assetguard_socket
from assetguard.core.exception import AssetGuardError, AssetGuardInternalError, AssetGuardResourceNotFound
from assetguard.core.utils import cut_array, load_assetguard_xml, safe_move
...
DEFAULT_CTI_URL = 'https://cti.assetguard.com'
...
def _replace_custom_values(opt_value: Union[list, dict, str]) -> Union[list, dict, str]:
    """Replace custom values introduced by 'load_assetguard_xml' with their real values.
...
def _ossecconf2json(xml_conf: str) -> dict:
    ...
    -------
    dict
        Final JSON with the assetguard-manager.conf content.
    """
    final_json = {}

    for root in list(xml_conf):
        if root.tag.lower() == "assetguard_config":
...
def _rcl2json(filepath: str) -> dict:
    ...
    try:
        ...
        raise AssetGuardError(1101, str(e))
...
def get_ossec_conf(section: str = None, field: str = None, conf_file: str = common.OSSEC_CONF,
    ...
    AssetGuardError(1101)
        Requested component does not exist.
    AssetGuardError(1102)
        Invalid section.
    AssetGuardError(1103)
        Invalid field in section.
    AssetGuardError(1106)
        Requested section not present in configuration.

    Returns
    -------
    dict
        assetguard-manager.conf (manager) as dictionary.
    """
    try:
        # Read XML
        ...
        if not from_import:
            ...
            print(f"assetguard-manager-apid: There is an error in the assetguard-manager.conf file: {str(e)}")
            sys.exit(0)
    ...
    if section:
        try:
            ...
            if section not in CONF_SECTIONS.keys():
                raise AssetGuardError(1102, extra_message=e.args[0])
            else:
                raise AssetGuardError(1106, extra_message=e.args[0])
...
def get_agent_conf(group_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if not os_path.exists(agent_conf):
        raise AssetGuardError(1006, agent_conf)
    ...
    try:
        ...
        # Parse XML to JSON
            ...
            # Read XML
            xml_data = load_assetguard_xml(agent_conf)
...
def get_agent_conf_multigroup(multigroup_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if multigroup_id and not os_path.exists(os_path.join(common.MULTI_GROUPS_PATH, multigroup_id)) or not multigroup_id:
        raise AssetGuardResourceNotFound(1710, extra_message=multigroup_id if multigroup_id else "No multigroup provided")
    ...
    if not os_path.exists(agent_conf):
        raise AssetGuardError(1006, extra_message=os_path.join("ASSETGUARD_PATH", "var", "multigroups", agent_conf))
...
def get_file_conf(filename: str, group_id: str = None, type_conf: str = None, raw: bool = False) -> dict | str:
    ...
    if not os_path.exists(file_path):
        raise AssetGuardError(1006, file_path)
    ...
    if type_conf:
        if type_conf in types:
            ...
            raise AssetGuardError(1104, f'{type_conf}. Valid types: {types.keys()}')
    else:
...
def parse_internal_options(high_name: str, low_name: str) -> str:
    ...
    Raises
    ------
    AssetGuardInternalError(1107)
        Internal options file not found.
    AssetGuardInternalError(1108)
    ...
    if not os_path.exists(common.INTERNAL_OPTIONS_CONF):
        raise AssetGuardInternalError(1107)
    ...
    try:
        ...
        raise AssetGuardInternalError(1108, e.args[0])
...
def get_internal_options_value(high_name: str, low_name: str, max_: int, min_: int) -> int:
    ...
    Raises
    ------
    AssetGuardError(1109)
        Option must be a digit.
    AssetGuardError(1110)
    ...
    if not option.isdigit():
        raise AssetGuardError(1109, f'Option: {high_name}.{low_name}. Value: {option}')

    option = int(option)
    if option < min_ or option > max_:
        raise AssetGuardError(1110, f'Max value: {max_}. Min value: {min_}. Found: {option}.')
...
def upload_group_configuration(group_id: str, file_content: str) -> str:
    ...
    AssetGuardError(1113)
        XML syntax error.
    AssetGuardError(1114)
        AssetGuard syntax error.
    AssetGuardError(1115)
        Error executing verify-agent-conf.
    AssetGuardInternalError(1743)
        Error running AssetGuard syntax validator.
    ...
    try:
        # check AssetGuard xml format
        try:
            subprocess.check_output([os_path.join(common.ASSETGUARD_PATH, "bin", "verify-agent-conf"), '-f', tmp_file_path],
            ...
            # Syscheck remote configuration in '/var/assetguard-manager/tmp/api_tmp_file_2019-01-08-01-1546959069.xml' is corrupted.
            # \n\n
            ...
            if output_regex:
                raise AssetGuardError(1114, ' '.join(output_regex))
            else:
                raise AssetGuardError(1115, e.output.decode())
        except Exception as e:
            raise AssetGuardInternalError(1743, str(e))

        # move temporary file to group folder
        try:
            new_conf_path = os_path.join(common.SHARED_PATH, group_id, "agent.conf")
            safe_move(tmp_file_path, new_conf_path, ownership=(common.assetguard_uid(), common.assetguard_gid()),
                      permissions=0o660)
        except Exception as e:
            raise AssetGuardInternalError(1016, extra_message=str(e))
...
def upload_group_file(group_id: str, file_data: str, file_name: str = 'agent.conf') -> str:
    ...
    AssetGuardResourceNotFound(1710)
        Group was not found.
    AssetGuardError(1112)
        Empty files are not supported.
    AssetGuardError(1111)
    ...
    if file_name == 'agent.conf':
        if len(file_data) == 0:
            raise AssetGuardError(1112)

        return upload_group_configuration(group_id, file_data)
    else:
        raise AssetGuardError(1111)
...
def get_active_configuration(component: str, configuration: str, agent_id: Optional[str] = None) -> dict:
    ...
    component_socket_mapping = {'agent': 'analysis', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'assetguard-manager-db': 'wdb', 'wmodules': 'wmodules'}
    component_socket_dir_mapping = {'agent': 'sockets', 'analysis': 'sockets',
                                    ...
                                    'request': 'sockets', 'syscheck': 'sockets', 'assetguard-manager-db': 'db',
                                    'wmodules': 'sockets'}

    if not component or not configuration:
        raise AssetGuardError(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise AssetGuardError(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = os_path.join(common.ASSETGUARD_PATH, "queue", component_socket_dir_mapping[component],
                                   component_socket_mapping[component])

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise AssetGuardError(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            # Socket connection
            try:
                s = assetguard_socket.AssetGuardSocket(dest_socket)
            except AssetGuardInternalError:
                raise
            except Exception as unhandled_exc:
                raise AssetGuardInternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise AssetGuardInternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
        ...
        # JSON socket message
            ...
            msg = assetguard_socket.create_assetguard_socket_message(origin={'module': common.origin_module.get()},
                                                           command=GETCONFIG_COMMAND,
                                                           parameters={'section': configuration})

            # Socket connection
            try:
                s = assetguard_socket.AssetGuardSocketJSON(dest_socket)
            except AssetGuardInternalError:
                raise
            except Exception as unhandled_exc:
                raise AssetGuardInternalError(1121, extra_message=str(unhandled_exc))
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.ASSETGUARD_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise AssetGuardError(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,
