...
import assetguard.core.exception as wexception
...
class AbstractAssetGuardResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.AssetGuardInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractAssetGuardResult):
            self.dikt = dct.dikt
        else:
            raise wexception.AssetGuardInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractAssetGuardResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractAssetGuardResult objects. When merged with a AssetGuardException, the result is
        always a AssetGuardException

        Parameters
        ----------
        other : AbstractAssetGuardResult or wexception.AssetGuardException

        Raises
        ------
        AssetGuardInternalError(1000)
            AssetGuard internal error when two object could not be merged.

        Returns
        -------
        AbstractAssetGuardResult or wexception.AssetGuardException
            Resultant object.
        """
        if isinstance(other, wexception.AssetGuardException):
            return other
        elif not isinstance(other, (dict, AbstractAssetGuardResult)):
            raise wexception.AssetGuardInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractAssetGuardResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in AssetGuardResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in assetguard.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractAssetGuardResult
            Filtered AbstractAssetGuardResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in AssetGuardResult subclasses.
        ...
        -------
        AbstractAssetGuardResult
            Sorted AbstractAssetGuardResult
        """
...
class AssetGuardResult(AbstractAssetGuardResult):
    """
...
class AffectedItemsAssetGuardResult(AbstractAssetGuardResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.AssetGuardException = None):
        ...
        error : wexception.AssetGuardException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsAssetGuardResult
        ...
        wexception.AssetGuardInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsAssetGuardResult):
            raise wexception.AssetGuardInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsAssetGuardResult with self.

        Parameters
        ----------
        other : AffectedItemsAssetGuardResult
            AffectedItemsAssetGuardResult object to merge.
        ...
        if isinstance(other, wexception.AssetGuardError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.AssetGuardException):
            return result
        elif not isinstance(other, AffectedItemsAssetGuardResult):
            raise wexception.AssetGuardInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsAssetGuardResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractAssetGuardResult
            Instance of AbstractAssetGuardResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsAssetGuardResult object.

        Returns
        -------
        dict
            Return AffectedItemsAssetGuardResult as a dictionary with fields data, message and error.
        """
