...
from assetguard.core.common import origin_module
...
def create_assetguard_queue_socket_msg(flag: str, str_agent_id: str, msg: str, is_restart: bool = False) -> str:
    """Create message that will be sent to the AssetGuardQueue socket.
    ...
    -------
    str
        Message that will be sent to the AssetGuardQueue socket.
    """
...
class BaseQueue:
    ...
    def _connect(self):
        try:
            ...
            if length_send_buffer < AssetGuardQueue.MAX_MSG_SIZE:
                self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, AssetGuardQueue.MAX_MSG_SIZE)
        except Exception:
            raise AssetGuardInternalError(1010, self.path)
    ...
    def _send(self, msg: bytes) -> None:
        ...
        try:
            ...
            if sent == 0:
                raise AssetGuardInternalError(1011, self.path)
        except socket.error:
            raise AssetGuardInternalError(1011, self.path)
...
class AssetGuardQueue(BaseQueue):
    """
    AssetGuardQueue Object.
    """
    ...
    def send_msg_to_agent(self, msg: str = '', agent_id: str = '', msg_type: str = '') -> str:
        ...
        Raises
        ------
        AssetGuardInternalError(1012)
            If the message was invalid to queue.
        AssetGuardError(1014)
        ...
        msg_is_no_ar = msg in [AssetGuardQueue.HC_SK_RESTART, AssetGuardQueue.HC_FORCE_RECONNECT]
        msg_is_restart = msg in [AssetGuardQueue.RESTART_AGENTS, AssetGuardQueue.RESTART_AGENTS_JSON]
        ...
        # AR
        if msg_type == AssetGuardQueue.AR_TYPE:
            socket_msg = create_assetguard_queue_socket_msg(flag, str_agent_id, msg)
            # Return message
        ...
        # Restart agents
            ...
            # If msg is not a non active-response command and not a restart command, raises AssetGuardInternalError
            if not msg_is_no_ar and not msg_is_restart:
                raise AssetGuardInternalError(1012, msg)
            socket_msg = create_assetguard_queue_socket_msg(flag, str_agent_id, msg, is_restart=msg_is_restart)
            # Return message
            if msg == AssetGuardQueue.HC_SK_RESTART:
                ret_msg = "Restarting Syscheck on agent" if agent_id else "Restarting Syscheck on all agents"
            elif msg == AssetGuardQueue.HC_FORCE_RECONNECT:
                ret_msg = "Reconnecting agent" if agent_id else "Reconnecting all agents"
            else:  # msg == AssetGuardQueue.RESTART_AGENTS or msg == AssetGuardQueue.RESTART_AGENTS_JSON
        ...
        try:
            ...
            raise AssetGuardError(1014, extra_message=f": AssetGuardQueue socket with path {self.path}")

        return ret_msg


class AssetGuardAnalysisdQueue(BaseQueue):
    """
    AssetGuardAnalysisdQueue Object.
    """
    ...
    def send_msg(self, msg_header: str, msg: str):
        ...
        try:
            ...
            raise AssetGuardError(1014, extra_message=f': AssetGuardAnalysisdQueue socket with path {self.path}. {str(e)}')
