...
class Indexer:
    """
    Interface to connect with AssetGuard Indexer.

    This class handles the asynchronous connection to the AssetGuard Indexer
    (OpenSearch) nodes, managing authentication and SSL configuration.

    Parameters
    ----------
    hosts : List[str]
        List of hostnames or IP addresses of the AssetGuard Indexer nodes.
    ports : List[int]
    ...
    async def connect(self) -> None:
        """
        Establish a connection to the AssetGuard Indexer and verify its status.
        ...
        try:
            ...
            raise AssetGuardIndexerError(2200, extra_message=e.error)
        except ssl.SSLError as e:
            raise AssetGuardIndexerError(2200, extra_message=e.reason)
    ...
    async def close(self) -> None:
        """
        Close the AssetGuard Indexer client session asynchronously.
        """
        getLogger("assetguard").debug("Closing the indexer client session.")
    ...
    **kwargs : dict
    ...
    for attempt in range(retries + 1):
        try:
            ...
            return indexer
        except AssetGuardIndexerError as e:
            if attempt == retries:
                await indexer.close()
                raise e
...
async def get_indexer_client() -> AsyncIterator[Indexer]:
    ...
    try:
        ...
        raise AssetGuardException(
            code=1003, message=f"Failed to parse AssetGuard configuration: {e}"
        )

    indexer_section = assetguard_config.get("indexer", {})
    ...
    if not ssl_config:
        raise AssetGuardException(code=1005, message="Missing SSL configuration")
    ...
    try:
        ...
        raise AssetGuardException(code=1013, message=f"Failed to parse host URLs: {e}")
    ...
    try:
        ...
        raise AssetGuardException(code=1015, message=f"Failed to create indexer client: {e}")

    try:
        yield client
    except Exception as e:
        getLogger("assetguard").logger.error(f"Error in indexer client context: {e}")
        raise
    finally:
        try:
            await client.close()
        except Exception as e:
            getLogger("assetguard").logger.warning(
