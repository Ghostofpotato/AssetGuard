...
class TestUpdateAgentClusterName:
    ...
    async def test_script_contains_cluster_name_field(
        ...
        """Verify script updates assetguard.cluster.name field."""
        mock_indexer_client.update_by_query.return_value = {"updated": 1}
        ...
        assert "assetguard.cluster.name" in script_source
        assert "ctx._source.assetguard.cluster.name" in script_source
...
class TestUpdateAgentClusterNameScriptContent:
    ...
    @pytest.mark.asyncio
    async def test_script_initializes_assetguard_structure(
        self, max_version_index, mock_indexer_client
    ):
        """Verify script safely initializes assetguard structure."""
        mock_indexer_client.update_by_query.return_value = {"updated": 1}
        ...
        # Should initialize assetguard if null
        assert "ctx._source.assetguard == null" in script_source or "assetguard" in script_source
