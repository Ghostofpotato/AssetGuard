...
from assetguard.core.indexer.base import IndexerKey
...
class MaxVersionIndex:
    ...
    STATE_INDEX_PATTERN = "assetguard-states-*"
    MODULE_INDICES_MAP = {
        "fim": [
            "assetguard-states-fim-files",
            "assetguard-states-fim-registry-keys",
            "assetguard-states-fim-registry-values",
        ],
        "sca": [
    ...
    async def update_agent_groups(
        ...
        body = {
            "query": {
                "bool": {
                    "must": [
                        {"term": {"assetguard.agent.id": agent_id}}
                    ],
                    "filter": [
                        {
                            "bool": {
            ...
            "script": {
                "lang": "painless",
                "source": """
                    if (ctx._source.assetguard == null) { ctx._source.assetguard = [:]; }
                    if (ctx._source.assetguard.agent == null) { ctx._source.assetguard.agent = [:]; }

                    if (ctx._source.assetguard.agent.groups != null && ctx._source.assetguard.agent.groups == params.groups) {
                        ctx.op = "noop";
                        return;
                    }
        ...
        for module, indices in self.MODULE_INDICES_MAP.items():
            for index in indices:
                try:
                    ...
                    logging.getLogger("assetguard").debug(
                        f"[{module}] Updated agent_id={agent_id} " f"index={index}"
                    )

                except Exception as e:
                    logging.getLogger("assetguard").error(
                        f"[{module}] Failed updating index={index} "
                        f"agent_id={agent_id}: {e}"
                    )
                    results[index] = {"error": str(e)}
            logging.getLogger("assetguard").info(f"Summary: {summary}")
            summary["index"] = []
    ...
    async def update_agent_cluster_name(
        ...
        agent_id : str
            Agent identifier to match documents.
        cluster_name : str
            Cluster name to set on the agent document (``assetguard.cluster.name``).
        global_version : int
        ...
        ``assetguard.cluster.name`` field. Documents with the same cluster name are
        ...
        body = {
            ...
            "script": {
                ...
                "source": """
                    if (ctx._source.assetguard == null) { ctx._source.assetguard = [:]; }
                    if (ctx._source.assetguard.agent == null) { ctx._source.assetguard.agent = [:]; }
                    if (ctx._source.assetguard.cluster == null) { ctx._source.assetguard.cluster = [:]; }

                    if (ctx._source.assetguard.cluster.name == params.clusterName) {
                        ctx.op = "noop";
                        return;
                    }
                    ctx._source.assetguard.cluster.name = params.clusterName;
                    if (ctx._source.state == null) { ctx._source.state = [:]; }
