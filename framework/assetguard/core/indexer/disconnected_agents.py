...
from assetguard.core.exception import (
    ...
    AssetGuardResourceNotFound,
)
from assetguard.core.indexer.indexer import get_indexer_client
...
class DisconnectedAgentSyncTasks:
    ...
    minimum duration, queries the AssetGuard Indexer to obtain the maximum
    ...
    def __init__(
        ...
        # Use from_import=True to avoid raising during tests when assetguard configuration file
    ...
    async def _get_disconnected_agents_filter_by_time(self) -> List[dict]:
        ...
        This method queries the AssetGuard database for agents whose status is not
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved {len(agents)} non-active agents from AssetGuardDB "
            ...
            self.logger.error(
                f"Error retrieving non-connected agents from AssetGuardDB: {e}",
                exc_info=True,
            )
            return []
    ...
    async def _get_max_versions_batch_from_indexer(self, agent_ids: List[str]) -> dict:
        """
        Query the AssetGuard Indexer for maximum document versions of multiple agents.
        ...
        try:
            ...
            if result:
                ...
                raise Exception("Failed query to assetguard-indexer")
    ...
    async def disconnected_agent_group_sync(
        ...
        for agent_id in agent_list:
            if agent_id not in system_agents:
                result.add_failed_item(id_=agent_id, error=AssetGuardResourceNotFound(1701))
            ...
            for agent_id in valid_agents:
                try:
                    ...
                    result.add_failed_item(
                        id_=agent_id,
                        error=AssetGuardInternalError(1000, extra_message=str(e)),
                    )
    ...
    async def _get_disconnected_agents(self) -> List[str]:
        """
        Retrieve non-connected agents from AssetGuardDB.

        Queries the AssetGuardDB for agents whose status is not `active` and
    ...
    async def _get_cluster_name_from_indexer(
        ...
        Only agents that have an associated assetguard.cluster.name will be returned.
        ...
        query = {
            ...
            "query": {
                "bool": {
                    "filter": [
                        {"terms": {"assetguard.agent.id": agent_ids}},
                        {"exists": {"field": "assetguard.cluster.name"}},
                    ],
                    "must_not": [
                        {"term": {"assetguard.cluster.name.keyword": cluster_name}}
                    ],
                }
            },
            "aggs": {
                "by_agent": {
                    "terms": {"field": "assetguard.agent.id", "size": len(agent_ids)},
                    "aggs": {
                        "cluster_name": {
                            "terms": {"field": "assetguard.cluster.name", "size": 5}
                        }
                    },
                }
            },
