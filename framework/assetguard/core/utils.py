...
import assetguard.core.results as results
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.ASSETGUARD_PATH}/var/run...")
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a AssetGuard framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    Raises
    ------
    AssetGuardError(1400)
        Invalid offset.
    AssetGuardError(1401)
        Invalid limit.
    AssetGuardError(1405)
        Limit exceeding the maximum permitted.
    AssetGuardError(1406)
        Invalid limit (0).
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise AssetGuardError(1405, extra_message=str(limit))
        elif limit == 0:
            raise AssetGuardError(1406)
    ...
    if offset < 0:
        raise AssetGuardError(1400)
    elif limit < 1:
        raise AssetGuardError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    Raises
    ------
    AssetGuardError(1403)
        Not a valid sort field.
    AssetGuardError(1402)
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise AssetGuardError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'assetguard' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise AssetGuardError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise AssetGuardError(1724, "{}".format(', '.join(select)))
...
def delete_assetguard_file(full_path: str) -> bool:
    """Delete a AssetGuard file.
    ...
    Raises
    ------
    AssetGuardError(1906)
        File does not exist.
    AssetGuardError(1907)
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.ASSETGUARD_PATH) or '..' in full_path:
        raise AssetGuardError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise AssetGuardError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_remote_commands(new_conf: Element, original_conf: Element):
    ...
    LOCALFILE_HIERARCHY = ['assetguard_config', 'localfile']
    WODLE_HIERARCHY = ['assetguard_config', 'wodle']
    ...
    if not LOCALFILE_SETTINGS[ALLOW_KEY]:
        ...
        if normalize(_filter_remote_commands(new_localfile, LOCALFILE_SETTINGS[EXCEPTIONS_KEY])) \
            ...
            raise AssetGuardError(1124, extra_message="localfile")
    ...
    if not WODLE_SETTINGS[ALLOW_KEY]:
        ...
        if normalize(_filter_wodle_commands(new_wodle, WODLE_SETTINGS[EXCEPTIONS_KEY])) \
            ...
            raise AssetGuardError(1124, extra_message="wodle")
...
def xml_to_dict(root, section_path: list):
    ...
    ----------
    root : Element
        Root element containing one or more assetguard_config nodes.
    section_path : list
...
def check_assetguard_limits_unchanged(new_conf, original_conf):
    """Check if AssetGuard limits remain unchanged.
    ...
    """
    CONFIG_LIMITS_HIERARCHY = ['assetguard_config', 'global', 'limits']
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if normalize(new_limits) != normalize(original_limits):
            raise AssetGuardError(1127, extra_message=f"global > limits > {disabled_limit}")
...
def check_agents_allow_higher_versions(new_conf: Element, original_conf: Element):
    ...
    Raises
    ------
    AssetGuardError(1129)
        Raised if the agents allow_higher_versions setting is modified in the configuration to upload.
    """

    AUTH_HIERARCHY = ['assetguard_config', 'auth', 'allow_higher_versions']
    REMOTE_HIERARCHY = ['assetguard_config', 'remote', 'allow_higher_versions']
    ...
    if not upload_configuration['agents']['allow_higher_versions']['allow']:
        ...
        if normalize(new_auth) != normalize(original_auth):
            raise AssetGuardError(1129, extra_message='auth > allow_higher_versions')
        ...
        if normalize(new_remote) != normalize(original_remote):
            raise AssetGuardError(1129, extra_message='remote > allow_higher_versions')
...
def check_indexer(new_conf, original_conf):
    ...
    CONFIG_INDEXER_HIERARCHY = ['assetguard_config', 'indexer']
    upload_configuration = configuration.api_conf['upload_configuration']
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if normalize(new_indexer) != normalize(original_indexer):
            raise AssetGuardError(1127, extra_message='indexer')


def load_assetguard_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise AssetGuardError(1113, extra_message=str(e))
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise AssetGuardError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise AssetGuardError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class AssetGuardDBBackend(AbstractDatabaseBackend):
    """
    This class describes a assetguard db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise AssetGuardError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return AssetGuardDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through AssetGuardDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class AssetGuardDBQuery(object):
    """This class describes a database query for assetguard."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        """AssetGuard DB Query constructor.
        ...
        # For example, the following regex -> (name!=assetguard;id>5),group=webserver <- would return 3 different matches:
        #   (name != assetguard ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, AssetGuardDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise AssetGuardError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise AssetGuardError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        ...
        if select_fields:
            ...
            if not select_fields_set or not select_fields_set.issubset(allowed_select_fields):
                raise AssetGuardError(1724, "Allowed select fields: {0}. Fields: {1}". \
    ...
    def _parse_query(self):
        ...
        if not self.query_regex.match(self.q):
            raise AssetGuardError(1407, self.q)
        ...
        for open_level, field, operator, value, close_level, separator in self.query_regex.findall(self.q):
            if field not in allowed_query_fields:
                raise AssetGuardError(1408, "Available fields: {}. Field: {}".format(
                    ', '.join(sorted(allowed_query_fields)), field)
                )
            if operator not in self.query_operators:
                raise AssetGuardError(1409,
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise AssetGuardError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'AssetGuardDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'AssetGuardDBQueryGroups':
            resource = 'name'
        else:
            raise AssetGuardInternalError(1123)
...
class AssetGuardDBQueryDistinct(AssetGuardDBQuery):
    ...
    def _add_filters_to_query(self):
        AssetGuardDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise AssetGuardError(1410)

        AssetGuardDBQuery._add_select_to_query(self)
...
def validate_assetguard_xml(content: str):
    """Validate AssetGuard XML files (assetguard-manager.conf)
    ...
    try:
        ...
        check_assetguard_limits_unchanged(incoming_xml, current_xml)

    except ExpatError:
        raise AssetGuardError(1113)
    except AssetGuardError as e:
        raise e
    except Exception as e:
        raise AssetGuardError(1113, str(e))


def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    """Upload files (assetguard-manager.conf).
    ...
    Raises
    ------
    AssetGuardInternalError(1005)
        Error reading file.
    AssetGuardInternalError(1016)
        Error moving file.
    AssetGuardError(1006)
    ...
    try:
        ...
        raise AssetGuardInternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.ASSETGUARD_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.assetguard_uid(), common.assetguard_gid()), permissions=0o660)
    except PermissionError as exc:
        raise AssetGuardError(1006) from exc
    except Error as exc:
        raise AssetGuardInternalError(1016) from exc

    return results.AssetGuardResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.ASSETGUARD_PATH) -> str:
    """Return a relative path from the AssetGuard base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.ASSETGUARD_PATH`
...
def get_localtime() -> tzinfo:
    ...
    -------
    timezone : tzinfo
        The localtime timezone. AssetGuard custom localtime has precedence over the
        system one. If none is found, UTC is returned.   """
    if (tz := gettz(common.ASSETGUARD_LOCALTIME_PATH)) is not None:
        return tz
