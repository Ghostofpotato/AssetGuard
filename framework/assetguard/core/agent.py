...
from assetguard.core import common, configuration, stats
from assetguard.core.InputValidator import InputValidator
from assetguard.core.cluster.utils import get_manager_status
from assetguard.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from assetguard.core.exception import AssetGuardException, AssetGuardError, AssetGuardInternalError, AssetGuardResourceNotFound
from assetguard.core.utils import AssetGuardVersion, plain_dict_to_nested_dict, get_fields_to_nest, AssetGuardDBQuery, \
    AssetGuardDBQueryDistinct, AssetGuardDBQueryGroupBy, AssetGuardDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from assetguard.core.assetguard_queue import AssetGuardQueue
from assetguard.core.assetguard_socket import AssetGuardSocket, AssetGuardSocketJSON, create_assetguard_socket_message
from assetguard.core.wdb import AssetGuardDBConnection
from assetguard.core.wdb_http import get_wdb_http_client
from assetguard.rbac.utils import resource_cache
...
class AssetGuardDBQueryAgents(AssetGuardDBQuery):
    """Class used to query AssetGuard agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_assetguard_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = AssetGuardDBBackend(query_format='global')
        AssetGuardDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _normalize_retrieved_versions(self, query_results: dict):
        ...
        for item in query_results['items']:
            if 'version' in item:
                item['version'] = str(AssetGuardVersion(item['version'], treat_as_empty=[None, 'N/A']))

    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the AssetGuard query."""
        AssetGuardDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return AssetGuardDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the AssetGuard query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            AssetGuardDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise AssetGuardError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            AssetGuardDBQuery._process_filter(self, field_name, field_filter, q_filter)


class AssetGuardDBQueryGroup(AssetGuardDBQuery):
    """Class used to query AssetGuard groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = AssetGuardDBBackend(query_format='global')
        AssetGuardDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class AssetGuardDBQueryDistinctAgents(AssetGuardDBQueryDistinct, AssetGuardDBQueryAgents):
    pass


class AssetGuardDBQueryGroupByAgents(AssetGuardDBQueryGroupBy, AssetGuardDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        AssetGuardDBQueryAgents.__init__(self, *args, **kwargs)
        AssetGuardDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return AssetGuardDBQuery._format_data_into_dictionary(self)


class AssetGuardDBQueryMultigroups(AssetGuardDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        AssetGuardDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        AssetGuardResourceNotFound(1701)
            Agent does not exist.
        """
        with AssetGuardDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
                                query="id={}".format(self.id), count=False, get_data=True,
                                remove_extra_fields=False) as db_query:
            try:
                data = db_query.run()['items'][0]
            except IndexError:
                raise AssetGuardResourceNotFound(1701)
    ...
    def reconnect(self, wq: AssetGuardQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : AssetGuardQueue
            AssetGuardQueue used for the active response message.

        Raises
        ------
        AssetGuardError(1707)
            If the agent to be reconnected is not active.

        Returns
        -------
        str
            Message generated by AssetGuard.
        """
        # Check if agent is active
        self.get_basic_information()
        if self.status.lower() != 'active':
            raise AssetGuardError(1707)

        # Send force reconnect message to the AssetGuardQueue
        ret_msg = wq.send_msg_to_agent(AssetGuardQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        Raises
        ------
        AssetGuardError(1726)
            Authd is not running.
        AssetGuardInternalError(1757)
            Unhandled exception.
        ...
        str
            Message generated by AssetGuard.
        """
        # Check that assetguard-manager-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except AssetGuardInternalError as e:
            # assetguard-manager-authd is not running due to a problem with /proc availability
            raise AssetGuardError(1726, extra_message=str(e))

        if manager_status.get('assetguard-manager-authd') != 'running':
            # assetguard-manager-authd is not running
            raise AssetGuardError(1726)
        ...
        try:
            ...
            raise AssetGuardInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = AssetGuardSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise AssetGuardError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise AssetGuardInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to AssetGuard using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise AssetGuardError(1705, extra_message=name)
            elif e.code == 9007:
                raise AssetGuardError(1706, extra_message=ip)
            elif e.code == 9012:
                raise AssetGuardError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = AssetGuardDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = AssetGuardDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    async def add_group_to_agent(group_id: str, agent_id: str, replace: bool = False, replace_list: list = None) -> str:
        ...
        Raises
        ------
        AssetGuardInternalError(2007)
            Body is empty.
        AssetGuardError(1752)
            Could not force single group for the agent.
        AssetGuardError(1737)
    ...
    async def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        Raises
        ------
        AssetGuardResourceNotFound(1710)
            The group was not found.
        AssetGuardError(1734)
            Error removing agent from group.
        AssetGuardError(1745)
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise AssetGuardResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise AssetGuardError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise AssetGuardError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or AssetGuard vX.Y.Z.

        Raises
        ------
        AssetGuardError(1735)
        ...
        """
        if AssetGuardVersion(agent_version) < AssetGuardVersion(common.ACTIVE_CONFIG_VERSION):
            raise AssetGuardInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        AssetGuardInternalError(1015)
            Agent version is null.
        AssetGuardInternalError(1735)
        ...
        if self.version is None:
            raise AssetGuardInternalError(1015)
        agent_version = AssetGuardVersion(self.version)
        required_version = AssetGuardVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise AssetGuardInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_assetguard_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_assetguard_version_format(filters: dict):
    """Verify and format the specified assetguard version into the 'assetguard vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"assetguard {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^assetguard \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: AssetGuardQueue = None) -> str:
    ...
    wq : AssetGuardQueue
        AssetGuardQueue used for the active response messages.
    ...
    str
        Message generated by AssetGuard.
    """
    # If the AssetGuard agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if AssetGuardVersion(agent_version) >= AssetGuardVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(AssetGuardQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(AssetGuardQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_assetguard_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_assetguard_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = AssetGuardSocket(common.UPGRADE_SOCKET)
