...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        from assetguard.core.results import AssetGuardResult, AffectedItemsAssetGuardResult, _goes_before_than, nested_itemgetter, merge
        from assetguard import AssetGuardException, AssetGuardError

param_name = ['affected_items', 'total_affected_items', 'sort_fields', 'sort_casting', 'sort_ascending',
              'all_msg', 'some_msg', 'none_msg']
ASSETGUARD_EXCEPTION_CODE = 1725
FAILED_AGENT_ID = '999'


@pytest.fixture(scope='function')
def get_assetguard_result():
    return AssetGuardResult(dct={"data": {"items": [{"item1": "data1"}, {"item2": "OK"}], "message": "Everything ok"}},
                       str_priority=['KO', 'OK'])


@pytest.fixture(scope='function')
def get_assetguard_affected_item():
    def _get_affected(params=None):
        kwargs = {p_name: param for p_name, param in zip(param_name, params)}
        return AffectedItemsAssetGuardResult(**kwargs)

    return _get_affected


@pytest.fixture(scope='function')
def get_assetguard_failed_item():
    item = AffectedItemsAssetGuardResult()
    item.add_failed_item(id_=FAILED_AGENT_ID, error=AssetGuardException(ASSETGUARD_EXCEPTION_CODE))
    return item
...
def test_results_AssetGuardResult__merge_str(dikt, priority, get_assetguard_affected_item):
    """Test method `_merge_str` from `AssetGuardResult`.
        ...
        priority : list
            Used to set the AssetGuardResult priority.
        """
    assetguard_result = AssetGuardResult(deepcopy(dikt), str_priority=priority)
    assert isinstance(assetguard_result, AssetGuardResult)
    item2 = assetguard_result.dikt['data']['items'][1]['item2']
    merge_result = assetguard_result._merge_str(item2, 'KO')
    assert merge_result == priority[0] if priority else '{}|{}'.format(item2, 'KO')


def test_results_AssetGuardResult_to_dict(get_assetguard_result):
    """Test method `to_dict` from `AssetGuardResult`."""
    dict_result = get_assetguard_result.to_dict()
    assert isinstance(dict_result, dict)
    assert (key == result_key for key, result_key in zip(['str_priority', 'result'], dict_result.keys()))


def test_results_AssetGuardResult_render(get_assetguard_result):
    """Test method `render` from `AssetGuardResult`."""
    render_result = get_assetguard_result.render()
    assert isinstance(render_result, dict)
    assert render_result == get_assetguard_result.dikt


def test_results_AssetGuardResult_decode_json(get_assetguard_result):
    """Test class method `decode_json` from `AssetGuardResult`."""
    assetguard_result = get_assetguard_result
    decoded_result = AssetGuardResult.decode_json(assetguard_result.to_dict())
    assert decoded_result == assetguard_result
...
def test_results_AffectedItemsAssetGuardResult(get_assetguard_affected_item, param_value):
    """Test class `AffectedItemsAssetGuardResult`.
    ...
    """
    affected_result = get_assetguard_affected_item(param_value)
    assert isinstance(affected_result, AffectedItemsAssetGuardResult)
    for value, dikt_value in zip(param_value, affected_result.dikt.values()):
        assert value == dikt_value


def test_results_AffectedItemsAssetGuardResult_add_failed_item():
    """Test method `add_failed_item` from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
    id_list = ['001', '002']
    # Add two failed items with different id but same exception
    for agent_id in id_list:
        affected_result.add_failed_item(id_=agent_id, error=AssetGuardException(ASSETGUARD_EXCEPTION_CODE))

    assert affected_result.failed_items
    assert set(id_list) == next(iter(affected_result.failed_items.values()))


def test_results_AffectedItemsAssetGuardResult_add_failed_items_from(get_assetguard_failed_item):
    """Test method `add_failed_items_from` from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
    failed_result = get_assetguard_failed_item
    affected_result.add_failed_items_from(failed_result)
    assert affected_result.failed_items == failed_result.failed_items


def test_results_AffectedItemsAssetGuardResult_add_failed_items_from_exception():
    """Test raised exception from method `add_failed_items_from` from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
    with pytest.raises(AssetGuardException, match='.* 1000 .*'):
        affected_result.add_failed_items_from('Invalid type')


def test_results_AffectedItemsAssetGuardResult_remove_failed_items(get_assetguard_failed_item):
    """Test method `remove_failed_items` from class `AffectedItemsAssetGuardResult`."""
    failed_result = get_assetguard_failed_item
    failed_result.remove_failed_items(code={ASSETGUARD_EXCEPTION_CODE})
    assert not failed_result.failed_items


def test_results_AffectedItemsAssetGuardResult___or__(get_assetguard_failed_item):
    """Test method `__or__` from class `AffectedItemsAssetGuardResult`."""
    agent_list_1 = ['001', '002']
    agent_list_2 = ['004', '003']
    affected_item_1 = AffectedItemsAssetGuardResult(affected_items=deepcopy(agent_list_1))
    affected_item_2 = AffectedItemsAssetGuardResult(affected_items=deepcopy(agent_list_2))
    failed_item = get_assetguard_failed_item
...
@pytest.mark.parametrize('or_item, expected_result', [
    (AssetGuardError(ASSETGUARD_EXCEPTION_CODE, ids=['001']), AffectedItemsAssetGuardResult),
    (AssetGuardError(ASSETGUARD_EXCEPTION_CODE), AssetGuardException),
    (AssetGuardException(ASSETGUARD_EXCEPTION_CODE), AssetGuardException),
    ({'Invalid type': None}, None)
])
def test_results_AffectedItemsAssetGuardResult___or___exceptions(or_item, expected_result):
    """Test raised exceptions from method `__or__` from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
...
def test_results_AffectedItemsAssetGuardResult_to_dict():
    """Test method `to_dict` from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
...
def test_results_AffectedItemsAssetGuardResult_properties():
    """Test getters and setters from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
...
def test_results_AffectedItemsAssetGuardResult_failed_items_properties(get_assetguard_failed_item):
    """Test `failed_items` properties from class `AffectedItemsAssetGuardResult`."""
    fail_items = get_assetguard_failed_item.failed_items
    total_fail_items = get_assetguard_failed_item.total_failed_items
    assert isinstance(fail_items, dict)
    assert total_fail_items == 1


def test_results_AffectedItemsAssetGuardResult_message_property():
    """Test `message` property from class `AffectedItemsAssetGuardResult`."""
    ...
    none_msg_result = AffectedItemsAssetGuardResult(**messages).message
    all_msg_result = AffectedItemsAssetGuardResult(**messages, affected_items=['001']).message
    aux_result = AffectedItemsAssetGuardResult(**messages, affected_items=['001'])
    aux_result.add_failed_item(AssetGuardException(ASSETGUARD_EXCEPTION_CODE))
...
def test_results_AffectedItemsAssetGuardResult__merge_str(self_field, other_field, key, expected_result):
    """Test method `_merge_str` from class `AffectedItemsAssetGuardResult`."""
    affected_result = AffectedItemsAssetGuardResult()
    merge_result = affected_result._merge_str(self_field, other_field, key=key)
    assert merge_result == expected_result


def test_results_AffectedItemsAssetGuardResult_encode_decode_json(get_assetguard_affected_item):
    """Test methods `encode_json` and `decode_json` from class `AffectedItemsAssetGuardResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_assetguard_affected_item(param_list)
    affected_result.add_failed_item(id_=FAILED_AGENT_ID, error=AssetGuardException(ASSETGUARD_EXCEPTION_CODE))
    # Use a complete AffectedIemsAssetGuardResult to encode a json and then decode it
    json_item = affected_result.encode_json()
    decoded_json = AffectedItemsAssetGuardResult.decode_json(json_item)
    assert affected_result == decoded_json


def test_results_AffectedItemsAssetGuardResult_render(get_assetguard_affected_item):
    """Test method `render` from class `AffectedItemsAssetGuardResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_assetguard_affected_item(param_list)
    for agent_id in [FAILED_AGENT_ID, 'Invalid ID']:
        affected_result.add_failed_item(id_=agent_id, error=AssetGuardException(ASSETGUARD_EXCEPTION_CODE))
    # Render a valid AffectedItemsAssetGuardResult and check it has all the expected fields
