...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        from assetguard import AssetGuardException
        from assetguard.core.agent import AssetGuardDBQueryAgents
        from assetguard.core import utils, exception
        from assetguard.core.common import ASSETGUARD_PATH, AGENT_NAME_LEN_LIMIT
...
@patch('assetguard.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
...
def test_cut_array_ko(limit, offset, expected_exception):
    ...
    """
    with pytest.raises(exception.AssetGuardException, match=f'.* {expected_exception} .*'):
...
@patch('assetguard.core.utils.len', return_value=1)
@patch('assetguard.core.utils.cut_array')
@patch('assetguard.core.utils.select_array', return_value=ANY)
@patch('assetguard.core.utils.filter_array_by_query', return_value=ANY)
@patch('assetguard.core.utils.search_array', return_value=ANY)
@patch('assetguard.core.utils.sort_array', return_value=ANY)
...
@patch('assetguard.core.utils.chown')
...
@patch('assetguard.core.utils.common.ASSETGUARD_PATH', new='/test/path')
@patch('assetguard.core.utils.path.exists', return_value=True)
@patch('assetguard.core.utils.remove')
def test_delete_assetguard_file(mock_remove, mock_exists):
    """Check delete_file calls functions with expected params"""
    assert utils.delete_assetguard_file('/test/path/etc/file')
    mock_remove.assert_called_once_with('/test/path/etc/file')


@patch('assetguard.core.utils.common.ASSETGUARD_PATH', new='/test/path')
def test_delete_assetguard_file_ko():
    """Check delete_file calls functions with expected params"""
    with pytest.raises(utils.AssetGuardError, match=r'\b1907\b'):
        utils.delete_assetguard_file('/test/different_path/etc/file')

    with pytest.raises(utils.AssetGuardError, match=r'\b1907\b'):
        utils.delete_assetguard_file('/test/path/file/../../home')

    with patch('assetguard.core.utils.path.exists', return_value=False):
        with pytest.raises(utils.AssetGuardError, match=r'\b1906\b'):
            utils.delete_assetguard_file('/test/path/etc/file')

    with patch('assetguard.core.utils.path.exists', return_value=True):
        with pytest.raises(utils.AssetGuardError, match=r'\b1907\b'):
            utils.delete_assetguard_file('/test/path/etc/file')
...
@patch('assetguard.core.utils.chown')
@patch('assetguard.core.utils.chmod')
@patch('assetguard.core.utils.utime')
...
def test_safe_move_exception(mock_utime, mock_chmod, mock_chown):
    ...
    with TemporaryDirectory() as tmpdirname:
        tmp_file = NamedTemporaryFile(dir=tmpdirname, delete=False)
        target_file = os.path.join(tmpdirname, 'target')
        with patch('assetguard.core.utils.rename', side_effect=OSError(1)):
...
@patch('assetguard.core.utils.chmod')
@patch('assetguard.core.utils.mkdir')
@patch('assetguard.core.utils.curdir', new='var')
def test_mkdir_with_mode(mock_mkdir, mock_chmod, dir_name, path_exists):
    """Test mkdir_with_mode function."""
    with patch('assetguard.core.utils.path.exists', return_value=path_exists):
...
@patch('assetguard.core.utils.mkdir', side_effect=OSError)
def test_mkdir_with_mode_ko(mock_mkdir, dir_name, exists):
    """Test mkdir_with_mode function errors work."""
    with patch('assetguard.core.utils.path.exists', return_value=exists):
        with pytest.raises(OSError):
            utils.mkdir_with_mode(dir_name)


@patch('assetguard.core.utils.open')
@patch('assetguard.core.utils.iter', return_value=['1', '2'])
def test_md5(mock_iter, mock_open):
    """Test md5 function."""
    with patch('assetguard.core.utils.hashlib.md5') as md:
...
def test_blake2b(mock_iter, mock_open):
    """Test md5 function."""
    with patch('assetguard.core.utils.hashlib.blake2b') as blake2b_mock:
...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.AssetGuardException, match=".* 1723 .*"):
...
def test_get_hash(mock_open):
    ...
    with patch('assetguard.core.utils.iter', return_value=['1', '2']):
        with patch('assetguard.core.utils.hashlib.new') as md:
            md.return_value.update.side_effect = None
    ...
    with patch('assetguard.core.utils.iter', return_value=[]):
...
@pytest.mark.parametrize('new_conf, original_conf, allow_config, should_raise', [
    ...
    ("<assetguard_config><localfile><log_format>command</log_format><command>rm -rf /</command></localfile></assetguard_config>",
     "<assetguard_config><localfile><log_format>syslog</log_format><location>/var/log/test.log</location></localfile></assetguard_config>",
    ...
    ("<assetguard_config><localfile><log_format>command</log_format><command>echo test</command></localfile></assetguard_config>",
     "<assetguard_config><localfile><log_format>command</log_format><command>echo test</command></localfile></assetguard_config>",
    ...
    ('<assetguard_config><wodle name="command"><command>ls -la</command></wodle></assetguard_config>',
     '<assetguard_config><wodle name="command"><tag>value</tag></wodle></assetguard_config>',
    ...
    ('<assetguard_config><wodle name="command"><command>test</command></wodle></assetguard_config>',
     '<assetguard_config><wodle name="command"><command>test</command></wodle></assetguard_config>',
     {'localfile': {'allow': True, 'exceptions': []},
    ...
    ("<assetguard_config><other><value>test</value></other></assetguard_config>",
     "<assetguard_config><other><value>test</value></other></assetguard_config>",
     {'localfile': {'allow': False, 'exceptions': []},
      'wodle_command': {'allow': False, 'exceptions': []}},
     False),
    ...
    ("<assetguard_config><localfile><log_format>full_command</log_format><command>cat /etc/passwd</command></localfile></assetguard_config>",
     "<assetguard_config><localfile><log_format>syslog</log_format><location>/var/log/test.log</location></localfile></assetguard_config>",
     {'localfile': {'allow': False, 'exceptions': []},
      'wodle_command': {'allow': True, 'exceptions': []}},
     True),
    ...
    ("<assetguard_config><localfile><log_format>syslog</log_format><location>/var/log/test.log</location></localfile></assetguard_config>",
     "<assetguard_config><localfile><log_format>apache</log_format><location>/var/log/apache.log</location></localfile></assetguard_config>",
     {'localfile': {'allow': False, 'exceptions': []},
      'wodle_command': {'allow': False, 'exceptions': []}},
     False),
])
def test_check_remote_commands(new_conf, original_conf, allow_config, should_raise):
    ...
    xml_new_conf = utils.load_assetguard_xml(None, new_conf)
    xml_original_conf = utils.load_assetguard_xml(None, original_conf)

    with patch('assetguard.core.utils.configuration.api_conf', new=api_conf):
        if should_raise:
            with pytest.raises(exception.AssetGuardError, match=r'.* 1124 .*'):
                utils.check_remote_commands(xml_new_conf, xml_original_conf)
        else:
            utils.check_remote_commands(xml_new_conf, xml_original_conf)


@patch('assetguard.core.utils.compile', return_value='Something')
def test_basic_load_assetguard_xml(mock_compile):
    """Test basic load_assetguard_xml functionality."""
    with patch('assetguard.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_xml)
        result = utils.load_assetguard_xml('test_file')

        assert isinstance(result, Element)


def test_load_assetguard_xml():
    """Test load_assetguard_xml function."""
    ...
    for rule_file in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_assetguard_xml')):
        path = os.path.join(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_assetguard_xml'),
                            rule_file)
        original = parse(path).getroot()
        result = utils.load_assetguard_xml(path)
...
def test_load_assetguard_xml_ko(expected_exception):
    """Test load_assetguard_xml fails gracefully when reading an invalid utf-8 character sequence"""
    file_path = os.path.join(test_data_path, 'test_load_assetguard_xml_ko/invalid_utf8.xml')
    with pytest.raises(AssetGuardException, match=f'.* {expected_exception} .*'):
        utils.load_assetguard_xml(file_path)

@pytest.mark.parametrize('version1, version2', [
    ('AssetGuard v3.5.0', 'AssetGuard v3.5.2'),
    ('AssetGuard v3.6.1', 'AssetGuard v3.6.3'),
    ('AssetGuard v3.7.2', 'AssetGuard v3.8.0'),
    ('AssetGuard v3.8.0', 'AssetGuard v3.8.1'),
    ('AssetGuard v3.9.0', 'AssetGuard v3.9.2'),
    ('AssetGuard v3.9.10', 'AssetGuard v3.9.14'),
    ('AssetGuard v3.10.1', 'AssetGuard v3.10.10'),
    ('AssetGuard v4.10.10', 'AssetGuard v4.11.0'),
    ('AssetGuard v5.1.15', 'AssetGuard v5.2.0'),
    ('v3.6.0', 'v3.6.1'),
    ('v3.9.1', 'v3.9.2'),
    ('v4.0.0', 'v4.0.1'),
    ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test AssetGuardVersion class."""
    current_version = utils.AssetGuardVersion(version1)
    new_version = utils.AssetGuardVersion(version2)
...
@pytest.mark.parametrize('version1, version2', [
    ('v3.6.0', 'v.3.6.1'),
    ('AssetGuard v4', 'AssetGuard v5'),
    ('AssetGuard v3.9', 'AssetGuard v3.10'),
    ('ABC v3.10.1', 'ABC v3.10.12'),
    ('AssetGuardv3.9.0', 'AssetGuardv3.9.2'),
    ('3.9', '3.10'),
    ('3.9.0', '3.10'),
    ('3.10', '4.2'),
    ('3', '3.9.1')
...
def test_version_ko(version1, version2):
    """Test AssetGuardVersion class."""
    try:
        utils.AssetGuardVersion(version1)
        utils.AssetGuardVersion(version2)
    except ValueError:
        return


@pytest.mark.parametrize('version1, version2', [
    ('AssetGuard v3.10.10', 'AssetGuard v3.10.10'),
    ('AssetGuard v5.1.15', 'AssetGuard v5.1.15'),
    ('v3.6.0', 'v3.6.0'),
    ('v3.9.2', 'v3.9.2')
])
...
def test_AssetGuardVersion_to_array():
    """Test AssetGuardVersion.to_array function."""
    version = utils.AssetGuardVersion('AssetGuard v3.10.0-alpha4')

    assert isinstance(version.to_array(), list)


def test_AssetGuardVersion__str__():
    """Test AssetGuardVersion.__str__ function."""
    version = utils.AssetGuardVersion('AssetGuard v3.10.0-alpha4')
...
@pytest.mark.parametrize('version1, version2', [
    ('AssetGuard v3.5.2', 'AssetGuard v4.0.0'),
    ('AssetGuard v3.10.0-alpha', 'AssetGuard v3.10.0'),
    ('AssetGuard v3.10.0-alpha4', 'AssetGuard v3.10.0-beta4'),
    ('AssetGuard v3.10.0-alpha3', 'AssetGuard v3.10.0-alpha4'),
])
def test_AssetGuardVersion__ge__(version1, version2):
    """Test AssetGuardVersion.__ge__ function."""
    current_version = utils.AssetGuardVersion(version1)
    new_version = utils.AssetGuardVersion(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.AssetGuardException, match=".* 1411 .*"):
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_AssetGuardDBQuery_protected_clean_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                             query_filter, expected_query_filter, expected_wef):
    """Test AssetGuardDBQuery._clean_filter function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=500, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.AssetGuardDBBackend(agent_id=1),
...
@patch('socket.socket.connect')
@patch('assetguard.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_AssetGuardDBQuery_protected_add_limit_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                   limit, error, expected_exception):
    """Test AssetGuardDBQuery._add_limit_to_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=limit, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.AssetGuardDBBackend(agent_id=1),
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests AssetGuardDBQuery._sort_query function works"""

    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.AssetGuardDBBackend(agent_id=1), count=5,
                               get_data=None)
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_add_sort_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                  sort, expected_exception):
    """Test AssetGuardDBQuery._add_sort_to_query function."""
    fields = {'1': 'one', '2': 'two', '3': 'three', '4': 'four'}
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=sort,
                               search=None, select=None, filters=None,
                               fields=fields,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test AssetGuardDBQuery._add_search_to_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('glob.glob', return_value=True)
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_parse_select_filter(mock_socket_conn, mock_glob, mock_conn_db, mock_exists,
                                                    selector_fields, error, expected_exception):
    """Test AssetGuardDBQuery._parse_select_filter function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._parse_select_filter')
def test_AssetGuardDBQuery_protected_add_select_to_query(mock_parse, mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test AssetGuardDBQuery._add_select_to_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.path.exists', return_value=True)
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
def test_AssetGuardDBQuery_protected_parse_query_regex(mock_backend_connect, mock_exists, q, expected_query_filters):
    """Test AssetGuardDBQuery._parse_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_parse_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_glob,
                                                     mock_exists, filter_):
    """Test AssetGuardDBQuery._parse_legacy_filters function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=filter_,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.AssetGuardDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._parse_legacy_filters')
@patch('assetguard.core.utils.AssetGuardDBQuery._parse_query')
def test_AssetGuardDBQuery_parse_filters(mock_query, mock_filter, mock_socket_conn, mock_conn_db, mock_glob,
                                    mock_exists, filter, q):
    """Test AssetGuardDBQuery._parse_filters function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search={"negation": True, "value": "1"}, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=q,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._filter_status')
@patch('assetguard.core.utils.AssetGuardDBQuery._filter_date')
def test_AssetGuardDBQuery_protected_process_filter(mock_date, mock_status, mock_socket_conn, mock_conn_db,
                                               mock_glob, mock_exists, field_name, field_filter, q_filter):
    """Tests AssetGuardDBQuery._process_filter."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04', 'status': 'active'},
                               default_sort_field=None, query=None,
                               backend=utils.AssetGuardDBBackend(agent_id=0), count=5,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._process_filter')
def test_AssetGuardDBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn,
                                                     mock_conn_db, mock_glob, mock_exists):
    """Test AssetGuardDBQuery._add_filters_to_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=['os.name'],
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query='os.name=ubuntu',
                               backend=utils.AssetGuardDBBackend(agent_id=0), distinct=True,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('assetguard.core.utils.path.exists', return_value=True)
def test_AssetGuardDBQuery_protected_add_filters_to_query_final_query(mock_conn_db, mock_file_exists,
                                                                 filters, expected_query):
    """Test AssetGuardDBQuery._add_filters_to_query final query."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None, search=None, select=None,
                               fields={'id': 'id', 'status': 'status', 'group': 'group'},
                               default_sort_field=None, query='', backend=utils.AssetGuardDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_get_total_items(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test AssetGuardDBQuery._get_total_items function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_get_total_items_mitre(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.AssetGuardDBBackend(agent_id=0, query_format='mitre'), count=5,
                               get_data=None, date_fields=['date1', 'date2'])
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_substitute_params(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.AssetGuardDBQuery._get_total_items function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.AssetGuardDBQuery._format_data_into_dictionary."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_filter_status(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.AssetGuardDBQuery._filter_status function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_filter_date(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            date_filter, filter_db_name, time, error):
    """Test utils.AssetGuardDBQuery._filter_date function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
    ...
    with patch('assetguard.core.utils.get_timeframe_in_seconds', return_value=time):
        if error:
            with pytest.raises(exception.AssetGuardException, match=".* 1412 .*"):
...
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_general_run(mock_socket_conn, execute_value, expected_result):
    """Test utils.AssetGuardDBQuery.general_run function."""
    with patch('assetguard.core.utils.AssetGuardDBBackend.execute', return_value=execute_value):
        query = AssetGuardDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_oversized_run(mock_socket_conn, execute_value, rbac_ids, negate,
                                    final_rbac_ids, expected_result):
    """Test utils.AssetGuardDBQuery.oversized_run function."""
    with patch('assetguard.core.utils.AssetGuardDBBackend.execute', side_effect=[execute_value, final_rbac_ids]):
        query = AssetGuardDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._default_query')
def test_AssetGuardDBQuery_reset(mock_query, mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.AssetGuardDBQuery.reset function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'os.name'},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.AssetGuardDBQuery._default_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test utils.AssetGuardDBQuery._default_count_query function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            value):
    """Test utils.AssetGuardDBQuery._pass_filter function."""
    query = utils.AssetGuardDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQueryDistinct_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    """Test utils.AssetGuardDBQueryDistinct._default_query function."""
    query = utils.AssetGuardDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQueryDistinct_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.AssetGuardDBQueryDistinct._default_count_query function."""
    query = utils.AssetGuardDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._add_filters_to_query')
def test_AssetGuardDBQueryDistinct_protected_add_filters_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                             mock_glob, mock_exists):
    """Test utils.AssetGuardDBQueryDistinct._add_filters_to_query function."""
    query = utils.AssetGuardDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'name'},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True, default_sort_field='`group`',
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._add_select_to_query')
def test_AssetGuardDBQueryDistinct_protected_add_select_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                            mock_glob, mock_exists, select):
    """Test utils.AssetGuardDBQueryDistinct._add_select_to_query function."""
    query = utils.AssetGuardDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
    ...
    if len(select) > 1:
        with pytest.raises(exception.AssetGuardException, match=".* 1410 .*"):
            query._add_select_to_query()
    else:
        query._add_select_to_query()
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQueryDistinct_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db,
                                                                    mock_glob, mock_exists):
    """Test utils.AssetGuardDBQueryDistinct._format_data_into_dictionary function."""
    query = utils.AssetGuardDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'fields': ['name']},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests utils.AssetGuardDBQueryGroupBy.__init__ function works"""
    utils.AssetGuardDBQueryGroupBy(filter_fields=None, offset=0, limit=1, table='agent',
                              ...
                              backend=utils.AssetGuardDBBackend(agent_id=0), get_data=None,
...
@patch('assetguard.core.utils.AssetGuardDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('assetguard.core.utils.AssetGuardDBQuery._get_total_items')
def test_AssetGuardDBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db,
                                                       mock_glob, mock_exists):
    """Test utils.AssetGuardDBQueryGroupBy._get_total_items function."""
    query = utils.AssetGuardDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
...
@patch('assetguard.core.utils.AssetGuardDBQuery._add_select_to_query')
@patch('assetguard.core.utils.AssetGuardDBQuery._parse_select_filter')
def test_AssetGuardDBQueryGroupBy_protected_add_select_to_query(mock_parse, mock_add, mock_socket_conn,
                                                           mock_conn_db, mock_glob, mock_exists):
    """Test utils.AssetGuardDBQueryGroupBy._add_select_to_query function."""
    query = utils.AssetGuardDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
                                      limit=1, table='agent', sort=None, search=None,
                                      select={'name'}, filters=None,
                                      fields={'name': '`group`'}, query=None,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.AssetGuardError, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.AssetGuardError as e:
        assert e.code == 1724
...
@patch('assetguard.core.utils.check_assetguard_limits_unchanged')
@patch('assetguard.core.utils.check_remote_commands')
@patch('assetguard.core.utils.check_agents_allow_higher_versions')
@patch('assetguard.core.utils.check_indexer')
@patch('assetguard.core.manager.common.ASSETGUARD_PATH', new=test_files_path)
def test_validate_assetguard_xml(mock_check_indexer,
                            mock_agents_versions, mock_remote_commands, mock_unchanged_limits):
    """Test validate_assetguard_xml method works and methods inside are called with expected parameters"""
    ...
    with patch('builtins.open', m):
        utils.validate_assetguard_xml(xml_file)
...
def test_validate_assetguard_xml_ko(effect, expected_exception):
    """Tests validate_assetguard_xml function works when open function raises an exception.
    Parameters
    ----------
    effect : Exception
    ...
    with patch('assetguard.core.utils.load_assetguard_xml', side_effect=effect):
        with pytest.raises(AssetGuardException, match=f'.* {expected_exception} .*'):
            utils.validate_assetguard_xml(input_file)


@patch('assetguard.core.utils.full_copy')
...
@patch('assetguard.core.utils.full_copy', side_effect=IOError)
def test_delete_file_with_backup_ko(mock_copyfile):
    """Test delete_file_with_backup function exceptions."""
    with pytest.raises(utils.AssetGuardError, match='.* 1019 .*'):
        utils.delete_file_with_backup('test', 'test', str)


def test_to_relative_path():
    """Test to_relative_path function."""
    path = 'etc/assetguard-manager.conf'
    assert utils.to_relative_path(os.path.join(ASSETGUARD_PATH, path)) == path
...
@patch('assetguard.core.utils.copy2', new=copyfile)
...
@patch('assetguard.core.utils.get_localtime')
...
@pytest.mark.parametrize(
    'assetguard_localtime, system_localtime',
    [
        pytest.param(None, sentinel.system_tz, id='system_fallback'),
        pytest.param(sentinel.assetguard_tz, sentinel.system_tz, id='assetguard'),
        pytest.param(None, None, id='utc_fallback'),
    ]
)
@patch('assetguard.core.utils.gettz')
def test_get_localtime(mock_gettz, assetguard_localtime, system_localtime):
    """Test if the result is the expected timezone."""

    mock_gettz.side_effect = [assetguard_localtime, system_localtime]

    timezone = utils.get_localtime()

    calls = [call(ASSETGUARD_LOCALTIME_PATH)]

    if assetguard_localtime is None:
        calls.append(call())

    mock_gettz.assert_has_calls(calls)

    assert timezone is (
        assetguard_localtime
        if assetguard_localtime is not None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "<assetguard_config><global><limits><eps><maximum>300</maximum><timeframe>5</timeframe></eps></limits></global>"
            "</assetguard_config>",
            "<assetguard_config><global><limits><eps><maximum>300</maximum></eps></limits></global></assetguard_config>",
            False,
        ),
        # Different limits + adding a new eps option + unrelated configuration. Should be considered changed.
        (
            "<assetguard_config><global><logall>no</logall></global>"
        ...
        (
            "<assetguard_config><global><logall>yes</logall><limits><eps><maximum>300</maximum></eps></limits></global></assetguard_config>",
            "<assetguard_config><global><limits><eps><maximum>300</maximum></eps></limits></global></assetguard_config>",
            True,
        ),
        ...
        (
            "<assetguard_config><global><limits><eps><maximum>300</maximum></eps></limits></global></assetguard_config>"
            "<assetguard_config><global><limits><eps><maximum>100</maximum></eps></limits></global></assetguard_config>",
            "<assetguard_config><global><limits><eps><maximum>100</maximum></eps></limits></global></assetguard_config>"
            "<assetguard_config><global><limits><eps><maximum>300</maximum></eps></limits></global></assetguard_config>",
            False,
        ),
        ...
        (
            "<assetguard_config><global><limits><eps><maximum>300</maximum><timeframe>5</timeframe></eps></limits></global></assetguard_config>",
            "<assetguard_config><global><limits><eps><timeframe>5</timeframe><maximum>300</maximum></eps></limits></global></assetguard_config>",
            True,
        ),
    ],
)
...
def test_check_assetguard_limits_unchanged(new_conf, unchanged_limits_conf, original_conf, limits_conf):
    """Test if assetguard-manager.conf limits are protected by the API.

    When 'eps': {'allow': False} is set in the API configuration, the limits in assetguard-manager.conf cannot be changed.
    However, other configuration sections can be added, removed or modified.

    Parameters
    ----------
    new_conf : str
        New assetguard-manager.conf to be uploaded.
    unchanged_limits_conf : bool
        Whether the limits section in assetguard-manager.conf is the same as the original one.
    original_conf : str
        Original assetguard-manager.conf to be uploaded.
    limits_conf : dict
    ...
    with patch('assetguard.core.utils.configuration.api_conf', new=api_conf):
        if limits_conf['eps']['allow'] or unchanged_limits_conf:
            utils.check_assetguard_limits_unchanged(xml_new_conf, xml_original_conf)
        else:
            with pytest.raises(exception.AssetGuardError, match=".* 1127 .*"):
                utils.check_assetguard_limits_unchanged(xml_new_conf, xml_original_conf)


@pytest.mark.parametrize("new_conf, original_conf, agents_conf, should_raise", [
    ("<assetguard_config><auth></auth></assetguard_config>",
     "<assetguard_config><auth><allow_higher_versions>yes</allow_higher_versions></auth></assetguard_config>",
     {'allow_higher_versions': {'allow': False}},
     True),

    ("<assetguard_config><remote></remote></assetguard_config>",
     "<assetguard_config><remote><allow_higher_versions>yes</allow_higher_versions></remote></assetguard_config>",
     {'allow_higher_versions': {'allow': False}},
     True),
    ...
    ("<assetguard_config><remote><allow_higher_versions>yes</allow_higher_versions></remote></assetguard_config>",
     "<assetguard_config><remote><allow_higher_versions>no</allow_higher_versions></remote></assetguard_config>",
     {'allow_higher_versions': {'allow': True}},
     False),
])
def test_check_agents_allow_higher_versions(new_conf, original_conf, agents_conf, should_raise):
    """Check if assetguard-manager.conf agents versions are protected by the API."""
    ...
    with patch('assetguard.core.utils.configuration.api_conf', new=api_conf):
        if should_raise:
            with pytest.raises(exception.AssetGuardError, match=".* 1129 .*"):
...
@pytest.mark.parametrize("new_conf, original_conf, indexer_changed", [
    (
        "<assetguard_config><indexer></indexer></assetguard_config>",
        "<assetguard_config><indexer></indexer></assetguard_config>",
        False,
    ),
    (
        "<assetguard_config><indexer><hosts><host>https://0.0.0.0:9200/</host></hosts></indexer></assetguard_config>",
        "<assetguard_config><indexer><hosts><host>https://127.0.0.1:9200/</host></hosts></indexer></assetguard_config>",
        True,
    ),
    (
        "<assetguard_config><indexer><ssl><key>/var/assetguard-manager/etc/certs/server-key.pem</key></ssl>" \
        "</indexer></assetguard_config>",
        "<assetguard_config><indexer></indexer></assetguard_config>",
        True,
    ),
    (
        "<assetguard_config><indexer><ssl><key>/var/assetguard-manager/etc/certs/server-key.pem</key></ssl>" \
        "</indexer></assetguard_config>",
        "<assetguard_config><indexer><ssl><key>server-key.pem</key></ssl></indexer></assetguard_config>",
        True,
    ),
    (
        "<assetguard_config><auth><disabled>no</disabled></auth></assetguard_config>",
        "<assetguard_config><auth><disabled>yes</disabled></auth></assetguard_config>",
        False,
    ),
    (
        "<assetguard_config><indexer><enabled>no</enabled></indexer></assetguard_config>"
        "<assetguard_config><integration><name>custom-test-ampersand</name><hook_url>https://localhost?querystring1=1&querystring2=2</hook_url><alert_format>json</alert_format></integration></assetguard_config>",
        "<assetguard_config><indexer><enabled>no</enabled></indexer></assetguard_config>",
        False,
    ),
    (
...
def test_check_indexer(new_conf, original_conf, indexer_changed, indexer_allowed):
    """Check if the assetguard-manager.conf indexer section is protected by the API.
    ...
    new_conf : str
        New assetguard-manager.conf to be uploaded.
    original_conf : str
        Original assetguard-manager.conf.
...
def test_upload_file(mock_uid, mock_gid,
    ...
    ----------
    mock_uid: Mock
        mock of the assetguard.core.common.assetguard_uid function.
    mock_gid: Mock
        mock of the assetguard.core.common.assetguard_gid function.
    mock_chmod: Mock
        mock of the assetguard.core.utils.chmod function.
    mock_mks
        Mock of the assetguard.core.utils.tempfile.mkstemp function.
    chk_xml: bool
    ...
    with patch('assetguard.core.utils.open', mko):
        with patch('assetguard.core.utils.safe_move') as mock_safe_move:
...
@pytest.mark.parametrize(
    'sm_side_effect, w_side_effect, upload_error', [
        ...
        (None, True, (utils.AssetGuardInternalError, 1005)),
        # safe_move raises an Error()
        (Error(), False, (utils.AssetGuardInternalError, 1016)),
...
def test_upload_file_ko(mock_uid, mock_gid, mock_chmod, mock_mks,
    ...
    with patch('assetguard.core.utils.open', mko):
        with patch('assetguard.core.utils.safe_move', side_effect=sm_side_effect) as mock_safe_move:
