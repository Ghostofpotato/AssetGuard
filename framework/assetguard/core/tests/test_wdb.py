...
from assetguard.core.common import MAX_SOCKET_BUFFER_SIZE
...
def test_async_init():
    """Verify that AsyncAssetGuardDBConnection attributes are correct."""
    async_wdb = AsyncAssetGuardDBConnection('test')
...
async def test_async_open_connection(open_unix_connection_mock):
    ...
    async_wdb = AsyncAssetGuardDBConnection(loop='test_loop')
...
def test_async_close():
    ...
    async_wdb = AsyncAssetGuardDBConnection()
...
async def test_async_send_ko():
    ...
    with pytest.raises(exception.AssetGuardInternalError, match=".* 2005 .*"):
    ...
    with pytest.raises(exception.AssetGuardInternalError, match=r'\b2010\b'):
        await async_wdb._send('test')

    # AssetGuard-db error response.
    encoded_response = 'err Error message'.encode(encoding='utf-8')
    async_wdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    with pytest.raises(exception.AssetGuardError, match=r'\b2003\b'):
        await async_wdb._send('test')


@pytest.mark.asyncio
async def test_run_wdb_command():
    """Test `AssetGuardDBConnection.run_wdb_command` method."""
    send_result = ('status', '["data"]')
    command = "any wdb command"

    wdb_con = AsyncAssetGuardDBConnection()
    with patch('assetguard.core.wdb.AsyncAssetGuardDBConnection._send', return_value=send_result) as wdb_send_mock:
...
async def test_run_wdb_command_ko(wdb_response):
    """Test `AssetGuardDBConnection.run_wdb_command` method expected exceptions."""
    with patch('assetguard.core.wdb.AsyncAssetGuardDBConnection._send', return_value=wdb_response):
        wdb_con = AsyncAssetGuardDBConnection()
        with pytest.raises(exception.AssetGuardInternalError, match=".* 2007 .*") as expected_exc:
...
def test_failed_connection():
    ...
    with patch('assetguard.core.common.WDB_SOCKET', '/this/path/doesnt/exist'):
        with pytest.raises(exception.AssetGuardException, match=".* 2005 .*"):
            AssetGuardDBConnection()
...
def test_wrong_character_encodings_wdb(send_mock, connect_mock):
    """
    Tests receiving a text with a bad character encoding from assetguard db
    """
...
def test_failed_send_private(send_mock, connect_mock):
    ...
    with patch('socket.socket.recv', side_effect=recv_mock):
        mywdb = AssetGuardDBConnection()
        with pytest.raises(exception.AssetGuardException, match=".* 2003 .*"):
            mywdb._send('test_msg')

    with patch('socket.socket.recv', return_value=b'a' * (MAX_SOCKET_BUFFER_SIZE + 1)):
        mywdb = AssetGuardDBConnection()
        with pytest.raises(exception.AssetGuardException, match=".* 2009 .*"):
            mywdb._send('test_msg')
...
def test_query_input_validation_private(send_mock, connect_mock, error_query):
    mywdb = AssetGuardDBConnection()
    with pytest.raises(exception.AssetGuardException, match=".* 2004 .*"):
...
@patch("socket.socket.connect")
@patch("socket.socket.send")
@patch("assetguard.core.wdb.AssetGuardDBConnection._send")
def test_execute(send_mock, socket_send_mock, connect_mock):
    ...
    with patch("assetguard.core.wdb.AssetGuardDBConnection._send", new=send_mock):
...
def test_execute_pagination(socket_send_mock, connect_mock):
    ...
    # Test pagination
    with patch("assetguard.core.wdb.AssetGuardDBConnection._send",
               side_effect=[[{'total': 5}], exception.AssetGuardInternalError(2009), ['ok', '{"total": 5}'],
                            ['ok', '{"total": 5}']]):
        mywdb.execute("agent 001 sql select test from test offset 1 limit 500")

    # Test pagination error
    with patch("assetguard.core.wdb.AssetGuardDBConnection._send",
               side_effect=[[{'total': 5}], exception.AssetGuardInternalError(2009)]):
        with pytest.raises(exception.AssetGuardInternalError, match=".* 2009 .*"):
            mywdb.execute("agent 001 sql select test from test offset 1 limit 1")
...
def test_failed_execute(send_mock, connect_mock, error_query, error_type, expected_exception, delete, update):
    ...
    if not error_type:
        ...
        with patch("assetguard.core.wdb.AssetGuardDBConnection._send", return_value=[{'total': 5}]):
            with patch("assetguard.core.wdb.min", side_effect=error_type):
                with pytest.raises(exception.AssetGuardException, match=f'.* {expected_exception} .*'):
                    mywdb.execute(error_query, delete=delete, update=update)
...
def test_AssetGuardDBConnection_loads(string):
    """Test that the `loads` method from the class `AssetGuardDBConnection` cleans empty objects from the result."""
    result = AssetGuardDBConnection.loads(string)
    assert len(result) == 1
