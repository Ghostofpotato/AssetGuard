...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        from assetguard.core.agent import *
        from assetguard.core.exception import AssetGuardException
        from api.util import remove_nones_to_dict
        from assetguard.rbac.utils import RESOURCES_CACHE
...
def get_AssetGuardDBQuery_params(wdb_class):
    """Get default parameters for the specified AssetGuardDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the AssetGuardDBQuery class. Example: 'Agents' to get default parameters from `AssetGuardDBQueryAgents` class.
    ...
    """
    with patch('assetguard.core.agent.AssetGuardDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'AssetGuardDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with AssetGuard packages availables with their hash
...
@patch("assetguard.core.agent.AssetGuardDBBackend")
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of AssetGuardDBQueryAgents works properly.
    ...
    if value:
        AssetGuardDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(AssetGuardException, match=".* 2005 .*"):
            AssetGuardDBQueryAgents()


@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of AssetGuardDBQueryAgents returns expected query"""
    query_agent = AssetGuardDBQueryAgents()
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of AssetGuardDBQueryAgents returns expected result
    ...
    """
    query_agent = AssetGuardDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of AssetGuardDBQueryAgents returns expected query"""
    query_agent = AssetGuardDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of AssetGuardDBQueryAgents returns expected data"""
    query_agent = AssetGuardDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of AssetGuardDBQueryAgents returns expected query"""
    query_agent = AssetGuardDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of AssetGuardDBQueryAgents returns expected query
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of AssetGuardDBQueryGroup works properly.
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of AssetGuardDBQueryGroup works properly.
        ...
        """
    query_group = AssetGuardDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of AssetGuardDBQueryGroupByAgents works properly."""
    query_group = AssetGuardDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of AssetGuardDBQueryGroupByAgents works properly."""
    query_group = AssetGuardDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'assetguard-master'},
                         {'count': 1, 'name': 'assetguard-agent1'}]
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if AssetGuardDBQueryGroupByAgents works properly."""
    query_group = AssetGuardDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_AssetGuardDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of AssetGuardDBQueryGroup works properly"""
    query_group = AssetGuardDBQueryGroup()
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of AssetGuardDBQueryMultigroups works properly."""
    query_multigroups = AssetGuardDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of AssetGuardDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = AssetGuardDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_AssetGuardDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of AssetGuardDBQueryMultigroups works properly."""
    query_multigroups = AssetGuardDBQueryMultigroups(group_id='test')
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_AssetGuardDBQueryMultigroups_get_total_items(mock_socket_conn, send_mock):
    """Tests if method _get_total_items of AssetGuardDBQueryMultigroups works properly."""
    query_multigroups = AssetGuardDBQueryMultigroups(group_id='test')
...
@patch('assetguard.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(AssetGuardResourceNotFound, match='.* 1701 .*'):
...
@patch('assetguard.core.agent.AssetGuardQueue.send_msg_to_agent')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(AssetGuardQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(AssetGuardQueue.HC_FORCE_RECONNECT, agent_id)


@patch('assetguard.core.agent.AssetGuardQueue')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(AssetGuardError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('assetguard.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('assetguard.core.agent.get_manager_status', return_value={'assetguard-manager-authd': 'running'}):
        agent = Agent('001')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(AssetGuardError, match='.* 1726 .*'):
        agent = Agent('001')
        agent.remove()


@patch('assetguard.core.agent.AssetGuardSocketJSON')
def test_agent_remove_authd(mock_assetguard_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('001')
    agent._remove_authd(purge=True)
    mock_assetguard_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(1).zfill(3), "purge": True}})
    mock_assetguard_socket.return_value.receive.assert_called_once()
    mock_assetguard_socket.return_value.close.assert_called_once()
...
@patch('assetguard.core.agent.Agent._add_authd')
...
@patch('assetguard.core.agent.get_manager_status', return_value={'assetguard-manager-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(AssetGuardError, match='.* 1706 .*'):
...
@patch('assetguard.core.agent.AssetGuardSocketJSON')
def test_agent_add_authd(mock_assetguard_socket, name, ip, id, key, force):
    ...
    mock_assetguard_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (AssetGuardError(9008, cmd_error=True), ".* 1705 .*"),
    (AssetGuardError(9007, cmd_error=True), ".* 1706 .*"),
    (AssetGuardError(9012, cmd_error=True), ".* 1708 .*"),
    (AssetGuardError(9000, cmd_error=True), ".* None")
])
@patch('assetguard.core.agent.AssetGuardSocketJSON')
def test_agent_add_authd_ko(mock_assetguard_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(AssetGuardError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_assetguard_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(AssetGuardError, match=expected_exception):
            agent._add_authd('test_add', '192.168.0.1')


@patch('assetguard.core.agent.rmtree')
@patch('assetguard.core.agent.path.exists', return_value=True)
@patch('assetguard.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('assetguard.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('assetguard.core.agent.Agent.set_agent_group_relationship')
...
@patch('assetguard.core.agent.Agent.get_agent_groups', return_value=['default'])
async def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('assetguard.core.agent.Agent.get_agent_groups', side_effect=AssetGuardError(2003)):
        with pytest.raises(AssetGuardInternalError, match='.* 2007 .*'):
...
@patch("assetguard.core.agent.Agent.get_basic_information")
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(AssetGuardError, match='.* 1722 .*'):
...
@patch('assetguard.core.agent.AssetGuardDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(AssetGuardInternalError, match='.* 2005 .*'):
...
@patch('assetguard.core.agent.Agent.set_agent_group_relationship')
@patch('assetguard.core.agent.Agent.group_exists', return_value=True)
@patch('assetguard.core.agent.Agent.get_basic_information')
async def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('assetguard.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
async def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('assetguard.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(AssetGuardResourceNotFound, match='.* 1710 .*'):
            await Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('assetguard.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(AssetGuardError, match='.* 1734 .*'):
            await Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('assetguard.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(AssetGuardError, match='.* 1745 .*'):
            await Agent.unset_single_group_agent('002', 'default', force=True)


@patch('assetguard.core.assetguard_socket.AssetGuardSocket')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_assetguard_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_assetguard_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'AssetGuard v4.0.0')
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_assetguard_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(AssetGuardError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'AssetGuard v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(AssetGuardError, match=".* 1307 .*"):
        agent.get_config('com', None, 'AssetGuard v4.0.0')
        agent.get_config(None, 'active-response', 'AssetGuard v4.0.0')

    # Agent AssetGuard version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(AssetGuardInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'AssetGuard v3.6.0')


@patch('assetguard.core.assetguard_socket.AssetGuardSocket')
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_assetguard_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_assetguard_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('assetguard.core.wdb.AssetGuardDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_assetguard_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(AssetGuardInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['AssetGuard v4.2.0', 'AssetGuard v4.0.0', 'AssetGuard v4.2.1', 'AssetGuard v3.13.2']])
])
@patch('assetguard.core.agent.AssetGuardQueue.send_msg_to_agent')
@patch('assetguard.core.agent.AssetGuardQueue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('assetguard.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = AssetGuardQueue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = AssetGuardQueue.RESTART_AGENTS_JSON if AssetGuardVersion(
                agent_version['version']) >= AssetGuardVersion(common.AR_LEGACY_VERSION) else AssetGuardQueue.RESTART_AGENTS
...
def test_get_agents_info():
    ...
    with patch('assetguard.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('assetguard.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `AssetGuardDBConnection.send` method.
    expected_agents : set
    ...
    with patch('assetguard.core.wdb.AssetGuardDBConnection.send', side_effect=wdb_response):
...
@patch('assetguard.core.agent.core_upgrade_agents')
