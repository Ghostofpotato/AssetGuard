...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        ...
        from assetguard.core import common, stats
...
def test_totals_ko_():
    ...
    with patch('assetguard.core.stats.open', side_effect=IOError):
        with pytest.raises(AssetGuardException, match=".* 1308 .*"):
            stats.totals_(date(1996, 8, 13))

    with patch("builtins.open", mock_open(read_data='15-571-3-2\n15--107--1483')):
        with pytest.raises(AssetGuardInternalError, match=".* 1309 .*"):
            stats.totals_(date(1996, 8, 13))


@patch('assetguard.core.common.STATS_PATH', new=test_data_path)
def test_weekly_():
...
@patch('assetguard.core.common.STATS_PATH', new='')
...
@patch('assetguard.core.assetguard_socket.AssetGuardSocketJSON.close')
@patch('assetguard.core.assetguard_socket.AssetGuardSocketJSON.send')
@patch('assetguard.core.assetguard_socket.AssetGuardSocketJSON.__init__', return_value=None)
def test_get_daemons_stats_socket(mock__init__, mock_send, mock_close, agents_list, expected_socket_response,
    ...
    with patch('assetguard.core.assetguard_socket.AssetGuardSocketJSON.receive',
...
def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(AssetGuardInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_ko():
    ...
    with patch('assetguard.core.stats.open'):
        with pytest.raises(AssetGuardInternalError, match=".* 1104 .*"):
            stats.get_daemons_stats_('filename')
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('assetguard.core.assetguard_socket.AssetGuardSocket.__init__', return_value=None) as mock_socket:
        with patch('assetguard.core.assetguard_socket.AssetGuardSocket.send', side_effect=None) as mock_send:
            with patch('assetguard.core.assetguard_socket.AssetGuardSocket.receive', return_value=response.encode()):
                with patch('assetguard.core.assetguard_socket.AssetGuardSocket.close', side_effect=None):
                    stats.get_daemons_stats_from_socket(agent_id, daemon)

        mock_socket.assert_called_once_with(os.path.join(common.ASSETGUARD_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket(agent_id, daemon, responses, expected, expected_socket_calls,
    ...
    with patch('assetguard.core.assetguard_socket.AssetGuardSocket.__init__', return_value=None) as mock_socket:
        with patch('assetguard.core.assetguard_socket.AssetGuardSocket.send', side_effect=None) as mock_send:
            with patch('assetguard.core.assetguard_socket.AssetGuardSocket.receive', side_effect=responses):
                with patch('assetguard.core.assetguard_socket.AssetGuardSocket.close', side_effect=None):
