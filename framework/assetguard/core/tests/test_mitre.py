...
from assetguard.tests.util import InitWDBSocketMock

with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        from assetguard.core.mitre import *


@patch('assetguard.core.utils.AssetGuardDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_mitre_test.sql'))
def test_AssetGuardDBQueryMitreMetadata(mock_wdb):
    """Verify that the method connects correctly to the database and returns the correct type."""
    db_query = AssetGuardDBQueryMitreMetadata()
    data = db_query.run()

    assert isinstance(db_query, AssetGuardDBQueryMitre) and isinstance(data, dict)


@pytest.mark.parametrize('wdb_query_class', [
    AssetGuardDBQueryMitreGroups,
    AssetGuardDBQueryMitreMitigations,
    AssetGuardDBQueryMitreReferences,
    AssetGuardDBQueryMitreTactics,
    AssetGuardDBQueryMitreTechniques,
    AssetGuardDBQueryMitreSoftware

])
@patch('assetguard.core.utils.AssetGuardDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_mitre_test.sql'))
def test_AssetGuardDBQueryMitre_classes(mock_wdb, wdb_query_class):
...
@patch('assetguard.core.utils.AssetGuardDBConnection')
