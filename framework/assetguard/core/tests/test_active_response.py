...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        ...
        from assetguard.core import active_response
...
@pytest.mark.parametrize('agent_version, builder_type', [
    ('AssetGuard v4.2.0', active_response.ARJsonMessage),
    ('AssetGuard v4.3.0', active_response.ARJsonMessage),
    ('AssetGuard v4.1.0', active_response.ARStrMessage)
])
...
@pytest.mark.parametrize('expected_exception, command, arguments', [
    ...
    (1652, 'random', []),
    (1652, 'invalid_cmd', []),
    (None, 'restart-assetguard0', []),
    (None, '!restart-assetguard0', []),
    (None, 'restart-assetguard0', ["arg1", "arg2"])
])
@patch('assetguard.core.common.AR_CONF', new=test_data_path)
def test_create_message(expected_exception, command, arguments):
    ...
    if expected_exception:
        with pytest.raises(AssetGuardError, match=f'.* {expected_exception} .*'):
...
@pytest.mark.parametrize('expected_exception, command, arguments, alert', [
    (1650, None, [], None),
    (None, 'restart-assetguard0', [], None),
    (None, 'restart-assetguard0', [], None),
    (None, 'restart-assetguard0', ["arg1", "arg2"], None),
...
@pytest.mark.parametrize('agent_id, agent_version, command, arguments, alert', [
    ('agent001', 'AssetGuard v4.14.0', 'ls', ['arg1', 'arg2'], 
...
def test_send_ar_message(agent_id, agent_version, command, arguments, alert):
    ...
    with patch.object(Agent, 'get_config', return_value=mock_agent_conf) as mock_get_config, \
        ...
        mock_wq.send_msg_to_agent.assert_called_once_with(msg=mock_message_builder_instance.create_message.return_value,
                                                          agent_id=agent_id,
                                                          msg_type=active_response.AssetGuardQueue.AR_TYPE)


@pytest.mark.parametrize('mock_agent_info, mock_agent_conf, expected_error_code',[
    ({'status': 'active', 'version': 'AssetGuard v4.14.0'}, {'active-response': {'disabled': 'yes'}}, 1750),
])
