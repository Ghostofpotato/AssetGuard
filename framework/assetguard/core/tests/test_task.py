...
with patch('assetguard.core.common.assetguard_uid'):
    with patch('assetguard.core.common.assetguard_gid'):
        from assetguard.core.task import *
        from assetguard.core.utils import AssetGuardDBQuery


def test_assetguard_db_query_task__init__():
    """Check if AssetGuardDBQueryTask is properly initialized."""
    with patch('assetguard.core.utils.AssetGuardDBQuery.__init__') as mock_wdbq, \
            patch('assetguard.core.utils.AssetGuardDBBackend.__init__', return_value=None) as mock_wdb_backend:
        AssetGuardDBQueryTask()
...
@patch('assetguard.core.assetguard_socket.AssetGuardSocket.receive', return_value=b'"{\'test\':\'test\'}"')
@patch('assetguard.core.assetguard_socket.AssetGuardSocket.send')
@patch('assetguard.core.assetguard_socket.AssetGuardSocket.close')
def test_send_to_tasks_socket(mock_close, mock_send, mock_receive, mock_connect):
    ...
    ----------
        mock_send: MagicMock
            Mock of AssetGuardSocket's method send.
        mock_connect: MagicMock
            Mock of AssetGuardSocket's method connect.
        mock_close: MagicMock
            Mock of AssetGuardSocket's method close.
        mock_receive: MagicMock
            Mock of AssetGuardSocket's method receive.
    """
    command = {'test': 'test'}
...
def test_send_to_tasks_socket_ko():
    ...
    with pytest.raises(AssetGuardInternalError, match=".* 1121 .*"):
        send_to_tasks_socket({'test': 'test'})


def test_assetguard_db_query_task__final_query():
    """Check if AssetGuardDBQueryTask's method _final_query works properly."""
    with patch('assetguard.core.utils.AssetGuardDBBackend.__init__', return_value=None), \
            patch('assetguard.core.utils.AssetGuardDBQuery._default_query', return_value='test'):
        wdbq_task = AssetGuardDBQueryTask()
        assert wdbq_task._final_query() == f'test WHERE task_id IN (test) LIMIT :limit OFFSET :offset'


def test_assetguard_db_query_task__format_data_into_dictionary():
    """Check that AssetGuardDBQueryTask's method _format_data_into_dictionary works properly."""
    data = [
...
def test_assetguard_db_query_task__process_filter(field_name, field_filter, q_filter):
    """Check that AssetGuardDBQueryTask's method _process_filter works properly."""
    with patch('assetguard.core.utils.AssetGuardDBBackend.__init__', return_value=None), \
            patch('assetguard.core.utils.AssetGuardDBQuery._default_query', return_value='test'):
        wdbq_task = AssetGuardDBQueryTask()

    with patch.object(AssetGuardDBQuery, '_process_filter') as mock_sup_proc:
