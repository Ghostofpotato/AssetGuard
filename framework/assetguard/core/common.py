...
def assetguard_uid() -> int:
    """Retrieve the numerical user ID for the assetguard user.
    ...
    int
        Numerical user ID.
    """
    return getpwnam(USER_NAME).pw_uid if globals()['_ASSETGUARD_UID'] is None else globals()['_ASSETGUARD_UID']


def assetguard_gid() -> int:
    """Retrieve the numerical group ID for the assetguard group.
    ...
    int
        Numerical group ID.
    """
    return getgrnam(GROUP_NAME).gr_gid if globals()['_ASSETGUARD_GID'] is None else globals()['_ASSETGUARD_GID']
...
def get_installation_uid() -> str:
    ...
    if os.path.exists(INSTALLATION_UID_PATH):
        ...
        with open(INSTALLATION_UID_PATH, 'w') as f:
            ...
            os.chown(f.name, assetguard_uid(), assetguard_gid())
...
# =========================================== AssetGuard constants and variables ============================================
# Token cache clear event.
token_cache_event = Event()
_ASSETGUARD_UID = None
_ASSETGUARD_GID = None
GROUP_NAME = 'assetguard-manager'
USER_NAME = 'assetguard-manager'
ASSETGUARD_PATH = find_assetguard_path()


# ============================================= AssetGuard constants - Commands =============================================
CHECK_CONFIG_COMMAND = 'check-manager-configuration'
RESTART_ASSETGUARD_COMMAND = 'restart-assetguard'


# =========================================== AssetGuard constants - Date format ============================================
DATE_FORMAT = "%Y-%m-%dT%H:%M:%SZ"
DECIMALS_DATE_FORMAT = "%Y-%m-%dT%H:%M:%S.%fZ"


# ========================================= AssetGuard constants - Size and limits ==========================================
...
# ============================================= AssetGuard constants - Version ==============================================
# Agent upgrading variables.
WPK_REPO_URL_4_X = "packages.assetguard.com/4.x/wpk/"
...
AR_LEGACY_VERSION = 'AssetGuard v4.2.0'
ACTIVE_CONFIG_VERSION = 'AssetGuard v4.0.0'
...
CLIENT_KEYS = os.path.join(ASSETGUARD_PATH, 'etc', 'client.keys')
SHARED_PATH = os.path.join(ASSETGUARD_PATH, 'etc', 'shared')


# ================================================= AssetGuard path - Misc ==================================================
ASSETGUARD_LOGS = os.path.join(ASSETGUARD_PATH, 'logs')
ASSETGUARD_LOG = os.path.join(ASSETGUARD_LOGS, 'assetguard-manager.log')
ASSETGUARD_LOG_JSON = os.path.join(ASSETGUARD_LOGS, 'assetguard-manager.json')
DATABASE_PATH = os.path.join(ASSETGUARD_PATH, 'var', 'db')
...
BACKUP_PATH = os.path.join(ASSETGUARD_PATH, 'backup')
MULTI_GROUPS_PATH = os.path.join(ASSETGUARD_PATH, 'var', 'multigroups')
DEFAULT_RBAC_RESOURCES = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'rbac', 'default')
ASSETGUARD_LOCALTIME_PATH = os.path.join(ASSETGUARD_PATH, 'etc', 'localtime')


# ================================================ AssetGuard path - Sockets ================================================
ANALYSISD_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'analysis')
AR_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'alerts', 'ar')
AUTHD_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'auth')
CONTROL_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'control')
UPGRADE_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'tasks', 'upgrade')
REMOTED_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'remote')
TASKS_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'tasks', 'task')
WDB_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'db', 'wdb')
WDB_HTTP_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'wdb-http')
WMODULES_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'wmodules')
QUEUE_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'queue')
KEY_STORE_SOCKET = os.path.join(ASSETGUARD_PATH, 'queue', 'sockets', 'keystore')

# ========================================== INSTALLATION UID PATH ====================================================
SECURITY_PATH = os.path.join(ASSETGUARD_PATH, 'api', 'configuration', 'security')
