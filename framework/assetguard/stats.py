...
from assetguard.core.agent import Agent, get_agents_info, get_rbac_filters, AssetGuardDBQueryAgents
...
from assetguard.core.stats import get_daemons_stats_, get_daemons_stats_socket, hourly_, totals_, weekly_
...
def totals(date: datetime.date) -> AffectedItemsAssetGuardResult:
    ...
    """
    result = AffectedItemsAssetGuardResult(all_msg='Statistical information for each node was successfully read',
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def hourly() -> AffectedItemsAssetGuardResult:
    ...
    """
    result = AffectedItemsAssetGuardResult(all_msg='Statistical information per hour for each node was successfully read',
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def weekly() -> AffectedItemsAssetGuardResult:
    ...
    """
    result = AffectedItemsAssetGuardResult(all_msg='Statistical information per week for each node was successfully read',
...
def get_daemons_stats_agents(daemons_list: list = None, agent_list: list = None):
    ...
    daemon_socket_mapping = {'assetguard-manager-remoted': common.REMOTED_SOCKET,
                             'assetguard-manager-analysisd': common.ANALYSISD_SOCKET}
    result = AffectedItemsAssetGuardResult(all_msg='Statistical information for each daemon was successfully read',
    ...
    if agent_list:
        if 'all' not in agent_list:
            ...
            with AssetGuardDBQueryAgents(limit=None, select=["id", "status"], **rbac_filters) as db_query:
                data = db_query.run()
            ...
            [result.add_failed_item(id_=agent, error=exception.AssetGuardResourceNotFound(1701)) for agent in
             not_found_agents]

            # Add non-active agents to failed_items
            non_active_agents = [agent['id'] for agent in data['items'] if agent['status'] != 'active']
            [result.add_failed_item(id_=agent, error=exception.AssetGuardError(1707)) for agent in non_active_agents]
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_daemons_stats(daemons_list: list = None) -> AffectedItemsAssetGuardResult:
    ...
    daemon_socket_mapping = {'assetguard-manager-remoted': common.REMOTED_SOCKET,
                             'assetguard-manager-analysisd': common.ANALYSISD_SOCKET,
                             'assetguard-manager-db': common.WDB_SOCKET}
    result = AffectedItemsAssetGuardResult(all_msg='Statistical information for each daemon was successfully read',
                                      some_msg='Could not read statistical information for some daemons',
                                      none_msg='Could not read statistical information for any daemon')
...
def get_agents_component_stats_json(agent_list: list = None, component: str = None) -> AffectedItemsAssetGuardResult:
    ...
    AffectedItemsAssetGuardResult
        Component stats.
    """
    result = AffectedItemsAssetGuardResult(all_msg='Statistical information for each agent was successfully read',
    ...
    for agent_id in agent_list:
        try:
            if agent_id not in system_agents:
                raise exception.AssetGuardResourceNotFound(1701)
