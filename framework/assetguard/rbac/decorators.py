...
from assetguard.core.agent import get_agents_info, get_groups, expand_group
from assetguard.core.common import rbac, broadcast, cluster_nodes
from assetguard.core.exception import AssetGuardPermissionError
from assetguard.core.results import AffectedItemsAssetGuardResult
from assetguard.rbac.orm import RolesManager, PoliciesManager, AuthenticationManager, RulesManager
...
def list_handler(result: AffectedItemsAssetGuardResult, original: dict = None, allowed: dict = None, target: dict = None,
                 add_denied: bool = False, **post_proc_kwargs: dict) -> AffectedItemsAssetGuardResult:
        ...
        for res_id, target_param in target.items():
            ...
            for denied_item in denied:
                result.add_failed_item(id_=denied_item,
                                       error=AssetGuardPermissionError(4000, extra_message=f'Resource type: {res_id}',
                                                                  ids=denied))
    if not add_denied or post_proc_kwargs.get('force'):
        ...
        if 'default_result_kwargs' in post_proc_kwargs and result is None:
            return AffectedItemsAssetGuardResult(**post_proc_kwargs['default_result_kwargs'])
...
def expose_resources(actions: list = None, resources: list = None, post_proc_func: callable = list_handler,
    ...
    """Decorator to apply user permissions on a AssetGuard framework function based on exposed action:resource pairs.
            ...
            for res_id, target_param in target_params.items():
                try:
                    ...
                    if add_denied:
                        ...
                        if res_id in integer_resources:
                            denied = {int(i) if i.isdigit() else i for i in denied}
                        raise AssetGuardPermissionError(4000,
...
def _mask_payload(payload, mask_text: str = MASK_DEFAULT) -> None:
    ...
    ----------
    payload :
        One of: dict, list, or AffectedItemsAssetGuardResult. Other types are ignored.
    mask_text : str
        Replacement text used for masked values.
    """
    if isinstance(payload, AffectedItemsAssetGuardResult):
