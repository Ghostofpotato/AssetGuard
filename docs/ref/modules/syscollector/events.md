...
This document describes the event formats generated by the Syscollector module in AssetGuard. These events use AssetGuard's custom field structure for inventory data.
...
```json
{
    ...
    {
        "browser_name": "chrome",
        "browser_profile_name": "Nico AssetGuard",
...
Syscollector events are processed by the Inventory Harvester and stored directly in AssetGuard Indexer without going through traditional rule processing, since they represent inventory state rather than security events.

## Integration

Access syscollector inventory data through:

- **AssetGuard API**: REST endpoints for querying current inventory state
- **AssetGuard Dashboard**: Built-in inventory visualization
- **Direct Database**: Query AssetGuard Indexer for raw event data