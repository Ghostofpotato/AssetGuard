# AssetGuard RBAC - How to create and map internal users

AssetGuard RBAC allows access to AssetGuard resources based on the roles and policies assigned to the users. It is an easy-to-use administration system that enables managing users' or entities' permissions to the system resources.

To learn more, see the Role-Based Access Control section.

The AssetGuard platform includes an internal user database that can be used for authentication. It can also be used in addition to an external authentication system such as LDAP or Active Directory. Learn how to create users and map them with AssetGuard in the sections below.

- Creating and setting a AssetGuard admin user
- Creating and setting a AssetGuard read-only user
- Creating an internal user and mapping it to AssetGuard
...
1. Log into the AssetGuard dashboard as administrator.
...
4. To map the user with AssetGuard:
...
```
/usr/share/assetguard-dashboard/data/assetguard/config/assetguard.yml
```
