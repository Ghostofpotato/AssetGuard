...
> **Note:** AssetGuard 5.0 uses JSON format with ECS (Elastic Common Schema) for alerts stored in `/var/ossec/logs/alerts/alerts.json`. All examples in this document are from actual alerts captured in a AssetGuard deployment.
...
```json
{
  ...
  "assetguard": {
    ...
    "integration": {
      "category": "security",
      "name": "assetguard-rootcheck",
      "decoders": ["core-assetguard-message", "integrations", "assetguard-rootcheck"]
    }
  },
  "agent": {
    "name": "ubuntu22",
    "version": "AssetGuard v5.0.0",
    "id": "002"
  },
  "host": {
    "os": {
...
- `assetguard_rootcheck.anomaly.type`: `dev-file`
...
- `assetguard_rootcheck.anomaly.type`: `hidden-process`
...
- `assetguard_rootcheck.anomaly.type`: `hidden-port`
...
```json
{
  ...
  "event": {
    "original": "File '/tmp/assetguard_rootcheck_test_177799/sys_test/world_writable_root_file' is owned by root and has written permissions to anyone.",
  ...
  "file": {
    "path": "/tmp/assetguard_rootcheck_test_177799/sys_test/world_writable_root_file",
    "directory": "/tmp/assetguard_rootcheck_test_177799/sys_test",
...
- `assetguard_rootcheck.vulnerability.type`: `world-writable`
...
| `assetguard.protocol.location` | Alert source | `rootcheck` |
| `agent.id` | Agent identifier | `002` |
| `agent.name` | Agent hostname | `ubuntu22` |
| `agent.version` | AssetGuard agent version | `AssetGuard v5.0.0` |
...
| `assetguard_rootcheck.anomaly.type` | Anomaly detections | Type of anomaly detected |
| `assetguard_rootcheck.vulnerability.type` | Vulnerabilities | Type of vulnerability |
| `assetguard_rootcheck.file.permissions` | File permissions | Permission mode |

### Anomaly and Vulnerability Types

The `assetguard_rootcheck.anomaly.type` field indicates the detection type:
...
The `assetguard_rootcheck.vulnerability.type` field indicates vulnerability types:
...
```bash
tail -f /var/ossec/logs/alerts/alerts.json | jq 'select(.assetguard.protocol.location=="rootcheck")'
```
...
tail -f /var/ossec/logs/alerts/alerts.json | jq 'select(.assetguard.protocol.location=="rootcheck") | {
  ...
  anomaly: .assetguard_rootcheck.anomaly.type
}'
```

**Exclude scan start/end messages:**
```bash
tail -f /var/ossec/logs/alerts/alerts.json | jq 'select(.assetguard.protocol.location=="rootcheck" and (.event.original | contains("Starting") or contains("Ending") | not))'
```
...
```bash
jq 'select(.assetguard.protocol.location=="rootcheck" and .assetguard_rootcheck.anomaly.type=="dev-file")' /var/ossec/logs/alerts/alerts.json
```

**World-writable files:**
```bash
jq 'select(.assetguard.protocol.location=="rootcheck" and .assetguard_rootcheck.vulnerability.type=="world-writable")' /var/ossec/logs/alerts/alerts.json
```

**Promiscuous mode (search in message):**
```bash
jq 'select(.assetguard.protocol.location=="rootcheck" and (.event.original | contains("promiscuous")))' /var/ossec/logs/alerts/alerts.json
```

**Specific agent:**
```bash
jq 'select(.assetguard.protocol.location=="rootcheck" and .agent.name=="ubuntu22")' /var/ossec/logs/alerts/alerts.json
```

### On AssetGuard Dashboard

Navigate to **Threat Hunting** or **Discover** module and use these filters:

**All rootcheck alerts:**
```
assetguard.protocol.location: rootcheck
```

**Exclude scan messages:**
```
assetguard.protocol.location: rootcheck AND NOT event.original: ("Starting rootcheck scan" OR "Ending rootcheck scan")
```
...
**Specific agent:**
```
assetguard.protocol.location: rootcheck AND agent.name: ubuntu22
```

**By event category:**
```
assetguard.protocol.location: rootcheck AND event.category: malware
```
...
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | .event.original' /var/ossec/logs/alerts/alerts.json
```

**Count alerts by anomaly type:**
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | .assetguard_rootcheck.anomaly.type // empty' /var/ossec/logs/alerts/alerts.json | sort | uniq -c
```

**List affected agents:**
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | .agent.name' /var/ossec/logs/alerts/alerts.json | sort -u
```

**Export alerts to CSV:**
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | [."@timestamp", .agent.name, .event.original, .assetguard_rootcheck.anomaly.type // "N/A"] | @csv' /var/ossec/logs/alerts/alerts.json > rootcheck_alerts.csv
```

**Get file paths from detections:**
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck" and .file.path?) | .file.path' /var/ossec/logs/alerts/alerts.json
```
...
jq --arg time "$ONE_HOUR_AGO" 'select(.assetguard.protocol.location=="rootcheck" and ."@timestamp" > $time)' /var/ossec/logs/alerts/alerts.json
```

**Multiple conditions:**
```bash
# Malware category alerts from specific agent
jq 'select(.assetguard.protocol.location=="rootcheck" and .agent.name=="ubuntu22" and (.event.category | contains(["malware"])))' /var/ossec/logs/alerts/alerts.json
```

**Group by agent and anomaly type:**
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | "\(.agent.name),\(.assetguard_rootcheck.anomaly.type // "N/A")"' /var/ossec/logs/alerts/alerts.json | sort | uniq -c
```
...
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | .agent.name' /var/ossec/logs/alerts/alerts.json | sort | uniq -c | sort -rn
```

**Alerts timeline (by hour):**
```bash
jq -r 'select(.assetguard.protocol.location=="rootcheck") | ."@timestamp" | split("T")[1] | split(":")[0]' /var/ossec/logs/alerts/alerts.json | sort | uniq -c
```
