...
This guide provides instructions for backing up and restoring AssetGuard manager and agent data. Regular backups are essential for disaster recovery and should be performed before major operations such as upgrades or configuration changes.
...
The following components should be included in your AssetGuard manager backup strategy:

#### Essential Data

- **Configuration files**: `/var/assetguard-manager/etc/`
  - `assetguard-manager.conf` - Main configuration file
  - `assetguard-manager-internal-options.conf` - Internal configuration overrides

- **Agent keys**: `/var/assetguard-manager/etc/client.keys`
  - Contains encryption keys for registered agents
  - Critical for agent communication

- **SSL/TLS certificates**: `/var/assetguard-manager/etc/certs/`
  - Manager certificates and keys
  - Root CA certificates

- **Global database**: `/var/assetguard-manager/var/db/global.db`
  - Agent information (registration, metadata)
  - Agent group assignments
  - Group membership data

- **Agent groups**: `/var/assetguard-manager/etc/shared/`
  - Group-specific configurations and files
  - Shared files distributed to agents in each group

#### Optional Data

- **Logs**: `/var/assetguard-manager/logs/`
...
# Stop the AssetGuard manager
sudo systemctl stop assetguard-manager

# Backup essential directories
sudo tar -czf $BACKUP_DIR/assetguard-manager-backup.tar.gz \
    -C /var/assetguard-manager \
    etc/ \
    var/db/global.db

# Start the AssetGuard manager
...
```bash
sudo tar -czf assetguard-manager-config-$(date +%Y%m%d).tar.gz -C /var/assetguard-manager etc/
```

**Agent keys only:**

```bash
sudo cp /var/assetguard-manager/etc/client.keys assetguard-client-keys-$(date +%Y%m%d).backup
```

**Global database only:**

```bash
sudo sqlite3 /var/assetguard-manager/var/db/global.db ".backup 'assetguard-global-db-$(date +%Y%m%d).db'"
```
...
tar -tzf $BACKUP_DIR/assetguard-etc.tar.gz > /dev/null && echo "Configuration backup verified" || echo "Backup verification failed"
...
tar -tzf $BACKUP_DIR/assetguard-etc.tar.gz | head -20
```
...
```bash
#!/bin/bash
# /usr/local/bin/assetguard-manager-backup.sh

BACKUP_BASE="/backup/assetguard-manager"
RETENTION_DAYS=30
...
# Perform backup
tar -czf $BACKUP_DIR/assetguard-etc.tar.gz -C /var/assetguard-manager etc/
sqlite3 /var/assetguard-manager/var/db/global.db ".backup '$BACKUP_DIR/db/global.db'"

# Verify backup
if tar -tzf $BACKUP_DIR/assetguard-etc.tar.gz > /dev/null 2>&1 && \
   sqlite3 $BACKUP_DIR/db/global.db "PRAGMA integrity_check" > /dev/null 2>&1; then
    echo "$(date): Manager backup completed successfully to $BACKUP_DIR" >> /var/log/assetguard-backup.log

    # Remove old backups
    find $BACKUP_BASE -type d -name "backup-*" -mtime +$RETENTION_DAYS -exec rm -rf {} \;
else
    echo "$(date): Manager backup FAILED - verification error" >> /var/log/assetguard-backup.log
    exit 1
fi
```
...
0 2 * * * /usr/local/bin/assetguard-manager-backup.sh
```
...
2. Ensure compatible AssetGuard version
...
**Step 1: Stop the AssetGuard manager**
...
```bash
sudo mv /var/assetguard-manager/etc /var/assetguard-manager/etc.old.$(date +%Y%m%d)
sudo mv /var/assetguard-manager/var/db/global.db /var/assetguard-manager/var/db/global.db.old.$(date +%Y%m%d)
```
...
sudo tail -f /var/assetguard-manager/logs/assetguard-manager.log
```
...
```bash
sudo systemctl stop assetguard-manager
sudo tar -xzf assetguard-manager-config-YYYYMMDD.tar.gz -C /var/assetguard-manager
sudo chown -R assetguard-manager:assetguard-manager /var/assetguard-manager/etc
sudo systemctl start assetguard-manager
```
...
```bash
sudo systemctl stop assetguard-manager
sudo cp assetguard-client-keys-YYYYMMDD.backup /var/assetguard-manager/etc/client.keys
sudo chown assetguard-manager:assetguard-manager /var/assetguard-manager/etc/client.keys
sudo chmod 640 /var/assetguard-manager/etc/client.keys
sudo systemctl start assetguard-manager
```
...
```bash
sudo systemctl stop assetguard-manager
sudo cp assetguard-global-db-YYYYMMDD.db /var/assetguard-manager/var/db/global.db
sudo chown assetguard-manager:assetguard-manager /var/assetguard-manager/var/db/global.db
sudo chmod 640 /var/assetguard-manager/var/db/global.db
sudo systemctl start assetguard-manager
```
...
sudo tar -czf $BACKUP_DIR/assetguard-worker-config.tar.gz -C /var/assetguard-manager/etc assetguard-manager.conf assetguard-manager-internal-options.conf
```
...
2. Verify cluster configuration in `/var/assetguard-manager/etc/assetguard-manager.conf`
3. Start the manager service
4. Verify cluster status: `sudo /var/assetguard-manager/bin/cluster_control -l`
...
The following components should be included in your AssetGuard agent backup strategy:
...
```bash
# Create backup directory with timestamp
BACKUP_DIR="/backup/assetguard-agent-$(date +%Y%m%d-%H%M%S)"
sudo mkdir -p $BACKUP_DIR

# Backup configuration and agent key
sudo tar -czf $BACKUP_DIR/assetguard-agent-etc.tar.gz -C /var/ossec etc/

# Optional: Backup local databases
sudo tar -czf $BACKUP_DIR/assetguard-agent-db.tar.gz -C /var/ossec queue/fim/db/ queue/syscollector/db/ queue/sca/ 2>/dev/null || true
...
```powershell
# Create backup directory with timestamp
$BackupDir = "C:\backup\assetguard-agent-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
# Backup configuration and agent key
sudo tar -czf $BACKUP_DIR/assetguard-agent-etc.tar.gz -C /Library/Ossec etc/

# Optional: Backup local databases
sudo tar -czf $BACKUP_DIR/assetguard-agent-db.tar.gz -C /Library/Ossec queue/fim/db/ queue/syscollector/db/ queue/sca/ 2>/dev/null || true
```
...
```bash
sudo tar -czf assetguard-agent-config-$(date +%Y%m%d).tar.gz -C /var/ossec etc/ossec.conf etc/client.keys etc/local_internal_options.conf
```

**Agent key only (Linux):**

```bash
sudo cp /var/ossec/etc/client.keys assetguard-agent-key-$(date +%Y%m%d).backup
```
...
```bash
# Stop the agent
sudo systemctl stop assetguard-agent

# Backup current configuration (optional)
sudo mv /var/ossec/etc /var/ossec/etc.old.$(date +%Y%m%d)

# Restore from backup
sudo tar -xzf $BACKUP_DIR/assetguard-agent-etc.tar.gz -C /var/ossec

# Optional: Restore databases
sudo tar -xzf $BACKUP_DIR/assetguard-agent-db.tar.gz -C /var/ossec 2>/dev/null || true
...
# Restore from backup
sudo tar -xzf $BACKUP_DIR/assetguard-agent-etc.tar.gz -C /Library/Ossec

# Optional: Restore databases
sudo tar -xzf $BACKUP_DIR/assetguard-agent-db.tar.gz -C /Library/Ossec 2>/dev/null || true

# Set proper permissions
sudo chown -R root:assetguard /Library/Ossec/etc
sudo chmod 640 /Library/Ossec/etc/client.keys

# Start the agent
sudo /Library/Ossec/bin/assetguard-control start
...
sudo sqlite3 /var/assetguard-manager/var/db/global.db "SELECT id, name FROM agent"
...
sudo systemctl stop assetguard-manager
sudo rm /var/assetguard-manager/var/db/global.db
sudo cp $BACKUP_DIR/db/global.db /var/assetguard-manager/var/db/global.db
sudo chown assetguard-manager:assetguard-manager /var/assetguard-manager/var/db/global.db
sudo systemctl start assetguard-manager
```
...
# Set proper permissions
sudo chown -R root:assetguard /var/ossec/queue
